...
# 本地二次开发：openai_proxy 模式下无需本地 vLLM（Mac 上装不了），按模式条件加载
_DURIAN_BOOT_ENGINE_MODE = os.getenv("DURIAN_ENGINE_MODE", "local_vllm").strip().lower()
if _DURIAN_BOOT_ENGINE_MODE in {"openai_proxy", "proxy", "vllm_server", "remote_vllm"}:
    LLM = SamplingParams = LoRARequest = None
else:
...
RAG_CONFIG = {
    ...
    "top_k": int(os.getenv("DURIAN_RAG_TOP_K", "6")),
    "candidate_top_k": int(os.getenv("DURIAN_RAG_CANDIDATE_TOP_K", "36")),
    "search_multiplier": int(os.getenv("DURIAN_RAG_SEARCH_MULTIPLIER", "5")),
    "query_timeout_sec": float(os.getenv("DURIAN_RAG_QUERY_TIMEOUT_SEC", "6.0")),
    "lock_wait_sec": float(os.getenv("DURIAN_RAG_LOCK_WAIT_SEC", "4.0")),
    "translate_timeout_sec": float(os.getenv("DURIAN_RAG_TRANSLATE_TIMEOUT_SEC", "2.0")),
    "min_score": float(os.getenv("DURIAN_RAG_MIN_SCORE", "0.35")),
    "fallback_min_score": float(os.getenv("DURIAN_RAG_FALLBACK_MIN_SCORE", "-1.0")),
    "max_evidence_chars": int(os.getenv("DURIAN_RAG_MAX_EVIDENCE_CHARS", "900")),
    "canonical_language": os.getenv("DURIAN_RAG_CANONICAL_LANGUAGE", "zh").strip().lower() or "zh",
    "require_canonical_text": env_bool("DURIAN_RAG_REQUIRE_CANONICAL_TEXT", True),
...
RAGFLOW_CONFIG = {
    "enabled": env_bool("DURIAN_RAGFLOW_ENABLED", False),
    "base_url": os.getenv(
        "DURIAN_RAGFLOW_BASE_URL",
        "http://127.0.0.1:19380/api/v1",
    ).rstrip("/"),
    "api_key": os.getenv("DURIAN_RAGFLOW_API_KEY", "").strip(),
    "dataset_ids": [
        value.strip()
        for value in os.getenv("DURIAN_RAGFLOW_DATASET_IDS", "").split(",")
        if value.strip()
    ],
    "top_k": int(os.getenv("DURIAN_RAGFLOW_TOP_K", "12")),
    "candidate_top_k": int(os.getenv("DURIAN_RAGFLOW_CANDIDATE_TOP_K", "16")),
    "timeout_sec": float(os.getenv("DURIAN_RAGFLOW_TIMEOUT_SEC", "8.0")),
    "fallback_enabled": env_bool("DURIAN_RAGFLOW_FALLBACK_ENABLED", True),
    "similarity_threshold": float(
        os.getenv("DURIAN_RAGFLOW_SIMILARITY_THRESHOLD", "0.18")
    ),
    "vector_similarity_weight": float(
        os.getenv("DURIAN_RAGFLOW_VECTOR_WEIGHT", "0.35")
    ),
    "cache_ttl_sec": float(os.getenv("DURIAN_RAGFLOW_CACHE_TTL_SEC", "900")),
    "cache_max_entries": int(os.getenv("DURIAN_RAGFLOW_CACHE_MAX_ENTRIES", "512")),
}

WEB_SEARCH_CONFIG = {
    "enabled": env_bool("DURIAN_WEB_SEARCH_ENABLED", True),
    "api_key": os.getenv("TAVILY_API_KEY", "").strip(),
    "endpoint": os.getenv("TAVILY_API_URL", "https://api.tavily.com/search").strip(),
    "top_k": int(os.getenv("DURIAN_WEB_SEARCH_TOP_K", "4")),
    "timeout_sec": float(os.getenv("DURIAN_WEB_SEARCH_TIMEOUT_SEC", "12")),
    "max_content_chars": int(os.getenv("DURIAN_WEB_SEARCH_MAX_CONTENT_CHARS", "1200")),
    "cache_ttl_sec": float(os.getenv("DURIAN_WEB_SEARCH_CACHE_TTL_SEC", "1800")),
    "cache_max_entries": int(os.getenv("DURIAN_WEB_SEARCH_CACHE_MAX_ENTRIES", "256")),
}

REGISTERED_DURIAN_SOURCE_URL = os.getenv(
    "DURIAN_REGISTERED_VARIETIES_URL",
    "https://durianpenang.com/registered-durian",
).strip()
REGISTERED_DURIAN_TIMEOUT_SEC = float(
    os.getenv("DURIAN_REGISTERED_VARIETIES_TIMEOUT_SEC", "8.0")
)

RERANK_CONFIG = {
    "enabled": env_bool("DURIAN_RERANK_ENABLED", False),
    "model": os.getenv("DURIAN_RERANK_MODEL", "BAAI/bge-reranker-v2-m3").strip(),
    "device": os.getenv("DURIAN_RERANK_DEVICE", "cpu").strip(),
    "top_k": int(os.getenv("DURIAN_RERANK_TOP_K", "0")),
    "candidate_limit": int(os.getenv("DURIAN_RERANK_CANDIDATE_LIMIT", "32")),
    "batch_size": int(os.getenv("DURIAN_RERANK_BATCH_SIZE", "4")),
    "max_query_chars": int(os.getenv("DURIAN_RERANK_MAX_QUERY_CHARS", "220")),
    "max_passage_chars": int(os.getenv("DURIAN_RERANK_MAX_PASSAGE_CHARS", "1200")),
    "min_score": float(os.getenv("DURIAN_RERANK_MIN_SCORE", "0.10")),
    "failure_cooldown_sec": float(os.getenv("DURIAN_RERANK_FAILURE_COOLDOWN_SEC", "300")),
    "cache_ttl_sec": float(os.getenv("DURIAN_RERANK_CACHE_TTL_SEC", "1800")),
    "cache_max_entries": int(os.getenv("DURIAN_RERANK_CACHE_MAX_ENTRIES", "512")),
}

RAG_QUERY_LOCK = Lock()
RERANK_LOCK = Lock()
RERANK_CACHE_LOCK = Lock()
WEB_SEARCH_CACHE_LOCK = Lock()
RAGFLOW_CONTEXT_CACHE_LOCK = Lock()
REGISTERED_DURIAN_CACHE_LOCK = Lock()
_RERANK_MODEL = None
_RERANK_LOAD_FAILED_AT = 0.0
_RERANK_LOAD_ERROR = ""
_RERANK_CACHE: Dict[str, Tuple[float, List[Dict[str, Any]]]] = {}
_WEB_SEARCH_CACHE: Dict[str, Tuple[float, List[Dict[str, Any]], str, str]] = {}
_RAGFLOW_CONTEXT_CACHE: Dict[str, Tuple[float, List[Dict[str, Any]], str, str]] = {}
_REGISTERED_DURIAN_CACHE: Dict[str, Tuple[float, List[Dict[str, Any]]]] = {}
...
INTERNAL_EMBEDDING_API_KEY = os.getenv(
    "DURIAN_INTERNAL_EMBEDDING_API_KEY",
    "",
).strip()
APP_USERS = {
    user.strip()
...
SYSTEM_PROMPTS = {
    "zh": (
        "你的唯一角色是榴莲专家，只回答榴莲相关问题。范围包括榴莲品种、编号、口感、食用、保存、购买、市场、种植管理和榴莲病虫害。"
        "如果用户问题明显与榴莲无关，不要使用非榴莲资料回答，不要转成医学、心理、通用农业或其他作物知识；应说明只能回答榴莲相关内容。"
    ...
    "en": (
        "Your only role is a durian expert. Answer only durian-related questions, including durian varieties, codes, taste, eating quality, storage, buying, market, cultivation, and durian pests or diseases. "
        "If the user's question is clearly unrelated to durian, do not answer from non-durian sources and do not switch into medical, psychological, generic farming, or other-crop knowledge; state that you can only answer durian-related questions. "
    ...
    "ms": (
        "Peranan tunggal anda ialah pakar durian. Jawab hanya soalan berkaitan durian, termasuk varieti, kod, rasa, kualiti makan, simpanan, pembelian, pasaran, penanaman, penyakit dan perosak durian. "
        "Jika soalan jelas tidak berkaitan durian, jangan jawab menggunakan sumber bukan durian atau beralih kepada perubatan, psikologi, pertanian umum atau tanaman lain; nyatakan bahawa anda hanya boleh menjawab soalan berkaitan durian. "
    ...
    "th": (
        "บทบาทเดียวของคุณคือผู้เชี่ยวชาญทุเรียน ตอบเฉพาะคำถามที่เกี่ยวกับทุเรียน เช่น สายพันธุ์ รหัส รสชาติ คุณภาพการกิน การเก็บรักษา การซื้อ ตลาด การปลูก โรคและแมลงของทุเรียน. "
        "หากคำถามไม่เกี่ยวกับทุเรียนอย่างชัดเจน อย่าใช้ข้อมูลนอกทุเรียนมาตอบ และอย่าเปลี่ยนไปเป็นความรู้ทางการแพทย์ จิตวิทยา เกษตรทั่วไป หรือพืชชนิดอื่น; ให้ระบุว่าคุณตอบได้เฉพาะเรื่องทุเรียน. "
...
class Message(BaseModel):
    ...
    evidence: Optional[List[Dict[str, Any]]] = Field(default=None, description="本条 assistant 消息绑定的 RAG evidence")
    evidence_quality: Optional[str] = Field(default="none", description="本条 assistant 消息的 RAG evidence quality")
...
class InferenceRequest(BaseModel):
    conversation_id: Optional[str] = Field(default=None, description="LangGraph thread / 对话 ID")
...
class OpenAIEmbeddingRequest(BaseModel):
    ...
    model: str = Field(default="durian-e5")
...
def verify_internal_embedding_api_key(
    x_api_key: str = Header(default=""),
    authorization: str = Header(default=""),
):
    if not INTERNAL_EMBEDDING_API_KEY:
        raise HTTPException(
            status_code=503,
            detail="Internal embedding API key is not configured",
        )
    token = ""
    ...
    if not secrets_compare(token, INTERNAL_EMBEDDING_API_KEY):
...
class ConversationStore:
    ...
    def _init_db(self):
        with self._connect() as conn:
            ...
            # 迁移：保存每条 assistant 消息对应的 RAG evidence。
            try:
                conn.execute("ALTER TABLE messages ADD COLUMN evidence_json TEXT")
                conn.commit()
                logger.info("✓ 已为 messages 表添加 evidence_json 列")
            except sqlite3.OperationalError:
                pass

            try:
                conn.execute("ALTER TABLE messages ADD COLUMN evidence_quality TEXT DEFAULT 'none'")
                conn.commit()
                logger.info("✓ 已为 messages 表添加 evidence_quality 列")
            except sqlite3.OperationalError:
                pass
    ...
    def get_conversation(self, conv_id: str, owner: str) -> Dict:
        with self._connect() as conn:
            ...
            messages = conn.execute(
                """
                SELECT role, content, image_url, active_context_card_json, created_at, evidence_json, evidence_quality
                FROM messages
        ...
        return {
            ...
            "messages": [
                {
                    "role": m[0],
                    "content": self._cap_visible_message_content(
                        self._strip_hidden_context_from_content(m[1] or ""),
                        m[0],
                    ),
                    "image_url": m[2],
                    "active_context_card": self._parse_context_card_json(m[3]),
                    "created_at": m[4],
                    "evidence": self._parse_evidence_json(m[5]),
                    "evidence_quality": m[6] or "none",
                }
                for m in messages
            ],
        }
    ...
    @staticmethod
    def _cap_visible_message_content(content: str, role: str = "assistant") -> str:
        text = str(content or "")
        max_chars = max(1000, int(os.getenv("DURIAN_STORE_MESSAGE_MAX_CHARS", "12000")))
        if str(role or "") == "user":
            max_chars = max(1000, int(os.getenv("DURIAN_STORE_USER_MESSAGE_MAX_CHARS", "4000")))
        if len(text) <= max_chars:
            return text
        head = max(1, int(max_chars * 0.78))
    ...
    @staticmethod
    def _parse_evidence_json(raw: Optional[str]) -> List[Dict[str, Any]]:
        if not raw:
            return []
        try:
            obj = json.loads(raw)
    ...
    def add_message(self, conv_id: str, owner: str, role: str, content: str, image_url: Optional[str] = None, active_context_card: Optional[Dict[str, Any]] = None, evidence: Optional[List[Dict[str, Any]]] = None, evidence_quality: Optional[str] = "none"):
        now = datetime.now().isoformat()
        msg_id = str(uuid.uuid4())
        visible_content = self._strip_hidden_context_from_content(content or "")
        capped_content = self._cap_visible_message_content(visible_content, role)
        if len(capped_content) != len(visible_content):
            logger.info(
                "Stored message content capped: role=%s chars=%d->%d conv_id=%s",
                role,
                len(visible_content),
                len(capped_content),
                conv_id,
            )
        visible_content = capped_content
        card_json = json.dumps(active_context_card, ensure_ascii=False) if isinstance(active_context_card, dict) else None
        evidence_json = json.dumps(evidence, ensure_ascii=False) if isinstance(evidence, list) else None
        evidence_quality_value = evidence_quality or "none" 
        ...
        with self._connect() as conn:
            ...
            conn.execute(
                """
                INSERT INTO messages(id, conversation_id, role, content, image_url, active_context_card_json, created_at, evidence_json, evidence_quality)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (msg_id, conv_id, role, visible_content, image_url, card_json, now, evidence_json, evidence_quality_value),
            )
...
def clean_mineru_ocr_text(raw: str) -> str:
    """Extract readable text from MinerU OCR dict-like fragments."""
    value = str(raw or "")
    if not value:
        return ""

    # Extract values from patterns like: 'content': 'xxx' or "content": "xxx"
    parts = []
    for m in re.finditer(r"""['"]content['"]\s*:\s*(['"])(.*?)\1""", value, flags=re.S):
        part = m.group(2)
        part = part.replace("\\n", "\n").replace("\\\\", "\\")
        ...
        if not part:
            continue
        # Drop pure equation/noisy fragments that are too short.
        if len(part) <= 2:
            continue
        if part.lower() in {"text", "image", "inline_equation"}:
            continue
        parts.append(part)
    ...
    # Remove remaining parser markers if any.
    cleaned = re.sub(r"\{?'bbox':\s*\[[^\]]+\],?", " ", cleaned)
    cleaned = re.sub(r'\{?"bbox":\s*\[[^\]]+\],?', " ", cleaned)
    cleaned = re.sub(r"['\"](?:type|angle|index|page_idx|page_size|image_path)['\"]\s*:\s*[^,}\]]+", " ", cleaned)
    cleaned = re.sub(r"\s+", " ", cleaned).strip()
    return cleaned



def clean_pdf_rag_text(raw: str) -> str:
    """Clean text extracted from uploaded PDFs / OCR parser outputs."""
    text = str(raw or "")
    if not text:
        return ""
    ...
    # If MinerU/parser structures leaked into text, extract human-readable content first.
    parser_markers = [
        "'bbox'", '"bbox"', "bbox:",
        "'spans'", '"spans"',
        "'lines'", '"lines"',
    ...
    if sum(1 for marker in parser_markers if marker in lower) >= 2:
        try:
            text = clean_mineru_ocr_text(text)
        except Exception:
            pass

    # Remove remaining parser fragments.
    text = re.sub(r"\{?\s*['\"]bbox['\"]\s*:\s*\[[^\]]*\]\s*,?", " ", text)
    text = re.sub(r"\{?\s*['\"](?:spans|lines|para_blocks|discarded_blocks)['\"]\s*:\s*\[[\s\S]*?\]\s*,?", " ", text)
...
def is_pdf_front_matter_or_parser_noise(chunk: Dict[str, Any]) -> bool:
    """Drop PDF chunks that are not useful agronomic evidence."""
    ...
    text = " ".join([
        ...
        str(chunk.get("index_text") or ""),
        str(chunk.get("original_text") or ""),
        str(chunk.get("evidence") or ""),
        str(chunk.get("doc") or ""),
    ...
    if sum(1 for marker in parser_markers if marker in lower) >= 3:
        return True

    # 书籍前言、作者分工、资料整理，不适合作为 RAG 证据。
    ...
    if any(marker in compact for marker in front_matter_markers):
        return True

    return False


def sanitize_rag_chunk_before_index(chunk: Dict[str, Any]) -> Dict[str, Any]:
    """Clean chunk text before noise filtering and indexing."""
    if not isinstance(chunk, dict):
        return chunk

    cleaned = chunk.copy()

    for field in ["text", "content", "index_text", "original_text", "display_text", "evidence"]:
        if field in cleaned and cleaned.get(field):
            cleaned[field] = clean_pdf_rag_text(str(cleaned.get(field) or ""))
...
def _rag_canonical_language() -> str:
    lang = str(RAG_CONFIG.get("canonical_language") or "zh").strip().lower()
    return lang if lang in {"zh", "en", "ms", "th"} else "zh"


def _normalize_rag_language(value: Any) -> str:
    lang = str(value or "").strip().lower().replace("_", "-")
    if lang in {"zh", "zh-cn", "zh-hans", "cn", "chinese"}:
        return "zh"
    if lang in {"en", "en-us", "en-gb", "english"}:
        return "en"
    if lang in {"ms", "ms-my", "malay", "bahasa", "bahasa-melayu"}:
        return "ms"
    if lang in {"th", "th-th", "thai"}:
        return "th"
    return ""


def _rag_declared_language(chunk: Dict[str, Any]) -> str:
    if not isinstance(chunk, dict):
        return ""
    direct = _normalize_rag_language(chunk.get("language") or chunk.get("lang"))
    if direct:
        return direct
    meta = chunk.get("metadata")
    if isinstance(meta, dict):
        return _normalize_rag_language(meta.get("language") or meta.get("lang"))
    return ""


def _rag_script_language_hint(text: str) -> str:
    value = str(text or "")
    cjk = len(re.findall(r"[\u4e00-\u9fff]", value))
    thai = len(re.findall(r"[\u0e00-\u0e7f]", value))
    latin = len(re.findall(r"[A-Za-z]", value))
    if cjk >= max(4, thai, latin // 6):
        return "zh"
    if thai >= max(8, cjk * 2, latin // 4):
        return "th"
    if latin >= 12:
        return "en"
    return "unknown"
...
def _first_nonempty_field(chunk: Dict[str, Any], fields: List[str]) -> str:
    for field in fields:
...
def _rag_canonical_qa_text(chunk: Dict[str, Any], lang: str) -> str:
    question = _first_nonempty_field(
        chunk,
        [
            f"question_{lang}",
            f"{lang}_question",
            f"translated_question_{lang}",
            f"question_translation_{lang}",
        ],
    )
    answer = _first_nonempty_field(
        chunk,
        [
            f"answer_{lang}",
            f"{lang}_answer",
            f"translated_answer_{lang}",
            f"answer_translation_{lang}",
        ],
    )
    ...
    return "\n".join(
        part
        for part in [f"Question: {question}" if question else "", f"Answer: {answer}" if answer else ""]
        if part
    )


def rag_index_text(chunk: Dict[str, Any]) -> str:
    """Canonical text used for embedding/search. Original text is only a fallback."""
    if not isinstance(chunk, dict):
        return ""

    lang = _rag_canonical_language()
    canonical_fields_by_lang = {
        "zh": [
            "canonical_text", "canonical_zh", "text_zh", "zh_text",
            "translated_text_zh", "translation_zh", "translated_zh",
            "translated_text", "translation",
        ],
        "en": [
            "canonical_text", "canonical_en", "text_en", "en_text",
            "translated_text_en", "translation_en", "translated_en",
            "translated_text", "translation",
        ],
        "ms": [
            "canonical_text", "canonical_ms", "text_ms", "ms_text",
            "translated_text_ms", "translation_ms", "translated_ms",
            "translated_text", "translation",
        ],
        "th": [
            "canonical_text", "canonical_th", "text_th", "th_text",
            "translated_text_th", "translation_th", "translated_th",
            "translated_text", "translation",
        ],
    }
    fields = canonical_fields_by_lang.get(lang, canonical_fields_by_lang["zh"])
    text = _first_nonempty_field(chunk, fields)
    if text:
        return text

    qa_text = _rag_canonical_qa_text(chunk, lang)
    if qa_text:
        return qa_text

    raw_text = _rag_chunk_text(chunk).strip()
    if not raw_text:
        return ""

    # MinerU OCR chunks often store parser structures as dict-like strings.
    # Use only the human-readable content values for indexing/display.
    if str(chunk.get("source_file") or "").lower() == "ocr_texts.json":
        cleaned_ocr = clean_mineru_ocr_text(raw_text)
        if cleaned_ocr:
            raw_text = cleaned_ocr

    declared_lang = _rag_declared_language(chunk)
    ...
    raw_lang = declared_lang or _rag_script_language_hint(raw_text)
    if raw_lang == lang or raw_lang == "unknown":
        return raw_text

    if not bool(RAG_CONFIG.get("require_canonical_text", True)):
        return raw_text
    return ""


def _rag_support_text(chunk: Dict[str, Any]) -> str:
    if not isinstance(chunk, dict):
        return ""
    return str(
        chunk.get("evidence")
        or chunk.get("source_evidence")
        or chunk.get("reference")
        or ""
    ).strip()


def is_synthetic_qa_chunk(chunk: Dict[str, Any]) -> bool:
    if not isinstance(chunk, dict):
        return False
    if str(chunk.get("type") or "").lower() == "qa_pair":
        return True
    if chunk.get("question") or chunk.get("answer"):
        return True
    doc = str(chunk.get("doc") or "")
    return doc.startswith("FAQ_")


def is_weak_synthetic_qa_chunk(chunk: Dict[str, Any]) -> bool:
    if not is_synthetic_qa_chunk(chunk):
        return False
    if not env_bool("DURIAN_RAG_DROP_WEAK_SYNTHETIC_QA", True):
        return False

    text = _rag_chunk_text(chunk)
    support = _rag_support_text(chunk)
    blob = f"{text}\n{support}".lower()
    weak_markers = [
        "no specific text",
    ...
    return any(marker in blob for marker in weak_markers)


def rag_display_text(chunk: Dict[str, Any]) -> str:
    text = rag_index_text(chunk).strip()
    if (
        is_synthetic_qa_chunk(chunk)
        and env_bool("DURIAN_RAG_USE_SOURCE_EVIDENCE_FOR_QA", True)
    ):
        support = _rag_support_text(chunk)
        if support and not is_weak_synthetic_qa_chunk(chunk):
            question = str(chunk.get("question") or "").strip()
            if question:
...
def _normalize_rerank_score(raw_score: float) -> float:
    ...
    if 0.0 <= value <= 1.0:
        return value
    if value >= 0:
        return float(1.0 / (1.0 + np.exp(-min(value, 60.0))))
    exp_value = np.exp(max(value, -60.0))
    return float(exp_value / (1.0 + exp_value))


def get_reranker_model():
    global _RERANK_MODEL, _RERANK_LOAD_FAILED_AT, _RERANK_LOAD_ERROR
    if not RERANK_CONFIG.get("enabled") or not RERANK_CONFIG.get("model"):
        return None

    now = time.time()
    if (
        _RERANK_MODEL is None
        and _RERANK_LOAD_FAILED_AT
        and now - _RERANK_LOAD_FAILED_AT < float(RERANK_CONFIG.get("failure_cooldown_sec", 300))
    ):
        return None
    ...
    with RERANK_LOCK:
        ...
        try:
            ...
            kwargs = {}
            device = str(RERANK_CONFIG.get("device") or "").strip()
            if device and device.lower() not in {"auto", "default"}:
                kwargs["device"] = device
            model_name = str(RERANK_CONFIG.get("model") or "").strip()
            logger.info("Loading RAG reranker model: model=%s device=%s", model_name, device or "default")
            _RERANK_MODEL = CrossEncoder(model_name, **kwargs)
            _RERANK_LOAD_ERROR = ""
            logger.info("RAG reranker loaded: model=%s", model_name)
            return _RERANK_MODEL
        except Exception as exc:
            _RERANK_LOAD_FAILED_AT = time.time()
            _RERANK_LOAD_ERROR = str(exc)
            logger.warning(
                "RAG reranker unavailable; retrieval will use original ranking: model=%s error=%s",
...
def _filter_evidence_by_query_topic(query: str, evidence: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    if not evidence:
        return evidence
    q = str(query or "").lower()
    disease_only_leaf_query = (
        any(term in q for term in ["炭疽", "叶斑", "anthracnose", "leaf spot"])
        and not any(term in q for term in [
            "红蜘蛛", "蓟马", "粉蚧", "蛀果虫", "果蝇", "木虱", "虫",
            "spider mite", "thrips", "mealybug", "fruit borer", "pest",
        ])
    )
    if not disease_only_leaf_query:
        return evidence

    unrelated_pest_terms = [
        "红蜘蛛", "蓟马", "粉蚧", "蛀果虫", "果蝇", "木虱", "虫子", "虫害",
        "spider mite", "thrips", "mealybug", "fruit borer", "pest",
    ]
    filtered = []
    dropped = 0
    for item in evidence:
        text = re.sub(r"\s+", " ", rag_display_text(item)).lower().strip()
        leading = text[:260]
        if any(term in leading for term in unrelated_pest_terms):
            dropped += 1
            continue
        filtered.append(item)
    if dropped:
        logger.info(
            "RAG topic filter dropped mixed pest evidence: query=%s before=%d after=%d dropped=%d",
            str(query or "")[:120],
            len(evidence),
            len(filtered),
            dropped,
...
def rerank_evidence(query: str, evidence: List[Dict[str, Any]], top_k: Optional[int] = None) -> List[Dict[str, Any]]:
    if not evidence or len(evidence) <= 1:
        return evidence
    normalized_query = re.sub(r"\s+", " ", str(query or "")).strip()
    if not normalized_query or not RERANK_CONFIG.get("enabled"):
        return evidence
    max_query_chars = max(40, int(RERANK_CONFIG.get("max_query_chars", 220)))
    if len(normalized_query) > max_query_chars:
        normalized_query = normalized_query[:max_query_chars]

    candidate_limit = max(1, int(RERANK_CONFIG.get("candidate_limit", 32)))
    candidates = list(evidence[:candidate_limit])
    tail = list(evidence[candidate_limit:])
    max_chars = max(160, int(RERANK_CONFIG.get("max_passage_chars", 1200)))
    ...
    for item in candidates:
        if not isinstance(item, dict):
            continue
        passage = re.sub(r"\s+", " ", rag_display_text(item)).strip()
        if not passage:
            continue
        clipped = passage[:max_chars]
        pairs.append((normalized_query, clipped))
        usable_candidates.append(item)
        fingerprint_parts.append({
            "doc": str(item.get("doc") or item.get("source") or item.get("url") or "")[:240],
            "text": clipped,
        })
    ...
    cache_key = ""
    cache_ttl = max(0.0, float(RERANK_CONFIG.get("cache_ttl_sec", 0) or 0))
    if cache_ttl > 0:
        raw_key = json.dumps(
            {
                "q": normalized_query,
                "top_k": int(top_k or RERANK_CONFIG.get("top_k") or 0),
                "min": float(RERANK_CONFIG.get("min_score") or 0.0),
                "items": fingerprint_parts,
            },
            ensure_ascii=False,
            sort_keys=True,
        )
        ...
        with RERANK_CACHE_LOCK:
            ...
            if cached and now - cached[0] <= cache_ttl:
                logger.info("RAG rerank cache hit: query=%s candidates=%d", normalized_query[:120], len(pairs))
    ...
    try:
        scores = reranker.predict(
            pairs,
            batch_size=max(1, int(RERANK_CONFIG.get("batch_size", 4))),
            show_progress_bar=False,
        )
        flat_scores = np.asarray(scores, dtype=float).reshape(-1).tolist()
    except Exception as exc:
        logger.warning("RAG reranker predict failed; using original ranking: error=%s", exc)
        return evidence

    scored = []
    for item, raw_score in zip(usable_candidates, flat_scores):
        new_item = item.copy()
        try:
            new_item["pre_rerank_score"] = float(item.get("score", 0.0) or 0.0)
        except Exception:
            new_item["pre_rerank_score"] = 0.0
        new_item["rerank_score"] = round(float(raw_score), 6)
        new_item["rerank_score_norm"] = round(_normalize_rerank_score(float(raw_score)), 6)
        scored.append(new_item)

    scored.sort(key=lambda item: float(item.get("rerank_score", 0.0)), reverse=True)
    min_score = float(RERANK_CONFIG.get("min_score") or 0.0)
    if min_score > 0:
        filtered = [
            item
            for item in scored
            if float(item.get("rerank_score", 0.0) or 0.0) >= min_score
        ]
        if not filtered and scored:
            # Cross-encoder scores are model/calibration dependent. An empty
            # threshold result must not erase candidates that already passed
            # lexical/anchor retrieval and force a second web retrieval pass.
            fallback_count = max(
                1,
                min(
                    len(scored),
                    int(os.getenv("DURIAN_RERANK_EMPTY_FALLBACK_TOP_K", "2")),
                ),
            )
            filtered = scored[:fallback_count]
            for item in filtered:
                item["rerank_threshold_fallback"] = True
            logger.info(
                "RAG rerank threshold fallback: kept=%d top=%.4f min=%.4f query=%s",
                len(filtered),
                float(scored[0].get("rerank_score", 0.0) or 0.0),
                min_score,
                normalized_query[:120],
            )
    else:
        filtered = scored
    rerank_top_k = int(top_k or RERANK_CONFIG.get("top_k") or 0)
    if rerank_top_k > 0:
        reranked = filtered[:rerank_top_k]
    else:
        reranked = filtered

    top_score = float(scored[0].get("rerank_score", 0.0)) if scored else 0.0
    logger.info(
        "RAG reranked evidence: query=%s before=%d scored=%d kept=%d returned=%d top=%.4f min=%.4f",
        normalized_query[:120],
        len(evidence),
        len(scored),
        len(filtered),
    ...
    result = reranked if min_score > 0 else reranked + tail
    if cache_key and cache_ttl > 0:
        with RERANK_CACHE_LOCK:
            _RERANK_CACHE[cache_key] = (time.time(), [dict(item) for item in result])
            max_entries = max(16, int(RERANK_CONFIG.get("cache_max_entries", 512) or 512))
            if len(_RERANK_CACHE) > max_entries:
                stale = sorted(_RERANK_CACHE.items(), key=lambda kv: kv[1][0])[: len(_RERANK_CACHE) - max_entries]
                for old_key, _old_value in stale:
                    _RERANK_CACHE.pop(old_key, None)
    return result

def is_rag_noise_chunk(chunk: Dict[str, Any]) -> bool:
    """Reject parser metadata and citation-only fragments before indexing/display."""
    text = rag_index_text(chunk).strip()
    if not text:
        return True
    if is_weak_synthetic_qa_chunk(chunk):
        return True

    lower = text.lower()
    compact = re.sub(r"\s+", " ", lower)

    # Keep high-value OCR disease sections from MinerU.
    # These chunks contain parser markers such as bbox/spans/lines, but they also contain
    # real agronomic content, so they must not be dropped as pure metadata noise.
    source_blob = " ".join([
        str(chunk.get("source_file") or ""),
        str(chunk.get("doc") or ""),
    ...
    if (
        str(chunk.get("source_file") or "").lower() == "ocr_texts.json"
        and any(term in compact for term in disease_ocr_terms)
    ):
        return False
    metadata_markers = [
        "'bbox'", '"bbox"', "bbox:",
        "'spans'", '"spans"', "'lines'", '"lines"',
        "'para_blocks'", '"para_blocks"', "'discarded_blocks'", '"discarded_blocks"',
        "'page_idx'", '"page_idx"', "'page_size'", '"page_size"',
        "'angle'", '"angle"', "'image_path'", '"image_path"',
        "'inline_equation'", '"inline_equation"', "'ref_text'", '"ref_text"',
    ]
    marker_hits = sum(1 for marker in metadata_markers if marker in compact)

    if "区域内的文本片段" in text or "类型为 ref_text" in text or "边界框为" in text:
        return True
    if re.match(r"^\s*(?:\[\s*)?\{['\"]bbox['\"]", text):
        return True
    if "bbox" in compact and "spans" in compact and "lines" in compact:
        return True
    if marker_hits >= 4:
        return True
    if marker_hits >= 2 and re.search(r"\[\s*\d{1,4}\s*,\s*\d{1,4}\s*,\s*\d{1,4}\s*,\s*\d{1,4}\s*\]", text):
        return True

    has_answer_shape = any(
        token in text
        for token in [
            "【问题】", "【答案】", "【结论摘要】", "【操作步骤】", "【风险提示】",
            "【Summary】", "【Steps】", "【Risk Warning】",
            "【สรุป】", "【ขั้นตอน", "【Ringkasan", "【Langkah",
        ]
    )
    if not has_answer_shape and len(text) < 520:
        source_blob = " ".join(
            str(chunk.get(key) or "")
            for key in ("doc", "source", "filename", "source_file")
        ).lower()
        citation_like = (
            bool(re.search(r"\b(?:19|20)\d{2}\b", text))
            and bool(re.search(r"\b[A-Z][A-Za-z]+,\s*[A-Z]\.", text))
            and any(term in compact for term in ["durian", "disease", "growing", "host of plant diseases", "bibliographic"])
        )
        if citation_like or ("bibliographic review" in source_blob and "参考依据" not in text):
            return True
...
# Optional LlamaIndex RAG backend.
DURIAN_RAG_BACKEND = os.getenv("DURIAN_RAG_BACKEND", "faiss").strip().lower()

try:
    from rag_llamaindex import retrieve_for_backend as llamaindex_retrieve_for_backend
except Exception as _llama_rag_import_error:
    llamaindex_retrieve_for_backend = None
    logger.warning("LlamaIndex RAG backend unavailable: %s", _llama_rag_import_error)


def use_llamaindex_rag_backend() -> bool:
    return DURIAN_RAG_BACKEND in {"llamaindex", "llama", "li"}


def retrieve_with_selected_rag_backend(query: str, limit: int = 5):
    """Switchable RAG retrieval entry.

    faiss: use existing backend flow
    llamaindex: use rag_llamaindex.retrieve_for_backend
    """
    if use_llamaindex_rag_backend():
        if llamaindex_retrieve_for_backend is None:
            logger.warning("DURIAN_RAG_BACKEND=llamaindex but backend import failed; returning no evidence")
            return []
        try:
            return llamaindex_retrieve_for_backend(query, top_k=limit)
        except Exception as exc:
            logger.exception("LlamaIndex retrieval failed: %s", exc)
            return []
...
class RAGRetriever:
    ...
    def _load_chunks(self):
        try:
            ...
            for path in self._configured_chunk_paths():
                ...
                loaded = 0
                skipped_path_noise = 0
                with open(path, "r", encoding="utf-8") as f:
                    for line in f:
                        if not line.strip():
                            continue
                        chunk = sanitize_rag_chunk_before_index(json.loads(line))
                        if is_pdf_front_matter_or_parser_noise(chunk):
                            skipped_noise += 1
                            skipped_path_noise += 1
                            continue
                        if is_rag_noise_chunk(chunk):
                            skipped_noise += 1
                            skipped_path_noise += 1
                            continue
                        index_text = rag_index_text(chunk)
                        if not index_text:
                            skipped_noise += 1
                            skipped_path_noise += 1
                            continue
                        chunk["index_text"] = index_text
                        chunk["canonical_language"] = _rag_canonical_language()
            ...
            self.chunks = chunks
            if skipped_noise:
                logger.info("RAG total noisy chunks skipped before indexing: %d", skipped_noise)
    ...
    def _load_embed_model(self):
        try:
            ...
            embed_device = os.getenv("DURIAN_RAG_EMBED_DEVICE", "cpu").strip().lower() or "cpu"
    ...
    def _encode(self, texts: List[str], batch_size: int = 64) -> np.ndarray:
        ...
        embeddings = self.model.encode(
            ...
            show_progress_bar=False,
            device=os.getenv("DURIAN_RAG_EMBED_DEVICE", "cpu").strip().lower() or "cpu",
        )
        return embeddings.astype(np.float32)

    def _load_or_build_index(self):
        """优先加载缓存索引，如果不存在则构建新索引"""
        if self.config.get("force_rebuild_index"):
            logger.info("强制重建 RAG FAISS 索引，跳过缓存")
            self._build_index()
            self._save_cache()
            return
    ...
    @staticmethod
    def _chunks_signature(chunks: List[Dict]) -> str:
        h = hashlib.sha1()
        h.update(f"canonical_language={_rag_canonical_language()}\n".encode("utf-8", errors="ignore"))
        for chunk in chunks or []:
            chunk_id = str(chunk.get("id") or chunk.get("text_sha1") or "")
            text = rag_index_text(chunk)
            text_hash = hashlib.sha1(text.encode("utf-8", errors="ignore")).hexdigest() if text else ""
            source = str(chunk.get("doc") or chunk.get("source") or chunk.get("source_file") or "")
            h.update(f"{chunk_id}\t{text_hash}\t{source}\n".encode("utf-8", errors="ignore"))
        return h.hexdigest()
    ...
    def _try_load_cache(self) -> bool:
        ...
        try:
            ...
            cached_chunks = cached_payload.get("chunks") if isinstance(cached_payload, dict) else cached_payload
            if not isinstance(cached_chunks, list):
                logger.warning("缓存数据块格式无效，重新构建索引")
                return False

            cached_noise_count = sum(1 for chunk in cached_chunks if is_rag_noise_chunk(chunk))
            if cached_noise_count:
                logger.warning("RAG cache contains noisy chunks (%d); rebuilding index", cached_noise_count)
                return False
            ...
            cached_signature = self._chunks_signature(cached_chunks)
            current_signature = self._chunks_signature(self.chunks)
            if cached_signature != current_signature:
                logger.warning("缓存数据块内容签名不匹配，重新构建索引")
                return False
            
            # 加载索引
            self.index = faiss.read_index(self.index_cache_path)
            if int(getattr(self.index, "ntotal", 0) or 0) != len(self.chunks):
                logger.warning("FAISS 索引记录数不匹配 (%d vs %d)", int(getattr(self.index, "ntotal", 0) or 0), len(self.chunks))
                self.index = None
                return False
    ...
    def _build_index(self):
        ...
        try:
            ...
            texts = [rag_index_text(c) for c in self.chunks]
    ...
    def retrieve(self, query: str, top_k: Optional[int] = None) -> Tuple[List[Dict], str]:
        if is_broad_short_keyword_query(query):
            logger.info(
                "RAG dense retrieve skipped for broad short keyword query: %s",
                str(query)[:100].replace("\n", " "),
            )
            return [], "none"
        ...
        try:
            ...
            search_multiplier = max(1, int(self.config.get("search_multiplier", 2) or 2))
            search_k = max(top_k + 2, top_k * search_multiplier)
            scores, ids = self.index.search(qv, search_k)
            ...
            for score, idx in zip(scores[0].tolist(), ids[0].tolist()):
                ...
                if not index_text:
                    continue
                original_text = str(chunk.get("original_text") or _rag_chunk_text(chunk) or "").strip()
                if original_text and original_text != index_text:
                    chunk["original_text"] = original_text
                chunk["index_text"] = index_text
                chunk["text"] = index_text
                chunk["canonical_language"] = _rag_canonical_language()
...
class RAGFlowRetriever:
    """RAGFlow hybrid retriever with a safe local fallback at the adapter layer."""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.enabled = bool(
            ...
            and self.config.get("dataset_ids")
        )
    ...
    def retrieve(
        ...
        *,
        ...
        try:
            ...
            requested_top_k = max(1, int(top_k or self.config.get("top_k", 12)))
            fetch_top_k = max(
                requested_top_k,
                int(candidate_top_k or requested_top_k),
            )
            vector_weight = (
                0.18
                if exact_mode
                else float(self.config.get("vector_similarity_weight", 0.35))
            )
            threshold = (
                0.05
                if exact_mode
                else float(self.config.get("similarity_threshold", 0.18))
            )
            response = httpx.post(
                f"{self.config['base_url']}/retrieval",
                headers={
                ...
                json={
                    ...
                    "dataset_ids": list(self.config.get("dataset_ids") or []),
                    "page": 1,
                    "page_size": max(fetch_top_k * 2, 20),
                    "top_k": max(fetch_top_k * 8, 64),
                ...
                timeout=float(
                    ...
                    else self.config.get("timeout_sec", 8.0)
                ),
                trust_env=False,
            )
            ...
            raw_chunks = (payload.get("data") or {}).get("chunks") or []
            ...
            for chunk in raw_chunks:
                ...
                evidence.append(
                    {
                        "id": str(chunk.get("id") or f"ragflow-{key[:16]}"),
                        "text": text,
                        "display_text": text,
                        "index_text": text,
                        "original_text": text,
                        ...
                        "score": score,
                        "dense_score": float(
                            chunk.get("vector_similarity") or score
                        ),
                        "lexical_score": float(
                            chunk.get("term_similarity") or 0.0
                        ),
                        "metadata": metadata,
                        "rag_backend": "ragflow",
                        "retrieval_backend": "ragflow_hybrid",
                    }
                )
                if len(evidence) >= fetch_top_k:
                    break

            if not evidence:
                return [], "none"
            top_score = max(float(item.get("score", 0.0)) for item in evidence)
            quality = (
                "high"
                if top_score >= 0.65
                else "medium"
            ...
            logger.info(
                "RAGFlow hybrid retrieve: query=%s evidence=%d fetch_top_k=%d quality=%s top_score=%.4f vector_weight=%.2f",
                str(query)[:120].replace("\n", " "),
                len(evidence),
                fetch_top_k,
                quality,
                top_score,
            ...
            logger.warning(
                "RAGFlow retrieval failed; grounded answer will be withheld: %s",
                exc,
            )
            return [], "none"
...
class DurianGPTModel:
    ...
    def load_model(self):
        ...
        if self.engine_mode in OPENAI_PROXY_MODES:
            ...
            models_url = f"{DURIAN_OPENAI_BASE_URL.rstrip('/')}/models"
            wait_seconds = max(
                1.0,
                float(os.getenv("DURIAN_PROXY_STARTUP_WAIT_SEC", "150")),
            )
            ...
            while time.monotonic() < deadline:
                try:
                    ...
                    if DURIAN_OPENAI_MODEL in model_ids:
                        break
                    last_error = (
                        f"model {DURIAN_OPENAI_MODEL!r} not listed; "
                ...
                raise RuntimeError(
                    "vLLM OpenAI Server 未就绪: "
                    f"url={models_url} error={last_error}"
                )
    ...
    def _proxy_tokenize_messages(
        self,
        messages: List[Dict[str, Any]],
    ) -> Tuple[int, int]:
        """Ask the already-loaded vLLM tokenizer for the exact chat token count."""
        httpx = import_httpx_for_proxy()
        base_url = DURIAN_OPENAI_BASE_URL.rstrip("/")
        ...
        return (
            ...
            int(data.get("max_model_len") or self.max_model_len),
        )

    @staticmethod
    def _shrink_system_message(content: str, ratio: float = 0.78) -> str:
        text = str(content or "")
        if len(text) <= 1400:
            return text
        target = max(1200, int(len(text) * ratio))
        head_size = int(target * 0.62)
        tail_size = max(0, target - head_size)
        return (
            text[:head_size]
            + "\n\n[较早的低优先级上下文已自动压缩]\n\n"
    ...
    def _fit_proxy_budget(
        self,
        messages: List[Dict[str, Any]],
        requested_output_tokens: int,
    ) -> Tuple[List[Dict[str, Any]], int]:
        """Fit input and output into vLLM's exact context window."""
        fitted = [dict(message) for message in self._trim_messages(messages)]
        requested = max(1, int(requested_output_tokens))
        safety_margin = max(
            16,
            int(os.getenv("DURIAN_PROXY_TOKEN_SAFETY_MARGIN", "64")),
        )
        minimum_output = min(
            requested,
            max(
                64,
                int(os.getenv("DURIAN_PROXY_MIN_OUTPUT_TOKENS", "256")),
            ),
        )
        preferred_output = min(
            requested,
            max(
                minimum_output,
                int(os.getenv("DURIAN_PROXY_PREFERRED_OUTPUT_TOKENS", "768")),
            ),
        )

        try:
            input_tokens, max_model_len = self._proxy_tokenize_messages(fitted)
        except Exception as exc:
            # The chat request would also be unavailable if the local tokenizer
            # endpoint is down. Keep a conservative fallback for compatibility.
            logger.warning(
                "vLLM 精确 tokenize 失败，使用保守估算: %s",
                exc,
            )
            rendered = "\n".join(
        ...
        # Preserve the current user message and the immediately preceding
        # context. Remove older raw history first; rolling/topic memory remains
        # available in the system instruction.
        while available < preferred_output:
            non_system_indices = [
                index
            ...
            if len(non_system_indices) <= 3:
                break
            del fitted[non_system_indices[0]]
            input_tokens, max_model_len = self._proxy_tokenize_messages(fitted)
            available = max_model_len - input_tokens - safety_margin

        # If system instructions plus the latest exchange are still too long,
        # compact only the low-priority middle of the system prompt.
        shrink_rounds = 0
        while available < preferred_output and shrink_rounds < 8:
            system_index = next(
                (
                    index
                    for index, message in enumerate(fitted)
                    if message.get("role") == "system"
                    and len(str(message.get("content") or "")) > 1400
                ),
                None,
            )
            if system_index is None:
                break
            old_content = str(fitted[system_index].get("content") or "")
            new_content = self._shrink_system_message(old_content)
            if len(new_content) >= len(old_content):
                break
            fitted[system_index]["content"] = new_content
            shrink_rounds += 1
            input_tokens, max_model_len = self._proxy_tokenize_messages(fitted)
            available = max_model_len - input_tokens - safety_margin
        ...
        if available < minimum_output:
            logger.warning(
                "OpenAI proxy output budget below minimum after trimming: input=%d available=%d minimum=%d preferred=%d max=%d messages=%d",
                input_tokens,
                available,
                minimum_output,
                preferred_output,
        ...
        if available <= 0:
            raise RuntimeError(
                "当前问题与必要系统指令已超过模型上下文上限，"
                "请缩短本轮输入后重试。"
            )

        final_output = max(1, min(requested, available))
        if (
            final_output != requested
            or len(fitted) != len(messages)
            or shrink_rounds
        ):
            logger.info(
                "OpenAI proxy token budget adjusted: input=%d output=%d->%d preferred=%d max=%d messages=%d->%d system_shrinks=%d",
                input_tokens,
                requested,
                final_output,
                preferred_output,
        ...
        return fitted, final_output

    @staticmethod
    def _retry_output_tokens_from_400(
        response_text: str,
        current_output_tokens: int,
    ) -> Optional[int]:
        text = str(response_text or "")
        max_match = re.search(r"maximum context length is\s+(\d+)", text)
        ...
        if not max_match or not input_match:
            return None
        available = int(max_match.group(1)) - int(input_match.group(1)) - 32
        if available < 1 or available >= int(current_output_tokens):
            return None
        return max(1, available)

    def _build_proxy_payload(self, messages: List[Dict], stream: bool, **kwargs) -> Dict[str, Any]:
        requested_max_tokens = self._normalize_proxy_max_tokens(
            kwargs.get("max_new_tokens", kwargs.get("max_tokens"))
        )
        fitted_messages, max_tokens = self._fit_proxy_budget(
            messages,
            ...
            with httpx.Client(timeout=DURIAN_OPENAI_TIMEOUT) as client:
                ...
                for attempt in range(2):
                    ...
                    if resp.status_code == 400:
                        body = resp.text
                        retry_tokens = self._retry_output_tokens_from_400(
                            body,
                            int(payload.get("max_tokens") or 1),
                        )
                        if attempt == 0 and retry_tokens is not None:
                            logger.warning(
                                "vLLM 非流式上下文预算二次校正: max_tokens=%s->%s",
                ...
                for attempt in range(2):
                    ...
                    with client.stream(
                        ...
                        if resp.status_code == 400:
                            ...
                            if attempt == 0 and retry_tokens is not None:
                                logger.warning(
                                    "vLLM 流式上下文预算二次校正: max_tokens=%s->%s",
                                    payload.get("max_tokens"),
                                    retry_tokens,
                                )
                                payload["max_tokens"] = retry_tokens
    ...
    @staticmethod
    def _compact_message_content(content: Any, max_chars: int) -> str:
        text = str(content or "")
        if len(text) <= max_chars:
            return text
        head = max(1, int(max_chars * 0.72))
        tail = max(0, max_chars - head - 26)
        return text[:head].rstrip() + "\n[历史内容已压缩]\n" + (text[-tail:].lstrip() if tail else "")

    def _compact_messages_for_model(self, messages: List[Dict]) -> List[Dict]:
        if not messages:
            return []
        ...
        compacted = []
        changed = 0
        system_cap = int(os.getenv("DURIAN_MODEL_SYSTEM_MAX_CHARS", "10000"))
        assistant_cap = int(os.getenv("DURIAN_MODEL_ASSISTANT_HISTORY_MAX_CHARS", "1200"))
        user_history_cap = int(os.getenv("DURIAN_MODEL_USER_HISTORY_MAX_CHARS", "800"))
        current_user_cap = int(os.getenv("DURIAN_MODEL_CURRENT_USER_MAX_CHARS", "3000"))
        for index, message in enumerate(messages):
            ...
            if role == "system":
                ...
                cap = current_user_cap
            else:
                cap = user_history_cap
            old_content = str(item.get("content") or "")
            new_content = self._compact_message_content(old_content, max(200, cap))
            if len(new_content) != len(old_content):
                changed += 1
        ...
        if changed:
            logger.info(
                "模型输入消息内容已压缩: changed=%d messages=%d",
                changed,
                len(messages),
            )
        return compacted
    ...
    def _trim_messages(self, messages: List[Dict]) -> List[Dict]:
        ...
        compacted = self._compact_messages_for_model(messages)
        if len(compacted) <= max_history:
            return compacted
...
def clean_model_output(text: str) -> str:
    ...
    # 清理多余空行和引用链接尾部的中英文标点，避免前端把标点并入 URL。
    text = re.sub(r"\n{3,}", "\n\n", text)
    text = normalize_reference_urls_in_text(text)
    return text.strip()
...
def detect_response_language(text: str, fallback: str = "zh") -> str:
    ...
    # Bare identifiers such as D229, HTTP 429, CVE-2026-1234 and GPT-5.6
    # carry no answer-language signal. Keep the caller's language instead of
    # turning an identifier-only Chinese request into an English response.
    identifier_only = bool(
        re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._/-]*(?:\s+[A-Za-z0-9][A-Za-z0-9._/-]*)*", value)
        and (
            bool(re.search(r"\d", value))
            or bool(re.search(r"[._/-]", value))
            or any(token.isupper() and len(token) >= 2 for token in re.findall(r"[A-Za-z]+", value))
        )
    )
    if identifier_only:
        return fallback if fallback in {"zh", "en", "ms", "th"} else "zh"
...
def classify_casual_message(text: str) -> Optional[str]:
    """Classify short conversation-control messages that must not reach the domain model."""
    compact = normalize_short_text(text)
    if not compact or len(compact) > 24:
        return None

    acknowledgements = {
        "谢谢", "感谢", "谢了", "好的", "好吧", "收到", "明白了", "知道了",
        "嗯", "哦", "噢", "哈哈", "呵呵",
        "thanks", "thankyou", "ok", "okay", "gotit",
        "terimakasih", "baik", "faham",
    ...
    abuse_terms = (
        "哈批", "傻逼", "煞笔", "傻比", "笨蛋", "蠢货", "白痴", "垃圾",
        "神经病", "有病", "废物", "滚", "闭嘴",
        "fuckyou", "idiot", "stupid", "shutup",
    )
    abuse_prefixes = ("你", "你是", "你真", "你个", "模型", "ai")
    if compact in abuse_terms or any(
        compact == f"{prefix}{term}"
...
def casual_reply(text: str, response_language: str) -> str:
    """Return a deterministic conversational reply without inventing domain facts."""
    kind = classify_casual_message(text)
    lang = (response_language or "zh").lower()
    if kind == "negative_feedback":
        replies = {
            "zh": "如果刚才的回答有问题，请直接指出哪一部分不对，或告诉我你希望我重新回答什么。我会按你的要求修正。",
            "en": "If the previous answer was wrong, tell me which part or what you want answered again, and I will correct it.",
            "ms": "Jika jawapan tadi salah, nyatakan bahagian yang bermasalah atau soalan yang perlu dijawab semula. Saya akan membetulkannya.",
            "th": "หากคำตอบก่อนหน้านี้ไม่ถูกต้อง โปรดบอกส่วนที่ผิดหรือสิ่งที่ต้องการให้ตอบใหม่ แล้วฉันจะแก้ไขให้",
        }
    else:
        replies = {
            "zh": "好的。如果你还有榴莲相关问题，可以继续问我。",
            "en": "You're welcome. You can continue with any durian-related question.",
            "ms": "Baik. Anda boleh terus bertanya tentang durian.",
            "th": "ได้เลย คุณสามารถถามเรื่องทุเรียนต่อได้",
        }
    return replies.get(lang, replies["zh"])
...
def has_recent_image_context(messages: List[Message], lookback: int = 12) -> bool:
    ...
    for msg in reversed(messages[-lookback:] if messages else []):
        if getattr(msg, "image_url", None):
            return True
        active_card = getattr(msg, "active_context_card", None)
        if isinstance(active_card, dict) and active_card:
            card_source = str(active_card.get("source") or "").lower()
            if active_card.get("image_url") or card_source.startswith("image"):
                return True
...
def classify_user_intent(text: str, has_recent_image: bool = False) -> str:
    ...
    # A fresh explicit identifier should override stale active context.
    # Examples: D163, HTTP 429, CVE-2026-1234.
    if re.search(r"(?<![A-Za-z0-9])(?:[A-Za-z]{1,8}[-_ ]?)?\d{2,8}(?![A-Za-z0-9])", q_raw):
        return "new_topic"
    ...
    explicit_follow_up_keywords = [
        ...
        "这个品种", "这种品种", "该品种", "这个情况", "这种情况", "这个病", "这片叶", "这棵树", "这个果", "继续刚才",
    ...
    short_follow_up_keywords = [
        ...
        "防治", "防治建议", "处理建议", "治疗方案", "防治方案", "用药", "用药建议", "管理建议",
        "具体怎么防治", "详细怎么处理", "详细处理", "详细建议",
        "什么病", "什么疾病", "是什么病", "是什么疾病", "哪种病", "哪类病", "病害", "病症",
        "serious", "spray", "treat", "treatment", "manage", "control", "save", "next",
        "serius", "sembur", "rawat", "kawal",
        "รุนแรง", "ฉีด", "รักษา",
    ]
    explicit_new_subject_keywords = [
        "炭疽", "叶斑", "蒂腐", "茎端腐", "根腐", "白根", "赤衣", "流胶", "疫病",
        "粉蚧", "蓟马", "蛀果虫", "果蝇", "红蜘蛛", "木虱",
        "果腐", "软腐", "霉变", "腐烂", "变质",
        "anthracnose", "leaf spot", "stem end rot", "root rot", "pink disease",
        "mealybug", "thrips", "fruit borer",
    ]
    has_explicit_new_subject = any(k in q for k in explicit_new_subject_keywords)
    if (
        has_recent_image
        and len(normalize_short_text(q_raw)) <= 24
        and any(k in q for k in short_follow_up_keywords)
        and not has_explicit_new_subject
    ):
        return "explicit_follow_up"

    comparison_follow_up = bool(
        has_recent_image
        and any(k in q for k in ["区别", "差别", "不同", "对比", "比较", "difference", "compare"])
        and any(k in q for k in ["和", "跟", "与", "同", "vs", "versus", "compared with"])
        and len(normalize_short_text(q_raw)) <= 36
    )
...
def extract_cultivar_subject_heuristic(text: str) -> str:
    """Extract a stable cultivar entity from exact code/name answers."""
    plain = strip_html_comments(unicodedata.normalize("NFKC", str(text or "")))
    if not plain:
        return ""
    code_pat = r"(?<![A-Za-z0-9])D\s*-?\s*(\d{1,4})(?!\d)"
    patterns = [
        rf"{code_pat}\s*(?:编号|品种编号|登记编号|registration\s+number|code)?\s*(?:对应的是|对应为|对应|是|=|:|：|corresponds\s+to|corresponds\s+with|is)\s*([^。\n；;.!?()（）|]{{1,90}})",
        rf"{code_pat}\s+([A-Z][A-Za-z0-9_-]*(?:\s+[A-Z][A-Za-z0-9_-]*){{0,4}})\s+(?:Durian|榴莲)\b",
        rf"{code_pat}\s+([A-Z][A-Za-z0-9_-]*(?:\s+[A-Z][A-Za-z0-9_-]*){{0,4}})(?:\s*[:：]|\s*\||\s*-)",
    ]
    for pattern in patterns:
        ...
        raw_alias = re.sub(r"[*`_#]+", " ", raw_alias)
        raw_alias = re.sub(r"(?i)\bdurian\b", " ", raw_alias).replace("榴莲", " ")
        raw_alias = re.split(
            r"(?:也常见写作|也被称为|也称为|又称为|又称|又叫|又名|俗称|别称|别名|also\s+known\s+as|known\s+as|aka|called|[,，/／])",
            raw_alias,
            maxsplit=1,
            flags=re.I,
        )[0]
        raw_alias = re.split(
            r"\s*[:：]\s*(?:taste|origin|facts?|guide|price|review|info|information|介绍|特点|风味|口感|来源|资料)\b",
            raw_alias,
            maxsplit=1,
            flags=re.I,
        )[0]
        ...
        if not raw_alias:
            continue
        if re.search(r"(?:编号|对应|品种是什么|what|which|source|reference|来源|资料)", raw_alias, flags=re.I):
            continue
        if len(raw_alias) > 40:
            continue
        return f"{code} {raw_alias}"
    return ""
...
def extract_main_subject_heuristic(text: str, fallback: str = "上一轮话题") -> str:
    ...
    if not text:
        return fallback
    cultivar_subject = extract_cultivar_subject_heuristic(text)
    if cultivar_subject:
        return cultivar_subject
    patterns = [
...
def normalize_context_card(raw: Dict[str, Any], fallback_id: str, recency_index: int = 0) -> Dict[str, Any]:
    ...
    raw_source = str(
        ...
        or (
            ...
            or raw.get("type") == "durian_image_context"
            else "text"
        )
    )
    normalized_source = (
        "image"
        if raw.get("image_url") or raw_source.lower().startswith("image")
        else raw_source
    )
    card = {
...
def context_card_anchor_terms_for_rag(card: Dict[str, Any]) -> str:
    haystack = " ".join(
        str(card.get(key) or "")
    ...
    anchor_rules = [
        ...
        (("蛀果虫", "fruit borer", "seed borer", "mudaria"), "榴莲蛀果虫 Fruit Borer 防治"),
        (("变质", "果肉", "腐烂", "霉变", "果腐", "软腐", "spoilage", "fruit rot"), "榴莲果实腐烂 霉变 果肉变质 果腐病 食用安全"),
    ]
    anchors = [label for triggers, label in anchor_rules if any(trigger in haystack for trigger in triggers)]
    return " ".join(anchors[:3])


def build_context_card_standalone_query(card: Dict[str, Any], query: str) -> str:
    subject = str(card.get("main_subject") or "").strip()
    parts = [
        context_card_anchor_terms_for_rag(card),
        subject if subject and subject not in str(query or "") else "",
        str(query or "").strip(),
    ]
    return "；".join(part for part in parts if part) or str(query or "").strip()
...
def llm_topic_transition_router(query: str, active_card: Optional[Dict[str, Any]], response_language: str = "zh") -> Dict[str, Any]:
    ...
    if quick_intent == "explicit_follow_up":
        subject = str(active_card.get("main_subject") or "").strip()
        route = {
            "transition": "continue",
            "confidence": 0.9,
            "referenced_context_id": active_card.get("context_id"),
            "intent": "general",
            "standalone_query": build_context_card_standalone_query(active_card, query),
...
def build_text_context_card(
    ...
    visible_response = strip_html_comments(response)
    cultivar_subject = extract_cultivar_subject_heuristic(visible_response) or extract_cultivar_subject_heuristic(user_query)

    if isinstance(base_card, dict) and base_card and not (
        cultivar_subject
        and cultivar_subject.lower() not in str(base_card.get("main_subject") or "").lower()
    ):
        card = dict(base_card)
        card.setdefault("context_id", f"ctx_{uuid.uuid4().hex[:12]}")
        base_source = str(card.get("source") or "")
        if card.get("image_url") or base_source.lower().startswith("image"):
            # Keep the root visual provenance across multiple follow-ups.
            # Otherwise the first text follow-up changes an image card into a
            # normal text card and the next pronoun-based question loses the
            # image subject.
            card["source"] = "image"
        else:
            card["source"] = source or base_source or "text_followup"
    ...
    scenario = infer_card_scenario(blob)
    category = infer_card_category(blob, scenario)
    subject = cultivar_subject or extract_main_subject_heuristic(blob)
    card = {
...
def compact_evidence_prompt_text(ev: Dict[str, Any], max_chars: int) -> str:
    text = str(
        ev.get("display_text")
        or ev.get("text")
        or ev.get("original_text")
        or ev.get("index_text")
        or ""
    )
    text = re.sub(r"\s+", " ", text).strip()
    if not text:
        text = re.sub(r"\s+", " ", rag_display_text(ev)).strip()
    return text[: max(120, int(max_chars))]


def compact_evidence_prompt_source(ev: Dict[str, Any]) -> str:
    title = re.sub(r"\s+", " ", str(ev.get("title") or "")).strip()
    raw = str(ev.get("url") or ev.get("source") or ev.get("doc") or "").strip()
    domain = ""
    if raw.startswith(("http://", "https://")):
        try:
            ...
            domain = ""
    if not title and raw and " | " in raw:
        title = raw.split(" | ", 1)[0].strip()
    if title and domain:
        return f"{title[:80]} | {domain[:80]}"
    if title:
        return title[:100]
    if domain:
        return domain[:100]
...
def build_evidence_text(evidence: List[Dict], max_chars: int) -> str:
    parts = []
    per_item_chars = min(
        max(160, int(max_chars)),
        int(os.getenv("DURIAN_PROMPT_EVIDENCE_ITEM_MAX_CHARS", "360")),
    )
    max_items = max(1, int(os.getenv("DURIAN_PROMPT_EVIDENCE_TOP_K", str(len(evidence) or 1))))
    total_max_chars = max(600, int(os.getenv("DURIAN_PROMPT_EVIDENCE_TOTAL_MAX_CHARS", "1600")))

    visible_index = 0
    for ev in list(evidence or [])[:max_items]:
        if is_weak_synthetic_qa_chunk(ev):
            continue
        visible_index += 1
        text = compact_evidence_prompt_text(ev, per_item_chars)
        doc = compact_evidence_prompt_source(ev)
        try:
            score = float(ev.get("rerank_score_norm") or ev.get("score", 0.0) or 0.0)
        except Exception:
            score = 0.0

        if doc:
            parts.append(f"[资料{visible_index} | 来源: {doc} | 相关度: {score:.3f}]\n{text}")
        else:
            parts.append(f"[资料{visible_index} | 相关度: {score:.3f}]\n{text}")

    joined = "\n\n".join(parts)
    return joined[:total_max_chars]

def build_image_prompt_evidence_text(evidence: List[Dict], image_profile: str) -> str:
    if not evidence:
        return ""

    if image_profile == "image_agri_medium":
        max_items = int(os.getenv("DURIAN_IMAGE_AGRI_PROMPT_EVIDENCE_TOP_K", "6"))
        max_chars = int(os.getenv("DURIAN_IMAGE_AGRI_PROMPT_EVIDENCE_CHARS", "360"))
    elif image_profile == "image_consumer_natural":
        max_items = int(os.getenv("DURIAN_IMAGE_CONSUMER_PROMPT_EVIDENCE_TOP_K", "4"))
        max_chars = int(os.getenv("DURIAN_IMAGE_CONSUMER_PROMPT_EVIDENCE_CHARS", "260"))
    else:
        max_items = int(os.getenv("DURIAN_IMAGE_GENERAL_PROMPT_EVIDENCE_TOP_K", "4"))
        max_chars = int(os.getenv("DURIAN_IMAGE_GENERAL_PROMPT_EVIDENCE_CHARS", "300"))

    return build_evidence_text(evidence[:max(1, max_items)], max_chars=max(120, max_chars))


def clamp_image_final_tokens(value: int) -> int:
    cap = int(os.getenv("DURIAN_IMAGE_FINAL_MAX_NEW_TOKENS_CAP", "768"))
    floor = int(os.getenv("DURIAN_IMAGE_FINAL_MIN_NEW_TOKENS", "256"))
    return max(floor, min(int(value), cap))


def normalize_stream_text(content: str) -> str:
    return str(content or "").replace("\\n", "\n").replace("\\t", "\t")


def is_missing_image_vl_result(result: Dict[str, Any]) -> bool:
    category = str(result.get("category_type") or "").strip().lower()
    try:
    ...
    combined = " ".join(
        ...
        for key in ("label", "label_zh", "visual_evidence", "cannot_confirm")
    ).lower()
    missing_markers = (
        "未提供图片",
        "缺少图像",
    ...
    return category == "unknown" and confidence <= 0.05 and any(
        marker in combined for marker in missing_markers
    )

def scope_web_query_to_durian(query: str) -> str:
    """Keep external search inside the durian domain without changing user wording."""
    normalized = re.sub(r"\s+", " ", unicodedata.normalize("NFKC", str(query or ""))).strip()
    if not normalized:
        return ""
    lower = normalized.lower()
    durian_markers = (
        "榴莲", "durian", "durio", "ทุเรียน", "musang", "monthong",
        "black thorn", "red prawn", "dury",
    )
    if any(marker in lower for marker in durian_markers):
        return normalized
    if re.search(r"(?<![A-Za-z0-9])D\s*-?\s*\d{1,4}(?!\d)", normalized, flags=re.I):
        return f"{normalized} durian"
    if re.search(r"[\u3400-\u9fff]", normalized):
        return f"{normalized} 榴莲 durian"
    return f"{normalized} durian"


def evidence_is_durian_scoped(item: Dict[str, Any]) -> bool:
    """Reject web snippets that are not actually about durian."""
    source = unicodedata.normalize(
        "NFKC",
        " ".join(
            str(item.get(key) or "")
            for key in ("title", "doc", "source", "url", "text", "original_text", "index_text")
        ),
    ).lower()
    if not source.strip():
        return False
    markers = (
        "榴莲", "durian", "durio", "ทุเรียน",
        "musang", "monthong", "black thorn", "red prawn", "duri hitam",
        "horloh", "hor lor", "dury", "durianed",
    )
...
def tavily_web_search(
    ...
    *,
    ...
    """Fetch grounded web snippets from Tavily without adding a new dependency."""
    del response_language
    normalized_query = re.sub(r"\s+", " ", str(query or "")).strip()
    if not normalized_query or not WEB_SEARCH_CONFIG["enabled"] or not WEB_SEARCH_CONFIG["api_key"]:
        return [], "none", ""
    search_query = scope_web_query_to_durian(normalized_query)

    requested_max_results = max(1, int(max_results or WEB_SEARCH_CONFIG["top_k"]))
    search_depth = os.getenv("DURIAN_WEB_SEARCH_DEPTH", "basic")
    cache_ttl = max(0.0, float(WEB_SEARCH_CONFIG.get("cache_ttl_sec", 0) or 0))
    cache_key = ""
    if cache_ttl > 0:
        raw_key = json.dumps(
            {
                "q": search_query,
                "top_k": requested_max_results,
                "depth": search_depth,
                "content_chars": int(WEB_SEARCH_CONFIG.get("max_content_chars", 1200)),
                "preserve_candidates": bool(preserve_candidates),
            },
            ensure_ascii=False,
            sort_keys=True,
        )
        ...
        with WEB_SEARCH_CACHE_LOCK:
            ...
            if cached and now - cached[0] <= cache_ttl:
                logger.info("Tavily web search cache hit: query=%s", normalized_query[:120])
                return [dict(item) for item in cached[1]], cached[2], cached[3]
            if cached:
    ...
    request = UrlRequest(
        WEB_SEARCH_CONFIG["endpoint"],
        data=json.dumps(payload).encode("utf-8"),
        headers={
            "Content-Type": "application/json",
            "Accept": "application/json",
            "User-Agent": "DurianGPT/1.0",
        },
        method="POST",
    )

    try:
        with urlopen(request, timeout=WEB_SEARCH_CONFIG["timeout_sec"]) as response:
            data = json.loads(response.read().decode("utf-8", errors="ignore"))
    except Exception as exc:
        logger.warning("Tavily web search failed: query=%s error=%s", normalized_query[:120], exc)
        return [], "none", ""
    ...
    for result in data.get("results") or []:
        ...
        url = normalize_reference_url(result.get("url"))
        content = re.sub(r"\s+", " ", str(result.get("content") or "")).strip()
        if not title or not url or not content:
            continue
        content = content[: max(240, WEB_SEARCH_CONFIG["max_content_chars"])]
        try:
        ...
        item = {
            ...
            "title": title,
            "score": score,
            "rag_backend": "web_tavily",
            "evidence_type": "web",
            "web_search_query": search_query,
        }
        if not evidence_is_durian_scoped(item):
            continue
        evidence.append(item)

    if not evidence:
        logger.info("Tavily web search returned no usable evidence: query=%s", normalized_query[:120])
        return [], "none", ""

    if (
        env_bool("DURIAN_RERANK_WEB_ENABLED", bool(RERANK_CONFIG.get("enabled")))
        and not preserve_candidates
    ):
        evidence = rerank_evidence(search_query, evidence, top_k=len(evidence))
    if preserve_candidates:
        evidence_quality = "medium"
    else:
        evidence, evidence_quality = validate_rag_evidence(
            search_query,
            evidence,
            "medium",
            top_k=requested_max_results,
        ...
        if not evidence:
            logger.info("Tavily web search evidence rejected by validation: query=%s", normalized_query[:120])
            return [], "none", ""
    evidence_text = build_evidence_text(
        evidence,
        max_chars=WEB_SEARCH_CONFIG["max_content_chars"],
    )
    if cache_key and cache_ttl > 0:
        with WEB_SEARCH_CACHE_LOCK:
            _WEB_SEARCH_CACHE[cache_key] = (time.time(), [dict(item) for item in evidence], evidence_quality, evidence_text)
            max_entries = max(16, int(WEB_SEARCH_CONFIG.get("cache_max_entries", 256) or 256))
            if len(_WEB_SEARCH_CACHE) > max_entries:
                stale = sorted(_WEB_SEARCH_CACHE.items(), key=lambda kv: kv[1][0])[: len(_WEB_SEARCH_CACHE) - max_entries]
                for old_key, _old_value in stale:
                    _WEB_SEARCH_CACHE.pop(old_key, None)
    logger.info(
        "Tavily web search returned evidence=%d quality=%s query=%s scoped_query=%s",
        len(evidence),
        evidence_quality,
        normalized_query[:120],
        search_query[:160],
    )
    return evidence, evidence_quality, evidence_text



def build_durian_code_web_query(query: str) -> str:
    normalized = unicodedata.normalize("NFKC", str(query or "")).strip()
    code_match = _DURIAN_CODE_RE.search(normalized)
    extras: List[str] = []
    if code_match:
        code = f"D{code_match.group(1)}"
        extras.extend(DURIAN_CULTIVAR_CODE_KNOWN_ALIASES.get(code, [])[:5])
        extras.extend(["durian cultivar", "榴莲 品种"])
    else:
        lower = normalized.lower()
        for alias, code in sorted(DURIAN_CULTIVAR_ALIAS_TO_CODE.items(), key=lambda item: len(item[0]), reverse=True):
            if alias in lower:
                extras.append(code)
                extras.extend(DURIAN_CULTIVAR_CODE_KNOWN_ALIASES.get(code, [])[:5])
                extras.extend(["durian cultivar", "榴莲 品种编号"])
                break
    deduped = []
    seen = set()
    for value in [normalized] + extras:
...
def build_durian_code_web_queries(query: str) -> List[str]:
    """Generate exact-first web queries for short registration identifiers."""
    normalized = unicodedata.normalize("NFKC", str(query or "")).strip()
    code_match = _DURIAN_CODE_RE.search(normalized)
    lookup_subject = extract_identifier_lookup_subject(normalized)
    candidates: List[str] = []
    if code_match:
        code = f"D{code_match.group(1)}"
        candidates.extend(
            (
                f'{code} durian registration',
                f'{code} Malaysia registered durian',
                f'{code} registered name durian',
                f'{code} 榴莲',
                f'{code} durian',
            )
        )
        known_aliases = DURIAN_CULTIVAR_CODE_KNOWN_ALIASES.get(code, [])
        if known_aliases:
            candidates.append(f'"{code}" {known_aliases[0]}')
    elif lookup_subject:
        # Unknown/future cultivar names must not depend on the built-in alias
        # table. Search for a local name/code binding in registry-style pages.
        candidates.extend(
            (
                f'site:durianpenang.com/registered-durian "{lookup_subject}"',
                f'"{lookup_subject}" durian',
                f'"{lookup_subject}" durian D',
                f'"{lookup_subject}" Malaysia durian registration',
                f'"{lookup_subject}" registered cultivar',
                f'"{lookup_subject}" 榴莲 品种编号',
            )
        )
    if normalized:
        candidates.append(normalized)
    candidates.append(build_durian_code_web_query(normalized))
...
def is_probable_durian_code_evidence(
    query: str,
    item: Dict[str, Any],
) -> bool:
    """Keep plausible reference context without requiring a rigid sentence shape.

    Exact extraction is an optimization. Unknown registration codes often appear
    in titles, captions, or short social posts without a formal "also known as"
    phrase, so those references must still reach the model.
    """
    source = unicodedata.normalize(
        "NFKC",
        "\n".join(
            [
                _evidence_source_text(item),
                str(item.get("doc") or ""),
    ...
    if not source:
        return False
    code_match = _DURIAN_CODE_RE.search(str(query or ""))
    if code_match:
        code = f"d{code_match.group(1)}"
        compact_source = re.sub(r"[\s-]+", "", source)
        if code not in compact_source:
            return False
    else:
        query_lower = unicodedata.normalize("NFKC", str(query or "")).lower()
        known_alias_match = any(
            alias in source
            for alias in DURIAN_CULTIVAR_ALIAS_TO_CODE
            if alias in query_lower
        )
        lookup_subject = extract_identifier_lookup_subject(query)
        compact_subject = re.sub(
            r"[\s_/-]+",
            "",
            unicodedata.normalize("NFKC", lookup_subject).lower(),
        )
        compact_source = re.sub(r"[\s_/-]+", "", source)
        if not known_alias_match and not (
            compact_subject and compact_subject in compact_source
        ):
            return False
        # A name-to-code lookup needs an actual D-code in the same candidate;
        # a page that merely mentions the cultivar name is not exact evidence.
        if not _DURIAN_CODE_RE.search(source):
            return False
    domain_markers = (
        "榴莲",
        "榴梿",
    ...
    if not any(marker in source for marker in domain_markers):
        return False

    # Social snippets often mention several cultivar codes in one caption.
    # Keep them only when the requested code has a local structural binding;
    # a bare occurrence of the code is not enough for an exact lookup.
    if any(marker in source for marker in ("facebook.com", "tiktok.com", "instagram.com")):
        if code_match:
            code = f"d{code_match.group(1)}"
            if not re.search(
                rf"{re.escape(code)}\s*(?:[|｜\t,:：=\-—]|对应|corresponds|registered|also|known)",
                source,
                re.I,
            ):
                return False
...
def _compact_registered_name(value: str) -> str:
    """Compare registry names independent of spacing and punctuation."""
    normalized = unicodedata.normalize("NFKC", str(value or "")).lower()
    return "".join(char for char in normalized if char.isalnum())


def retrieve_registered_durian_site_evidence(
    query: str,
) -> Tuple[List[Dict[str, Any]], str, str]:
    """Query the configured Malaysian registered-varieties table directly.

    Number/code questions use the registry page's server-side ``keyword``
    filter before RAGFlow or broad web search. This avoids relying on whether a
    search engine happened to index a particular table row.
    """
    if not is_exact_fact_query(query) or not REGISTERED_DURIAN_SOURCE_URL:
        return [], "none", ""

    normalized_query = unicodedata.normalize("NFKC", str(query or "")).strip()
    code_match = _DURIAN_CODE_RE.search(normalized_query)
    keyword = (
        f"D{code_match.group(1)}"
        if code_match
        else extract_identifier_lookup_subject(normalized_query)
    )
    if not keyword:
        return [], "none", ""

    cache_key = _compact_registered_name(keyword)
    cache_ttl = max(
        0.0,
        float(os.getenv("DURIAN_REGISTERED_VARIETIES_CACHE_TTL_SEC", "3600")),
    )
    if cache_ttl > 0:
        with REGISTERED_DURIAN_CACHE_LOCK:
            cached = _REGISTERED_DURIAN_CACHE.get(cache_key)
            if cached and time.time() - cached[0] <= cache_ttl:
                cached_evidence = [dict(item) for item in cached[1]]
                return (
                    cached_evidence,
                    "high" if cached_evidence else "none",
                    build_evidence_text(
                        cached_evidence,
                        max_chars=WEB_SEARCH_CONFIG["max_content_chars"],
                    ) if cached_evidence else "",
                )

    # The source page performs punctuation-sensitive SQL matching. Restore
    # common catalogue notation ("No. 1") and try both variants, while row
    # validation below remains punctuation-insensitive.
    dotted_keyword = re.sub(
        r"(?i)\b(no)\s+(\d+)\b",
        r"\1. \2",
        keyword,
    )
    keyword_candidates = list(dict.fromkeys([dotted_keyword, keyword]))
    page_html = ""
    matched_keyword = keyword
    last_error: Optional[Exception] = None
    for candidate in keyword_candidates:
        query_url = (
            REGISTERED_DURIAN_SOURCE_URL
            + ("&" if "?" in REGISTERED_DURIAN_SOURCE_URL else "?")
            + urlencode({"keyword": candidate})
        )
        try:
            request = UrlRequest(
                query_url,
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; DurianGPT/1.0)",
                    "Accept": "text/html,application/xhtml+xml",
                },
            )
            with urlopen(request, timeout=REGISTERED_DURIAN_TIMEOUT_SEC) as response:
                candidate_html = response.read().decode("utf-8", errors="replace")
        except Exception as exc:
            last_error = exc
            continue
        page_html = candidate_html
        matched_keyword = candidate
        if re.search(r"<tbody\b[\s\S]*?<td\b", candidate_html, flags=re.I):
            break
    if not page_html:
        logger.warning(
            "Registered durian source request failed: keyword=%s error=%s",
            keyword[:80],
            last_error,
        )
        return [], "none", ""
    ...
    requested_compact = _compact_registered_name(keyword)
    matched_rows: List[List[str]] = []
    for row_html in re.findall(r"<tr\b[^>]*>([\s\S]*?)</tr>", page_html, flags=re.I):
        cells = [
            clean_cell(cell)
            for cell in re.findall(r"<td\b[^>]*>([\s\S]*?)</td>", row_html, flags=re.I)
        ]
        if len(cells) < 2:
            continue
        row_code = re.sub(r"[\s_-]+", "", cells[0].upper())
        if code_match:
            if row_code != f"D{code_match.group(1)}":
                continue
        else:
            name_fields = cells[1:3]
            compact_names = [
                _compact_registered_name(field)
                for field in name_fields
                if field
            ]
            if not any(
                requested_compact == name
                or requested_compact in name.split(",")
                for name in compact_names
            ):
                continue
        matched_rows.append(cells[:8])
    ...
    for index, cells in enumerate(matched_rows[:4]):
        padded = cells + [""] * (len(headers) - len(cells))
        row_text = " | ".join(padded[: len(headers)])
        display_text = (
            "Registered Malaysian Durian Varieties\n"
            + " | ".join(headers)
            + "\n"
            + row_text
        )
        evidence.append(
            {
                "id": f"durianpenang-registry-{cache_key}-{index}",
                "text": display_text,
                ...
                "source": REGISTERED_DURIAN_SOURCE_URL,
                "url": REGISTERED_DURIAN_SOURCE_URL,
                "title": "Registered Malaysian Durian Varieties",
                "score": 1.0,
                "rag_backend": "durianpenang_registry",
                "retrieval_backend": "direct_registry",
                "evidence_type": "registered_variety",
            }
        )

    if cache_ttl > 0:
        with REGISTERED_DURIAN_CACHE_LOCK:
            _REGISTERED_DURIAN_CACHE[cache_key] = (
                time.time(),
                [dict(item) for item in evidence],
            )
    if not evidence:
        logger.info(
            "Registered durian source found no row: keyword=%s variants=%s",
            keyword[:80],
            keyword_candidates,
        )
        return [], "none", ""
    logger.info(
        "Registered durian source exact hit: keyword=%s rows=%d",
        matched_keyword[:80],
        len(evidence),
    )
    return (
...
def retrieve_durian_code_web_evidence(
    query: str,
    response_language: str = "zh",
) -> Tuple[List[Dict[str, Any]], str, str]:
    if not is_exact_fact_query(query):
        return [], "none", ""

    registry_evidence = retrieve_registered_durian_site_evidence(query)
    if registry_evidence[0]:
        return registry_evidence

    if not WEB_SEARCH_CONFIG["enabled"] or not WEB_SEARCH_CONFIG["api_key"]:
        return [], "none", ""

    web_queries = build_durian_code_web_queries(query)
    max_results = max(4, int(WEB_SEARCH_CONFIG.get("top_k", 4) or 4))
    web_results_by_query: Dict[str, List[Dict[str, Any]]] = {}
    with ThreadPoolExecutor(
        max_workers=min(6, max(1, len(web_queries))),
        thread_name_prefix="durian-code-web",
    ) as executor:
        futures = {
            executor.submit(
                tavily_web_search,
        ...
        for future in as_completed(futures):
            web_query = futures[future]
            try:
                web_evidence, _quality, _text = future.result()
            except Exception as exc:
                logger.warning(
                    "Durian code web query failed: query=%s error=%s",
                    web_query[:120],
                    exc,
                )
                continue
            web_results_by_query[web_query] = list(web_evidence or [])

    raw_web_evidence = [
        item
        for web_query in web_queries
        for item in web_results_by_query.get(web_query, [])
    ]
    exact_candidates = [
        item
        for item in raw_web_evidence
        if is_probable_durian_code_evidence(query, item)
    ]
    exact_candidates = rank_entity_evidence(query, exact_candidates)
    exact_evidence = merge_web_evidence(
        [],
        exact_candidates,
    ...
    if not exact_evidence:
        logger.info(
            "Durian code web fallback found no exact evidence: query=%s queries=%d raw=%d",
            str(query or "")[:120],
            len(web_queries),
            len(raw_web_evidence),
        )
        return [], "none", ""
    logger.info(
        "Durian code web fallback kept exact evidence=%d query=%s queries=%d",
        len(exact_evidence),
        str(query or "")[:120],
        len(web_queries),
    )
...
_SHORT_ENTITY_STOPWORDS = {
    "a", "an", "the", "is", "are", "what", "which", "how", "why",
    "of", "for", "about", "definition", "information", "identifier",
    "please", "this", "that",
}


def extract_short_entity_terms(query: str) -> List[str]:
    """Extract compact identifiers without assuming their domain."""
    value = unicodedata.normalize("NFKC", str(query or "")).strip()
    tokens = re.findall(r"[A-Za-z][A-Za-z0-9._/-]{1,}|(?<![A-Za-z])\d{2,}", value)
    result: List[str] = []
    seen = set()
    for token in tokens:
        normalized = token.strip()
        if not normalized or normalized.lower() in _SHORT_ENTITY_STOPWORDS:
            continue
        key = re.sub(r"[\s_-]+", "", normalized.lower())
...
def is_short_entity_query(query: str) -> bool:
    ...
    if not value or len(value) > 64:
        return False
    terms = extract_short_entity_terms(value)
    if not terms or len(terms) > 3:
        return False
    return any(
        any(char.isdigit() for char in term)
        or term.isupper()
        or (len(terms) == 1 and len(term) <= 32)
        for term in terms
    )


def build_short_entity_web_queries(query: str) -> List[str]:
    value = unicodedata.normalize("NFKC", str(query or "")).strip()
    terms = extract_short_entity_terms(value)
    if not terms:
...
def short_entity_evidence_matches(query: str, item: Dict[str, Any]) -> bool:
    terms = extract_short_entity_terms(query)
    if not terms:
        return False
    source = unicodedata.normalize("NFKC", _evidence_source_text(item)).lower()
    compact_source = re.sub(r"[\s_/-]+", "", source)
    return all(
        re.sub(r"[\s_/-]+", "", term.lower()) in compact_source
        for term in terms
    )


def retrieve_short_entity_web_evidence(
    query: str,
    response_language: str = "zh",
) -> Tuple[List[Dict[str, Any]], str, str]:
    if (
        not is_short_entity_query(query)
        or not WEB_SEARCH_CONFIG["enabled"]
        or not WEB_SEARCH_CONFIG["api_key"]
    ):
        return [], "none", ""

    web_queries = build_short_entity_web_queries(query)
    max_results = max(5, int(WEB_SEARCH_CONFIG.get("top_k", 4) or 4))
    results_by_query: Dict[str, List[Dict[str, Any]]] = {}
    with ThreadPoolExecutor(
        max_workers=min(6, max(1, len(web_queries))),
        thread_name_prefix="short-entity-web",
    ) as executor:
        futures = {
            executor.submit(
                tavily_web_search,
        ...
        for future in as_completed(futures):
            web_query = futures[future]
            try:
                candidates, _quality, _text = future.result()
            except Exception as exc:
                logger.warning(
                    "Short entity web query failed: query=%s error=%s",
                    web_query[:120],
                    exc,
                )
                continue
            results_by_query[web_query] = list(candidates or [])

    raw_candidates = [
        item
        for web_query in web_queries
        for item in results_by_query.get(web_query, [])
        if short_entity_evidence_matches(query, item)
    ]
    if not raw_candidates:
        logger.info("Short entity web fallback found no lexical evidence: query=%s", str(query or "")[:120])
        return [], "none", ""

    reranked = rerank_evidence(query, raw_candidates, top_k=len(raw_candidates))
    ranked_candidates = list(reranked or [])
    seen = {
        str(item.get("url") or item.get("source") or item.get("doc") or "")
        for item in ranked_candidates
    }
    ranked_candidates.extend(
        item
        for item in raw_candidates
        if str(item.get("url") or item.get("source") or item.get("doc") or "") not in seen
    )
    ranked_candidates = rank_entity_evidence(query, ranked_candidates)
    evidence = merge_web_evidence([], ranked_candidates, max_items=max_results)
    if not evidence:
        return [], "none", ""
    logger.info(
        "Short entity web fallback kept evidence=%d query=%s variants=%d",
        len(evidence),
        str(query or "")[:120],
        len(web_queries),
    )
...
def rank_entity_evidence(query: str, evidence: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Rank exact/short-entity evidence by lexical binding before generation.

    This is intentionally generic: the query terms drive the ranking, while
    source structure and authority are only tie-breakers. It prevents a social
    title or a partial snippet from outranking a registry-style row that binds
    the identifier to its name.
    """
    ranked: List[Tuple[float, int, Dict[str, Any]]] = []
    terms = extract_short_entity_terms(query)
    code_match = _DURIAN_CODE_RE.search(str(query or ""))
    code = f"D{code_match.group(1)}" if code_match else ""
    for index, raw_item in enumerate(evidence or []):
        item = dict(raw_item or {})
        text = unicodedata.normalize("NFKC", _evidence_source_text(item)).lower()
        compact = re.sub(r"[\s_/-]+", "", text)
        score = float(item.get("score") or item.get("rerank_score") or 0.0) * 0.25
        term_hits = sum(
            1
            for term in terms
            if re.sub(r"[\s_/-]+", "", str(term).lower()) in compact
        )
        score += 4.0 * term_hits / max(1, len(terms))

        if code:
            compact_code = re.sub(r"[\s_/-]+", "", code.lower())
            if compact_code in compact:
                score += 5.0
            if re.search(
                rf"{re.escape(code.lower())}\s*(?:[|｜|\t,:：=\-—]|对应|corresponds|registered|also|known)",
                text,
                re.I,
            ):
                score += 3.0
            try:
                parsed = extract_durian_code_lookup_answer(
                    query,
                    [item],
        ...
        source_blob = " ".join(
            str(item.get(key) or "")
            for key in ("title", "doc", "source", "url")
        ).lower()
        if any(marker in source_blob for marker in (".gov", ".edu", "jabatan", "department", "registry", "registered", "cultivar")):
            score += 2.0
        if any(marker in source_blob for marker in ("tiktok.com", "facebook.com", "instagram.com")):
            score -= 1.5
        item["entity_match_score"] = round(score, 4)
...
def merge_entity_evidence(
    ...
    *,
    ...
    """Merge entity evidence with web/structured hits ahead of broad local hits."""
    candidates = rank_entity_evidence(query, web_evidence) + rank_entity_evidence(query, local_evidence)
    return merge_web_evidence([], candidates, max_items=max_items)


def should_use_tavily_web_search(
    query: str,
    evidence: List[Dict[str, Any]],
    ...
    if not WEB_SEARCH_CONFIG["enabled"] or not WEB_SEARCH_CONFIG["api_key"]:
        return False
    if not str(query or "").strip() or is_smalltalk_query(str(query or "")):
        return False
    if env_bool("DURIAN_WEB_SEARCH_ALWAYS", False):
        return True
    quality = str(evidence_quality or "none").lower()
    if not evidence or quality in {"none", "low"}:
        return True
    return env_bool("DURIAN_WEB_SEARCH_ON_MEDIUM", False) and quality == "medium"


def merge_web_evidence(
    local_evidence: List[Dict[str, Any]],
    web_evidence: List[Dict[str, Any]],
    *,
    max_items: int = 8,
    ...
    seen_urls = set()
    if web_evidence:
        local_limit = max(1, max_items // 2)
        candidates = list(local_evidence or [])[:local_limit] + list(web_evidence or [])
        candidates += list(local_evidence or [])[local_limit:]
    else:
        candidates = list(local_evidence or [])
    for item in candidates:
        item = normalize_reference_item(item)
        url = str(item.get("url") or item.get("source") or "").strip()
        key = url or hashlib.sha1(rag_display_text(item).encode("utf-8", errors="ignore")).hexdigest()
        if key in seen_urls:
            continue
        seen_urls.add(key)
        merged.append(item)
...
_EXACT_FILL_BLANK_RE = re.compile(
...
def is_exact_answer_query(query: str) -> bool:
    value = str(query or "")
    backend_detector = globals().get("llamaindex_is_exact_answer_query")
    if callable(backend_detector):
        try:
            if bool(backend_detector(value)):
                return True
        except Exception:
            pass
    return bool(
        _EXACT_FILL_BLANK_RE.search(value)
        or ("填空" in value and any(mark in value for mark in ("___", "（", "(")))
        or is_exact_fact_query(value)
    )


_CULTIVAR_QUERY_ALIASES = {
    "黑刺": ["黑刺", "black thorn", "duri hitam", "ochee", "orchee"],
    "猫山王": ["猫山王", "musang king", "raja kunyit", "mao shan wang"],
    "红虾": ["红虾", "red prawn", "udang merah", "ang heh"],
    "苏丹王": ["苏丹王", "sultan", "d24"],
}

_DURIAN_CODE_RE = re.compile(
    r"(?<![A-Za-z0-9])D\s*-?\s*(\d{1,4})(?!\d)",
    re.I,
)

_IDENTIFIER_LOOKUP_TERMS = (
    "官方注册编号",
    "官方登记编号",
    "品种登记编号",
    "注册编号",
...
_IDENTIFIER_SYSTEM_DISCUSSION_TERMS = (
    "编号体系",
    "编码体系",
    "编号规则",
    "编码规则",
...
def extract_identifier_lookup_subject(query: str) -> str:
    """Extract the named entity from a short name-to-identifier question.

    This intentionally handles unknown cultivar names as well as aliases in the
    built-in table. Exact RAG should work for ``X 编号`` and ``X 是否有官方注册
    号`` without adding every future cultivar to a hard-coded dictionary.
    """
    original = unicodedata.normalize("NFKC", str(query or "")).strip()
    if not original or len(original) > 160:
        return ""
    lower = original.lower()
    if any(term in lower for term in _IDENTIFIER_SYSTEM_DISCUSSION_TERMS):
        return ""
    if not any(term in lower for term in _IDENTIFIER_LOOKUP_TERMS):
        return ""

    value = _DURIAN_CODE_RE.sub(" ", original)
    for term in sorted(_IDENTIFIER_LOOKUP_TERMS, key=len, reverse=True):
        value = re.sub(re.escape(term), " ", value, flags=re.I)
    value = re.sub(
        r"(?i)\b(?:what|which|is|does|do|has|have|the|a|an|of|for|durian|variety|cultivar)\b",
        " ",
        value,
    )
    value = re.sub(
        r"(?:请问|请查询|查询|查一下|帮我查|告诉我|我想知道|想确认|确认|"
        r"是否存在|有没有|是否有|有无|是什么|是多少|是哪一个|哪个|对应|"
        r"的|是|有|吗|呢)",
        " ",
        value,
    )
    value = re.sub(r"^(?:榴莲品种|品种|榴莲)\s*", "", value)
    value = re.sub(r"\s*(?:榴莲品种|品种|榴莲)$", "", value)
    value = re.sub(r"[\s，,。.!！?？:：;；、()（）\[\]【】'\"`]+", " ", value)
    value = re.sub(r"\s+", " ", value)
    # Retrieval queries may already be domain-scoped ("... 榴莲 durian").
    # Remove those scope words after whitespace normalization as well, so they
    # never become part of the entity anchor.
    value = re.sub(
        r"(?i)(?:^|\s)(?:榴莲品种|品种|榴莲|durian|variety|cultivar)(?=\s|$)",
        " ",
        value,
    )
...
# High-confidence Malaysian cultivar registration lookups. This is not a
# replacement for retrieval: exact lookup first tries to bind the requested code
# to names found in the retrieved reference window, then falls back to this map
# only when retrieval did not return usable evidence.
DURIAN_CULTIVAR_CODE_KNOWN_ALIASES = {
    "D24": ["苏丹王", "Sultan", "D24"],
    "D99": ["哥必利", "Kop Kecil", "Kob Kecil"],
    "D101": ["D101", "Mas Hadam"],
    "D123": ["Chanee", "青尼"],
    "D158": ["Kan Yao", "干尧", "Gan Yao"],
    "D159": ["Monthong", "金枕", "Mon Thong"],
    "D160": ["Tekka", "竹脚", "Musang Queen"],
    "D163": ["葫芦", "葫蘆", "Hor Lor", "Horlor", "Horloh", "Hulu", "Labu"],
    "D168": ["101", "IOI", "Johor Mas", "Hajah Hasmah", "Mas Muar"],
    "D175": ["红虾", "Red Prawn", "Udang Merah", "Ang Heh"],
    "D197": ["猫山王", "Musang King", "Raja Kunyit", "Mao Shan Wang"],
    "D198": ["金凤", "Golden Phoenix", "Jin Feng"],
    "D200": ["黑刺", "Black Thorn", "Duri Hitam", "Ochee", "Orchee"],
}

DURIAN_CULTIVAR_ALIAS_TO_CODE = {}
for _code, _aliases in DURIAN_CULTIVAR_CODE_KNOWN_ALIASES.items():
    for _alias in _aliases:
        _key = unicodedata.normalize("NFKC", str(_alias or "")).strip().lower()
        if _key and not re.fullmatch(r"d\s*-?\s*\d+", _key, flags=re.I):
            DURIAN_CULTIVAR_ALIAS_TO_CODE.setdefault(_key, _code)

_DURIAN_ALIAS_SPLIT_RE = re.compile(
    r"\s*(?:,|，|、|/|／|;|；|\||和|及|以及|与|或|还有|\band\b|\bor\b)\s*",
    re.I,
)
_DURIAN_ALIAS_LEAD_RE = re.compile(
    r"^(?:榴莲|durian|variety|cultivar|品种|又名|别名|俗称|称为|叫做|为)\s*",
    re.I,
)
_DURIAN_ALIAS_STOP_RE = re.compile(
    r"(?:。|\n|；|;|\.\s|\?|？|!|！|属于|是一种|是马来西亚|每年|作为|以|其|它|他|她)"
)
_DURIAN_ALIAS_START_NOISE_RE = re.compile(
    r"^(?:at|is|are|was|were|with|from|comes\s+from|has|have|offers?|posted|post)\b",
    re.I,
)
_DURIAN_ALIAS_SOURCE_NOISE_RE = re.compile(
    r"(?:\bfacebook\b|\binstagram\b|\btiktok\b|\byoutube\b|\bpost(?:ed)?\b|"
    r"\b\d{1,5}\s+[A-Za-z][A-Za-z0-9_-]{1,24}\b|"
    r"\b(?:shop|stall|store|seller|vendor|merchant)\b|"
    r"(?:水果店|专卖店|档口|摊位|商家|店铺))",
    re.I,
)
_DURIAN_ALIAS_DESCRIPTOR_NOISE_RE = re.compile(
    r"(?:清甜|花香|幼滑|少\s*纤维|少\s*纖維|纤维感|纖維感|纤维|纖維|"
    r"甜度|香气|香氣|口感|浓郁|濃郁|奶油|适合|適合|的人|入门|入門|"
    r"smooth|fragrant|sweet|creamy|taste|flavou?r|texture|beginner)",
    re.I,
)



_REFERENCE_URL_TRAILING_CHARS = " \t\r\n.,;:!?，。；：！？、'\"”’>》」』】）］｝"
_REFERENCE_URL_LEADING_CHARS = " \t\r\n<([{[‘“《「『（［｛"
_REFERENCE_URL_IN_TEXT_RE = re.compile(r"https?://[^\s<>\"'`]+", re.I)


def normalize_reference_url(value: Any) -> str:
    """Normalize URLs copied from search snippets and sentence punctuation."""
    url = unicodedata.normalize("NFKC", str(value or "")).strip()
    if not url:
        return ""
    url = url.strip(_REFERENCE_URL_LEADING_CHARS)
    while url:
        last = url[-1]
        if last in _REFERENCE_URL_TRAILING_CHARS:
            url = url[:-1].rstrip()
            continue
...
def normalize_reference_label(value: Any) -> str:
    ...
    if not label:
        return ""
    match = re.search(r"https?://[^\s<>]+", label, flags=re.I)
    if match:
        clean_url = normalize_reference_url(match.group(0))
        label = label[: match.start()] + clean_url + label[match.end() :]
    return label.rstrip(_REFERENCE_URL_TRAILING_CHARS + ")]}" ).strip()


def _is_markdown_link_url(text: str, start: int) -> bool:
...
def normalize_reference_urls_in_text(value: Any) -> str:
    """Clean URL tokens inside generated prose without breaking Markdown links."""
    text = str(value or "")
    if not text:
        return ""
    ...
    def _replace(match: re.Match) -> str:
        raw = match.group(0)
        clean_url = normalize_reference_url(raw)
        if not clean_url:
            return raw
        if _is_markdown_link_url(text, match.start()):
            return clean_url + ")"
        return clean_url

    return _REFERENCE_URL_IN_TEXT_RE.sub(_replace, text)


def normalize_reference_item(item: Dict[str, Any]) -> Dict[str, Any]:
    cloned = dict(item or {})
    for key in ("url", "source", "link"):
        if key in cloned and cloned.get(key):
            cloned[key] = normalize_reference_url(cloned.get(key))
    if cloned.get("doc"):
        cloned["doc"] = normalize_reference_label(cloned.get("doc"))
    return cloned


def _source_label(item: Dict[str, Any]) -> str:
    return normalize_reference_label(
        item.get("doc")
...
def _evidence_source_text(item: Dict[str, Any]) -> str:
    parts = []
    for key in ("title", "text", "display_text", "original_text", "index_text", "evidence"):
...
def _clean_cultivar_alias(value: str) -> str:
    alias = unicodedata.normalize("NFKC", str(value or ""))
    alias = re.sub(r"<[^>]+>", " ", alias)
    alias = re.sub(r"[*`#\[\]【】《》()（）]+", " ", alias)
    alias = re.sub(r"(?i)\bd\s*-?\s*\d{1,4}\b", " ", alias)
    alias = re.sub(r"(?i)\bdurian\b", " ", alias)
    alias = alias.replace("榴莲", " ")
    alias = _DURIAN_ALIAS_LEAD_RE.sub("", alias.strip())
    if _DURIAN_ALIAS_START_NOISE_RE.search(alias):
        return ""
    alias = re.split(
        r"\s*[:：]\s*(?:taste|origin|facts?|guide|price|review|info|information|介绍|特点|风味|口感|来源|资料)\b",
        alias,
    ...
    alias = re.split(
        r"(?:^|\s+)\b(?:at|is|are|was|were|with|from|comes\s+from|has|have|offers?)\b",
        alias,
        maxsplit=1,
        flags=re.I,
    )[0]
    alias = re.sub(r"\s+", " ", alias).strip(" -—:：,，。.;；|｜\t")
    if _DURIAN_ALIAS_START_NOISE_RE.search(alias) or _DURIAN_ALIAS_SOURCE_NOISE_RE.search(alias):
        return ""
    # Taste/texture/persona labels are evidence attributes, not cultivar names.
    if _DURIAN_ALIAS_DESCRIPTOR_NOISE_RE.search(alias):
        return ""
    if not alias or len(alias) > 36:
        return ""
    if len(alias) < 2 and not re.search(r"[\u3400-\u9fff]", alias):
        return ""
    if re.fullmatch(
        r"(?:n|new|a|an|the|is|are|was|were|at|of|from|in|on|for|with|has|have|registered|registration|variety|cultivar)",
        alias,
        flags=re.I,
    ):
        return ""
    if re.search(
        r"(?:^|\s)(?:is|are|a|an|the|new|registered|registration|malaysia|malaysian|variety|cultivar)(?:$|\s)",
        alias,
        flags=re.I,
    ):
        return ""
    if re.search(r"(?:为什么|原因|广泛|种植|优质|显著|特点|对应|参考|来源|资料|varieti|terbaru|bayan lepas|latest|new variety|from|dari|guide|info|known as|also known)", alias, re.I):
        return ""
    if not re.search(r"[A-Za-z0-9\u4e00-\u9fff]", alias):
        return ""
    return alias


def _dedupe_aliases(values: List[str], limit: int = 6) -> List[str]:
    result: List[str] = []
    seen = set()
    for value in values:
        alias = _clean_cultivar_alias(value)
        if not alias:
            continue
        key = alias.lower()
        if key in seen:
...
def _alias_fragments_from_raw(value: str) -> List[str]:
    raw = unicodedata.normalize("NFKC", str(value or "")).strip()
    if not raw:
        return []
    fragments: List[str] = []
    for match in re.finditer(r"([^()（）]{1,80}?)\s*[（(]([^()（）]{1,40})[）)]", raw):
        outside = match.group(1).strip(" -—:：,，。.;；|｜/／\t")
        inside = match.group(2).strip(" -—:：,，。.;；|｜/／\t")
        if outside:
            fragments.append(outside)
        if inside:
            fragments.append(inside)
    return fragments or [raw]


def _source_phrase(source: str, response_language: str = "zh") -> str:
    source = normalize_reference_urls_in_text(normalize_reference_label(source or "参考资料"))
    # Keep URL-adjacent closing punctuation out of browser auto-link detection.
    spacer = " " if re.search(r"https?://", source, flags=re.I) else ""
    if (response_language or "zh").lower().startswith("en"):
        return f"(Source: {source}{spacer})"
    return f"（来源：{source}{spacer}）"


def _format_code_alias_answer(code: str, aliases: List[str], source: str, response_language: str = "zh") -> str:
    aliases = _dedupe_aliases(aliases, limit=6)
    if not aliases:
        return ""
    primary = aliases[0]
    other = " / ".join(aliases[1:])
    source_phrase = _source_phrase(source, response_language)
    if (response_language or "zh").lower().startswith("en"):
        if other:
            return f"{code} corresponds to {primary} durian, also known as {other}. {source_phrase}"
        return f"{code} corresponds to {primary} durian. {source_phrase}"
    if other:
        return f"{code} 对应的是 {primary} 榴莲，也常见写作 {other}。{source_phrase}"
    return f"{code} 对应的是 {primary} 榴莲。{source_phrase}"


def _format_alias_code_answer(alias: str, code: str, source: str, response_language: str = "zh") -> str:
    source_phrase = _source_phrase(source, response_language)
    if (response_language or "zh").lower().startswith("en"):
        return f"{alias}'s durian registration code is {code}. {source_phrase}"
    return f"{alias} 的榴莲品种登记编号是 {code}。{source_phrase}"


def _known_aliases_present(code: str, text: str) -> List[str]:
    lower = unicodedata.normalize("NFKC", text or "").lower()
    aliases = []
    for alias in DURIAN_CULTIVAR_CODE_KNOWN_ALIASES.get(code, []):
        key = unicodedata.normalize("NFKC", alias).lower()
        if key and key in lower:
            aliases.append(alias)
    return _dedupe_aliases(aliases, limit=6)


def _extract_aliases_from_code_segment(code: str, segment: str) -> List[str]:
    normalized = unicodedata.normalize("NFKC", segment or "")
    aliases = _known_aliases_present(code, normalized)
    code_pattern = rf"D\s*-?\s*{re.escape(code[1:])}"
    relation = (
        r"(?:也被称为|也称为|又称为|又称|又叫|俗称|别称|被称为|称为|称作|叫做|"
        r"取为|写作|对应(?:的是|为)?|对应于|also\s+known\s+as|known\s+as|"
        r"called|aka|corresponds\s+to|registered\s+as|registered\s+name)"
    )
    patterns = [
        # Name-first prose: Hor Lor (Hulu), also known as D163.
        rf"(?is)([^。\n；;.!?]{{1,120}}?)\s*(?:,|，)\s*{relation}\s*{code_pattern}",
        rf"(?is)([^。\n；;.!?]{{1,120}}?)\s*(?:\(|（)\s*{relation}\s*{code_pattern}\s*(?:\)|）)",
        # Common registry/table forms: D229 | Cenderawasih, D229: Cenderawasih,
        # or a tab-separated row. The cleaner below removes headers and prose.
        rf"(?is){code_pattern}\s*(?:\||｜|\t|[,，;；:：=\-—])\s*([^\n|｜\t,，;；]{{1,120}})",
        rf"(?is)([^\n|｜\t,，;；]{{1,120}}?)\s*(?:\||｜|\t)\s*{code_pattern}",
        # Code-first prose without punctuation: D229 Cenderawasih.
        rf"(?is){code_pattern}\s+([^\n|｜\t,，;；]{{1,100}})",
        # Name-first forms require a visible binding: Cenderawasih (D229),
        # Cenderawasih - D229, or Cenderawasih D229. Do not allow arbitrary
        # text immediately before a code, which can bind a previous entity.
        rf"(?is)([^\n|｜\t,，;；]{{1,100}}?)\s*(?:\(|（)\s*{code_pattern}\s*(?:\)|）)",
        rf"(?is)([A-Za-z\u3400-\u9fff][A-Za-z0-9\u3400-\u9fff _-]{{1,100}}?)\s+(?:[-—:：])?\s*{code_pattern}",
        rf"(?is){code_pattern}.{{0,90}}?{relation}\s*([^。\n；;.!?？！]{{1,180}})",
        rf"(?is){relation}\s*([^。\n；;.!?？！]{{1,180}}).{{0,90}}?{code_pattern}",
        rf"(?is){code_pattern}\s*[:：=\-—]\s*([^。\n；;.!?？！]{{1,120}})",
        rf"(?is){code_pattern}\s*[,，]\s*([^。\n；;,.，]{{1,120}})",
    ]
    for pattern in patterns:
        for match in re.finditer(pattern, normalized):
            raw = match.group(1)
            stop = _DURIAN_ALIAS_STOP_RE.search(raw)
            if stop:
                raw = raw[: stop.start()]
            for fragment in _alias_fragments_from_raw(raw):
                aliases.extend(_DURIAN_ALIAS_SPLIT_RE.split(fragment))
    return _dedupe_aliases(aliases, limit=6)


def extract_durian_code_lookup_answer(
    query: str,
    evidence: List[Dict[str, Any]],
    response_language: str = "zh",
    *,
    ...
    """Deterministically answer durian cultivar code/name lookup questions.

    These facts are brittle under free generation: if a retrieved reference says
    D168 is 101/IOI, the model must not replace it with Red Prawn from memory.
    """
    if not is_exact_fact_query(query):
        return None
    ...
    requested_code_match = _DURIAN_CODE_RE.search(normalized_query)
    requested_code = f"D{requested_code_match.group(1)}" if requested_code_match else ""

    if requested_code:
        for item in evidence or []:
            source_text = _evidence_source_text(item)
            if not source_text:
                continue
            normalized_source = unicodedata.normalize("NFKC", source_text)
            code_matches = list(_DURIAN_CODE_RE.finditer(normalized_source))
            for code_index, code_match in enumerate(code_matches):
                if f"D{code_match.group(1)}" != requested_code:
                    continue
                segment_start = code_matches[code_index - 1].end() if code_index > 0 else max(0, code_match.start() - 260)
                segment_end = code_matches[code_index + 1].start() if code_index + 1 < len(code_matches) else min(len(normalized_source), code_match.end() + 320)
                segment = normalized_source[segment_start:segment_end]
                aliases = _extract_aliases_from_code_segment(requested_code, segment)
                if aliases:
                    return _format_code_alias_answer(
                        requested_code,
                        aliases,
                        _source_label(item),
                        response_language,
        ...
        if (
            allow_fallback
            and env_bool("DURIAN_EXACT_INTERNAL_FALLBACK_ENABLED", False)
            and not evidence
            and requested_code in DURIAN_CULTIVAR_CODE_KNOWN_ALIASES
        ):
            fallback = _format_code_alias_answer(
                requested_code,
                DURIAN_CULTIVAR_CODE_KNOWN_ALIASES[requested_code],
                "内置高置信品种登记表",
                response_language,
            )
            if fallback and (response_language or "zh").lower().startswith("zh"):
                return "参考资料不足。" + fallback
            return fallback
        return None

    requested_alias = ""
    requested_code_from_alias = ""
    for alias_key, code in sorted(DURIAN_CULTIVAR_ALIAS_TO_CODE.items(), key=lambda item: len(item[0]), reverse=True):
        if alias_key in query_lower:
            requested_alias = alias_key
            requested_code_from_alias = code
            break
    if not requested_alias or not requested_code_from_alias:
        return None

    display_alias = next(
        (
            alias
            for alias in DURIAN_CULTIVAR_CODE_KNOWN_ALIASES.get(requested_code_from_alias, [])
            if unicodedata.normalize("NFKC", alias).lower() == requested_alias
        ),
        requested_alias,
    )
    for item in evidence or []:
        ...
        if not source_text:
            continue
        lower = unicodedata.normalize("NFKC", source_text).lower()
        has_alias = requested_alias in lower
        has_code = any(f"D{match.group(1)}" == requested_code_from_alias for match in _DURIAN_CODE_RE.finditer(source_text))
        if has_alias and has_code:
            return _format_alias_code_answer(
                display_alias,
                requested_code_from_alias,
                _source_label(item),
                response_language,
    ...
    if (
        ...
        fallback = _format_alias_code_answer(
            display_alias,
            requested_code_from_alias,
            "内置高置信品种登记表",
            response_language,
...
def extract_registered_durian_answer(
    query: str,
    evidence: List[Dict[str, Any]],
    response_language: str = "zh",
) -> Optional[str]:
    """Extract exact code/name columns from the direct registry result."""
    normalized_query = unicodedata.normalize("NFKC", str(query or "")).strip()
    requested_code_match = _DURIAN_CODE_RE.search(normalized_query)
    requested_code = (
        f"D{requested_code_match.group(1)}"
        if requested_code_match
        else ""
    )
    requested_subject = extract_identifier_lookup_subject(normalized_query)
    requested_compact = _compact_registered_name(requested_subject)

    for item in evidence or []:
        if str(item.get("rag_backend") or "") != "durianpenang_registry":
            continue
        cells = [
            value.strip()
            for value in str(item.get("index_text") or "").split("|")
        ]
        if len(cells) < 2:
            continue
        row_code = re.sub(r"[\s_-]+", "", cells[0].upper())
        cultivar_name = cells[1].strip()
        other_names = [
            value.strip()
            for value in re.split(r"[,，;/]", cells[2] if len(cells) > 2 else "")
            if value.strip()
        ]
        source = _source_label(item)

        if requested_code and row_code == requested_code:
            aliases = [cultivar_name] + other_names
            aliases = [value for value in aliases if value and value not in {"—", "-"}]
            if aliases:
                return _format_code_alias_answer(
                    requested_code,
                    aliases,
        ...
        if requested_compact:
            row_names = [cultivar_name] + other_names
            if any(
                _compact_registered_name(value) == requested_compact
                for value in row_names
                if value
            ):
                return _format_alias_code_answer(
                    cultivar_name or requested_subject,
                    row_code,
                    source,
                    response_language,
                )
...
def format_registered_durian_details(
    evidence: List[Dict[str, Any]],
    response_language: str = "zh",
) -> Optional[str]:
    """Render a registry row directly, without generative fact expansion."""
    registry_item = next(
        (
            item
            for item in evidence or []
    ...
    cells = [
        value.strip()
        for value in str(registry_item.get("index_text") or "").split("|")
    ]
    if len(cells) < 2 or not cells[0] or not cells[1]:
        return None
    cells += [""] * (8 - len(cells))
    code, name, other_name, state, year, registered_by, category, characteristics = cells[:8]
    source_title = str(registry_item.get("title") or "Registered Malaysian Durian Varieties")
    source_url = str(registry_item.get("url") or registry_item.get("source") or "").strip()

    labels = {
        "zh": ("品种名称", "官方登记编号", "其他名称", "登记州属", "登记年份", "登记机构", "申请类别", "登记表原文特征", "来源"),
        "en": ("Variety name", "Official registration code", "Other name", "State", "Registration year", "Registered by", "Applicant category", "Registry characteristics", "Source"),
        "ms": ("Nama varieti", "Kod pendaftaran rasmi", "Nama lain", "Negeri", "Tahun pendaftaran", "Didaftarkan oleh", "Kategori pemohon", "Ciri asal dalam daftar", "Sumber"),
        "th": ("ชื่อพันธุ์", "รหัสทะเบียนอย่างเป็นทางการ", "ชื่ออื่น", "รัฐ", "ปีที่จดทะเบียน", "หน่วยงานจดทะเบียน", "ประเภทผู้สมัคร", "ลักษณะตามทะเบียนต้นฉบับ", "แหล่งที่มา"),
    }
    language = str(response_language or "zh").lower()
    lang_key = language if language in labels else "zh"
    field_labels = labels[lang_key]
    values = [name, code, other_name, state, year, registered_by, category, characteristics]
    lines = [f"- **{label}**：{value}" for label, value in zip(field_labels[:8], values) if value]
    source_value = f"[{source_title}]({source_url})" if source_url else source_title
    lines.append(f"- **{field_labels[8]}**：{source_value}")
    intro = {
        "zh": "根据官方登记表，这个品种的完整登记信息如下：",
        "en": "According to the official registry row, the complete recorded information is:",
        "ms": "Berdasarkan baris daftar rasmi, maklumat lengkap yang direkodkan ialah:",
        "th": "ตามแถวข้อมูลทะเบียนอย่างเป็นทางการ รายละเอียดที่บันทึกไว้มีดังนี้:",
    }[lang_key]
    return intro + "\n\n" + "\n".join(lines)



def extract_partial_exact_answer(
    query: str,
    evidence: List[Dict[str, Any]],
    response_language: str = "zh",
) -> Optional[str]:
    """Return a transparent partial hit instead of letting exact facts drift."""
    if not is_exact_fact_query(query) or not evidence:
        return None
    code_match = _DURIAN_CODE_RE.search(str(query or ""))
    if not code_match:
        return None
    ...
    matched_item = next(
        (
            ...
            in re.sub(
                r"[\s_/-]+",
                "",
                unicodedata.normalize("NFKC", _evidence_source_text(item)).lower(),
            )
        ),
        None,
    )
    if not matched_item:
        return None
    source = _source_label(matched_item)
    if (response_language or "zh").lower().startswith("en"):
        return (
            f"The retrieved material confirms that {code} appears in a durian reference, "
            f"but this passage does not reliably state the corresponding cultivar name. (Source: {source})"
        )
    if (response_language or "zh").lower().startswith("ms"):
        return (
            f"Bahan rujukan mengesahkan {code} muncul dalam rujukan durian, "
            f"tetapi petikan ini tidak menyatakan nama varieti yang sepadan dengan boleh dipercayai. (Sumber: {source})"
        )
    if (response_language or "zh").lower().startswith("th"):
        return (
            f"เอกสารที่ค้นพบยืนยันว่า {code} ปรากฏอยู่ในข้อมูลทุเรียน "
            f"แต่ข้อความนี้ยังไม่ระบุชื่อสายพันธุ์ที่ตรงกันอย่างน่าเชื่อถือ (แหล่งที่มา: {source})"
        )
    return f"检索资料确认 {code} 出现在榴莲品种资料中，但当前片段没有可靠给出它对应的品种名称。（来源：{source}）"


def is_exact_fact_query(query: str) -> bool:
    value = unicodedata.normalize("NFKC", str(query or "")).lower()
    code_matches = list(_DURIAN_CODE_RE.finditer(value))

    # A single registration code is already a complete lookup subject. Short
    # continuations such as "D200呢" must not fall through to free generation
    # merely because the user omitted the word "编号".
    if len(code_matches) == 1 and len(value) <= 80:
        code_match = code_matches[0]
        remainder = value[: code_match.start()] + value[code_match.end() :]
        compact_remainder = re.sub(
            r"[\s，,。.!！?？:：;；、()（）\[\]【】]+",
            "",
            remainder,
        )
        ...
        if compact_remainder in short_code_lookups:
            return True
    ...
    if any(
        alias in value
        for aliases in _CULTIVAR_QUERY_ALIASES.values()
        for alias in aliases
    ):
        return True

    # Any short, named entity followed by an identifier intent is an exact
    # lookup, even when the cultivar has never been hard-coded before. This
    # keeps ``Dato Nina 编号`` and future cultivars out of conversational
    # rewriting/free generation while excluding broad questions about the
    # numbering system itself.
    if extract_identifier_lookup_subject(value):
        return True

    # Unknown cultivar names must still enter the evidence gate. Otherwise a
    # short "X 的编码是什么" question falls through to free generation and the
    # model may invent a registration number. Keep broad discussions such as
    # "编码体系如何工作" outside exact mode.
    exact_ask = any(
        term in value
        for term in (
            "是什么",
    ...
    return bool(exact_ask and len(value) <= 120)


def _normalize_exact_text(text: str) -> str:
    value = unicodedata.normalize("NFKC", str(text or ""))
    backend_normalizer = globals().get("llamaindex_normalize_ocr_units")
    if callable(backend_normalizer):
        try:
            value = backend_normalizer(value)
        except Exception:
            pass
    value = re.sub(r"°\s*[cC]", "℃", value)
    value = re.sub(r"<!--[\s\S]*?-->", " ", value)
    value = re.sub(r"[*`#]+", "", value)
    value = re.sub(r"\\+mathrm\{mm\}", "mm", value, flags=re.I)
    value = re.sub(r"\\+mathrm\{cm\}", "cm", value, flags=re.I)
    value = re.sub(
        r"\b(?:content|ntent|text|type|spans|lines)\b",
        "",
        value,
        flags=re.I,
    )
...
def _exact_question_body(query: str) -> str:
    value = str(query or "").strip()
    if not value:
        return ""
    prefix_markers = ("填空", "原文", "知识库", "只填写", "只填")
    colon_positions = [value.find(mark) for mark in ("：", ":") if mark in value]
    if colon_positions and any(marker in value[: max(colon_positions) + 1] for marker in prefix_markers):
        value = value[min(position for position in colon_positions if position >= 0) + 1 :]
    value = re.sub(
...
def extract_exact_fill_answer(
    query: str,
    evidence: List[Dict[str, Any]],
) -> Optional[str]:
    """Deterministically copy blank values from retrieved source text."""
    body = _exact_question_body(query)
    if not body or not _EXACT_FILL_BLANK_RE.search(body):
        return None

    raw_segments = _EXACT_FILL_BLANK_RE.split(body)
    segments = [_normalize_exact_text(segment) for segment in raw_segments]
    ...
    for item in evidence or []:
        source_text = str(
            ...
            or item.get("index_text")
            or ""
        )
        normalized_source = _normalize_exact_text(source_text)
        if not normalized_source:
            continue

        first_anchor = segments[0]
        if first_anchor:
            starts = []
            search_at = 0
            while True:
                found = normalized_source.find(first_anchor, search_at)
                if found < 0:
                    break
                starts.append(found + len(first_anchor))
                search_at = found + 1
        else:
            starts = [0]

        candidates = []
        for initial_cursor in starts:
            cursor = initial_cursor
            answers = []
            matched = True
            for index in range(blank_count):
                next_anchor = segments[index + 1]
                if next_anchor:
                    next_position = normalized_source.find(next_anchor, cursor)
                    if next_position < 0:
                        matched = False
                        break
                    answer = normalized_source[cursor:next_position]
                    cursor = next_position + len(next_anchor)
                else:
                    tail = normalized_source[cursor:]
                    boundary = re.search(r"[。;；,，]", tail)
                    answer = tail[: boundary.start()] if boundary else tail[:60]
                    cursor += len(answer)
            ...
            if matched and len(answers) == blank_count:
        ...
        if candidates:
            ...
            if any(
                phrase in query
                for phrase in ("只填写答案", "只填答案", "只填写", "只填")
            ):
                return "；".join(answers)
            if len(answers) == 1:
                return f"填空答案：{answers[0]}"
            return "填空答案：" + "；".join(
                f"{index}. {answer}"
...
def extract_exact_fact_answer(
    query: str,
    evidence: List[Dict[str, Any]],
) -> Optional[str]:
    """Extract a cultivar/code pair from one cited source window."""
    if not is_exact_fact_query(query):
        return None
    ...
    for canonical_name, aliases in _CULTIVAR_QUERY_ALIASES.items():
        if any(alias in normalized_query for alias in aliases):
            requested_name = canonical_name
            requested_aliases = aliases
            break

    # Do not require every cultivar to be present in a fixed alias map. For a
    # short identifier lookup, bind the entity written by the user to a D-code
    # found on the same source line/window. The evidence still has to contain
    # both the name and the code; otherwise extraction returns no answer.
    if not requested_name:
        generic_subject = extract_identifier_lookup_subject(query)
        if generic_subject:
            requested_name = generic_subject
            requested_aliases = [generic_subject.lower()]
    ...
    for item in evidence or []:
        ...
        source_name_hit = next(
            (alias for alias in requested_aliases if alias in source_lower),
            "",
        )

        if requested_name:
            if not source_name_hit:
                continue
            alias_at = source_lower.find(source_name_hit)

            # Cultivar fact tables commonly keep several name/code pairs in a
            # single chunk. Resolve the code on the alias's own source line
            # first; a global nearest-code rule can otherwise attach the code
            # from the preceding row (for example D200 to 猫山王/D197).
            line_start = normalized_source.rfind("\n", 0, alias_at) + 1
            line_end = normalized_source.find("\n", alias_at)
            if line_end < 0:
                line_end = len(normalized_source)
            same_line = normalized_source[line_start:line_end]
            alias_in_line = source_lower[line_start:line_end].find(
                source_name_hit
            )
            line_codes = list(
                _DURIAN_CODE_RE.finditer(same_line)
            )
            if line_codes:
                # Registry tables usually place the code before the cultivar
                # name (``D2 | Dato Nina``), while prose may put it after.
                # Always choose the nearest code on the same row; preferring a
                # later code can accidentally jump into the next table row.
                selected = min(
                    line_codes,
                    key=lambda match: min(
                        abs(match.start() - alias_in_line),
                        abs(match.end() - alias_in_line),
                    ),
                )
                code = f"D{selected.group(1)}"
                source = str(
                    item.get("doc")
                    or item.get("source_file")
                    or "知识库原文"
                ...
                return (
                    f"{requested_name}的品种登记编号是 {code}。"
                    f"（来源：{source}）"
                )

            window_start = max(0, alias_at - 120)
            window_end = min(
                len(normalized_source),
                alias_at + len(source_name_hit) + 160,
            )
            window = normalized_source[window_start:window_end]
            codes = list(
                _DURIAN_CODE_RE.finditer(window)
            )
            if not codes:
                continue
            alias_in_window = source_lower[window_start:window_end].find(
                source_name_hit
            )
            nearest = min(
                codes,
                key=lambda match: min(
                    abs(match.start() - alias_in_window),
                    abs(match.end() - alias_in_window),
                ),
            )
            code = f"D{nearest.group(1)}"
            source = str(
                item.get("doc")
                or item.get("source_file")
                or "知识库原文"
            )
            return f"{requested_name}的品种登记编号是 {code}。（来源：{source}）"

        if requested_code and requested_code_match:
            # Reverse lookup must tolerate OCR/Markdown variants such as
            # "D 197" and "D-197". Bind the requested code to the closest
            # cultivar alias inside the interval bounded by its neighbouring
            # codes; this prevents a multi-row fact chunk from attaching D197
            # to the D200 row.
            code_number = requested_code_match.group(1)
            code_matches = list(_DURIAN_CODE_RE.finditer(normalized_source))
            for code_index, code_match in enumerate(code_matches):
                if code_match.group(1) != code_number:
                    continue

                segment_start = (
                    code_matches[code_index - 1].end()
                    if code_index > 0
                    else max(0, code_match.start() - 240)
                )
                segment_end = (
                    code_matches[code_index + 1].start()
                    if code_index + 1 < len(code_matches)
                    else min(len(normalized_source), code_match.end() + 240)
                )
                segment_lower = source_lower[segment_start:segment_end]
                code_at = code_match.start() - segment_start
                alias_hits = []
                for canonical_name, aliases in _CULTIVAR_QUERY_ALIASES.items():
                    for alias in aliases:
                        search_at = 0
                        while True:
                            alias_at = segment_lower.find(alias, search_at)
                            if alias_at < 0:
                                break
                            alias_hits.append(
                                (
                                    canonical_name,
                                    alias_at,
                                    abs(alias_at - code_at),
                                    alias_at > code_at,
                                )
                            )
                            search_at = alias_at + max(1, len(alias))

                if not alias_hits:
                    continue
                # Prefer the nearest name before the code (the authoritative
                # facts use "cultivar = code"), then the nearest name after it
                # for code-first tables.
                before_code = [
                    hit for hit in alias_hits if not hit[3]
                ]
                selected = (
                    min(before_code, key=lambda hit: hit[2])
                    if before_code
                    else min(alias_hits, key=lambda hit: hit[2])
                )
                source = str(
                    item.get("doc")
                    or item.get("source_file")
                ...
                return (
                    f"{requested_code}对应的榴莲品种是{selected[0]}。"
                    f"（来源：{source}）"
                )
    return None


def exact_answer_not_found(response_language: str = "zh") -> str:
    messages = {
        "zh": "知识库原文中没有找到能够可靠回答这个精确问题的内容。",
        "en": "No source passage in the knowledge base reliably answers this exact question.",
        "ms": "Tiada petikan sumber dalam pangkalan pengetahuan yang dapat menjawab soalan tepat ini dengan boleh dipercayai.",
        "th": "ไม่พบข้อความต้นฉบับในฐานความรู้ที่ตอบคำถามแบบเจาะจงนี้ได้อย่างน่าเชื่อถือ",
    }
    return messages.get(response_language, messages["zh"])


def grounded_answer_not_found(response_language: str = "zh") -> str:
    messages = {
        "zh": "RAGFlow 知识库中没有找到足以支持回答这项内容的原文，因此我不进行推测。",
        "en": "RAGFlow did not find a source passage sufficient to support an answer, so I will not speculate.",
        "ms": "RAGFlow tidak menemui petikan sumber yang mencukupi untuk menyokong jawapan, jadi saya tidak akan membuat andaian.",
        "th": "RAGFlow ไม่พบข้อความต้นฉบับที่เพียงพอสำหรับรองรับคำตอบ ดังนั้นฉันจะไม่คาดเดา",
    }
    return messages.get(response_language, messages["zh"])


def image_grounded_answer_not_found(
    label: str,
    confidence: float,
    response_language: str = "zh",
) -> str:
    messages = {
        "zh": (
            f"视觉模型初步识别为“{label}”，置信度约为{confidence:.0%}；"
            "但 RAGFlow 知识库没有找到可用于核实或支持进一步建议的原文，"
            "因此我不补充品种事实、病害结论或处理方案。"
        ),
        "en": (
            f"The vision model tentatively identified “{label}” with about "
            f"{confidence:.0%} confidence, but RAGFlow found no source passage "
            "to verify it or support further advice."
        ),
        "ms": (
            f"Model penglihatan mengenal pasti “{label}” secara awal dengan "
            f"keyakinan kira-kira {confidence:.0%}, tetapi RAGFlow tidak "
            "menemui sumber untuk pengesahan atau nasihat lanjut."
        ),
        "th": (
            f"โมเดลภาพประเมินเบื้องต้นว่าเป็น “{label}” ด้วยความมั่นใจประมาณ "
            f"{confidence:.0%} แต่ RAGFlow ไม่พบแหล่งข้อมูลสำหรับยืนยันหรือให้คำแนะนำเพิ่มเติม"
        ),
    }
    return messages.get(response_language, messages["zh"])


def image_no_rag_caution_instruction(response_language: str = "zh") -> str:
    messages = {
        "zh": (
            "\n\n【无知识库命中时的图片回答规则】"
            "本轮 RAGFlow 没有找到可直接引用的原文。仍然要基于视觉识别结果、可见症状和用户问题给出谨慎分析。"
            "明确说明这是初步判断；不要编造来源、品种编号、药剂名称、剂量或保证性结论。优先给低风险检查和处理方向。"
        ),
        "en": (
            "\n\n[Image answer rule when no knowledge-base source is found] "
            "RAGFlow did not find a directly citable passage for this turn. Still answer from the visual label, visible evidence, and the user's question. "
            "Make the uncertainty clear; do not invent sources, cultivar codes, chemical names, dosages, or guaranteed conclusions. Prefer low-risk checks and next steps."
        ),
        "ms": (
            "\n\n[Peraturan jawapan imej apabila tiada sumber pangkalan pengetahuan] "
            "RAGFlow tidak menemui petikan yang boleh dipetik secara langsung. Jawab berdasarkan label visual, bukti imej dan soalan pengguna secara berhati-hati. "
            "Nyatakan ketidakpastian; jangan reka sumber, kod varieti, nama bahan kimia, dos atau kesimpulan pasti."
        ),
        "th": (
            "\n\n[กฎคำตอบภาพเมื่อไม่พบแหล่งข้อมูลในฐานความรู้] "
            "RAGFlow ไม่พบข้อความที่อ้างอิงได้โดยตรง ให้ตอบจากป้ายกำกับภาพ หลักฐานที่เห็น และคำถามของผู้ใช้อย่างระมัดระวัง "
            "ระบุความไม่แน่นอน และอย่าสร้างแหล่งที่มา รหัสสายพันธุ์ ชื่อสารเคมี อัตราใช้ หรือข้อสรุปที่รับประกันเอง."
        ),
    }
    return messages.get(response_language, messages["zh"])


def text_no_rag_caution_instruction(response_language: str = "zh") -> str:
    messages = {
        "zh": (
            "\n\n【无可靠知识库命中时的文本回答规则】"
            "本轮没有可直接引用的 RAGFlow 原文。仍然要回答用户问题，但必须用谨慎语气。"
            "如果用户提到一个知识库未命中的品种名、编号、药剂、病名或精确事实，不要把它硬套成已知品种或编造事实；"
            "没有可靠资料时，不得输出甜度指数、10分制评分、明确排名、固定产地、价格、药剂剂量或固定周期；"
            "要明确说“未找到可靠资料确认该名称/事实”，再给通用判断、排查方向或需要补充的信息。"
        ),
        "en": (
            "\n\n[Text answer rule when no reliable knowledge-base source is found] "
            "No directly citable RAGFlow passage was found. Still answer cautiously. "
            "If the user mentions an unverified cultivar name, code, chemical, disease name, or exact fact, do not map it to a known item or invent facts; state that it is not confirmed, then give general checks or next steps."
        ),
        "ms": (
            "\n\n[Peraturan jawapan teks apabila tiada sumber pangkalan pengetahuan yang boleh dipercayai] "
            "Jawab dengan berhati-hati. Jika nama varieti, kod, bahan kimia, penyakit atau fakta tepat tidak disahkan oleh sumber, jangan reka atau padankan kepada item lain."
        ),
        "th": (
            "\n\n[กฎคำตอบข้อความเมื่อไม่พบแหล่งข้อมูลที่เชื่อถือได้] "
            "ตอบอย่างระมัดระวัง หากชื่อสายพันธุ์ รหัส สารเคมี โรค หรือข้อเท็จจริงเฉพาะไม่ถูกยืนยัน อย่าเดาหรือจับคู่เป็นสิ่งอื่น."
        ),
    }
    return messages.get(response_language, messages["zh"])


def extract_named_durian_subjects(query: str) -> List[str]:
    query_text = unicodedata.normalize("NFKC", str(query or "")).lower()
    subjects: List[str] = []
    for match in re.finditer(r"([\u3400-\u9fffA-Za-z0-9][\u3400-\u9fffA-Za-z0-9\- ]{0,18})榴莲", query_text):
        subject = re.sub(r"\s+", "", match.group(1)).strip(" ，,。.!！?？:：;；、()（）[]【】")
        subject = re.sub(r"^(?:请问|请|帮我|这个|那个|我的|一颗|一棵|这种|有关|关于)+", "", subject)
        if 2 <= len(subject) <= 12 and subject not in {"什么", "哪些", "怎么", "如何", "种植", "普通", "一般"}:
            subjects.append(subject)
    return subjects


def build_no_rag_named_subject_response(query: str, response_language: str = "zh") -> str:
    subjects = extract_named_durian_subjects(query)
    if not subjects:
        return ""
    subject = subjects[0]
    query_text = str(query or "").lower()
    asks_fertilizer = any(term in query_text for term in ["施肥", "肥料", "fertilizer", "baja", "ปุ๋ย"])

    if response_language == "en":
        return (
            f"I did not find reliable knowledge-base material confirming '{subject} durian', so I should not map it to another cultivar or invent cultivar-specific facts.\n\n"
            "For a real durian tree, use general fertilization principles: check tree age, leaf color, recent flushing/flowering/fruiting stage, soil moisture and drainage first; apply balanced organic matter and split NPK applications instead of one heavy dose; reduce nitrogen during flowering and fruit filling; and correct pH/drainage before increasing fertilizer."
        )
    if response_language == "ms":
        return (
            f"Saya tidak menemui sumber pangkalan pengetahuan yang mengesahkan '{subject} durian', jadi saya tidak patut memadankannya kepada varieti lain atau mereka fakta khusus.\n\n"
            "Untuk pokok durian sebenar, ikut prinsip umum: semak umur pokok, warna daun, fasa pucuk/bunga/buah, kelembapan dan saliran tanah; gunakan bahan organik dan baja NPK secara berpecah; elakkan nitrogen berlebihan semasa berbunga dan pembesaran buah."
        )
    if response_language == "th":
        return (
            f"ไม่พบข้อมูลในฐานความรู้ที่ยืนยัน '{subject} durian' จึงไม่ควรจับคู่เป็นสายพันธุ์อื่นหรือสร้างข้อเท็จจริงเฉพาะขึ้นเอง.\n\n"
            "สำหรับต้นทุเรียนจริง ให้ใช้หลักทั่วไป: ตรวจอายุต้น สีใบ ระยะใบอ่อน/ดอก/ผล ความชื้นและการระบายน้ำ; ใส่อินทรียวัตถุและแบ่งใส่ปุ๋ย NPK เป็นครั้ง ๆ; ลดไนโตรเจนมากเกินไปในช่วงออกดอกและขยายผล."
        )

    if asks_fertilizer:
        return (
            f"我没有在知识库里找到能确认“{subject}榴莲”的可靠资料，所以不能把它硬套成金枕、猫山王或其他已知品种来讲。\n\n"
            "如果你说的是一棵真实榴莲树，施肥先按通用榴莲管理处理：看树龄、叶色、新梢/开花/坐果阶段、土壤湿度和排水；先补有机质和改良根区，再少量多次追施复合肥；开花和膨果期不要猛加氮肥；如果叶黄或长势弱，先排查积水、根系和 pH，再决定是否加肥。"
        )
    return (
        f"我没有在知识库里找到能确认“{subject}榴莲”的可靠资料，所以不能把它硬套成某个已知品种或编造具体事实。\n\n"
        "你可以补充图片、产地、登记名、果肉/果壳特征或你想问的具体场景；在没有这些信息前，我只能给通用榴莲种植或消费判断。"
    )
...
def build_messages(
    ...
    system_prompt += language_lock_instruction(response_language)
    ...
    if evidence_text:
        grounding_rule = {
            "zh": (
                "\n\n【唯一事实来源】本轮事实性内容只能来自下方检索资料（本地知识库或联网资料）。"
                "历史消息、记忆卡和图片标签只能帮助理解用户指代，不能作为品种编号、特征、处理方法或其他事实的依据。"
                "禁止使用模型自身记忆补写原文没有的信息，禁止把推测写成事实。"
            ),
            "en": (
                "\n\n[Single factual source] Factual claims in this answer must come only from the RAGFlow passages below. "
                "Conversation history, memory cards, and image labels may resolve references but are not factual sources. "
                "Do not fill gaps from model memory or present guesses as facts."
            ),
            "ms": (
                "\n\n[Sumber fakta tunggal] Semua dakwaan fakta mesti datang hanya daripada petikan RAGFlow di bawah. "
                "Sejarah, kad memori dan label imej hanya boleh membantu memahami rujukan pengguna."
            ),
            "th": (
                "\n\n[แหล่งข้อเท็จจริงเดียว] ข้อเท็จจริงในคำตอบต้องมาจากข้อความ RAGFlow ด้านล่างเท่านั้น "
                "ประวัติ ความจำ และป้ายกำกับภาพใช้ได้เฉพาะเพื่อทำความเข้าใจสิ่งที่ผู้ใช้กล่าวถึง",
            ),
        }
        system_prompt += grounding_rule.get(
            response_language,
            grounding_rule["zh"],
        )
        if response_language == "en":
            system_prompt += (
                "\n\nThe following are retrieved source passages from the local knowledge base or web search. Use only relevant statements from them. "
                "If a requested detail is absent, say it is absent; do not add general advice from memory.\n\n"
                f"{evidence_text}"
            )
        elif response_language == "ms":
            system_prompt += (
            ...
            system_prompt += (
                "\n\n以下是本轮检索到的参考资料（本地知识库或联网资料）。只能使用其中与问题直接相关的内容；"
                "资料没有的细节必须明确说没有；不得用模型记忆补充精确评分、产地断言、品种排名、价格、药剂剂量或固定周期。\n\n"
                f"{evidence_text}"
            )

    else:
        system_prompt += text_no_rag_caution_instruction(response_language)
...
def run_analyze_pest_core_from_bytes(
    ...
    evidence, evidence_quality, evidence_text = (
        retrieve_ragflow_grounded_context(
            search_query,
            response_language,
            exact_mode=False,
            relevance_query=(query + "\n" + class_name).strip(),
            timeout_sec=float(os.getenv("DURIAN_RAGFLOW_LIVE_TIMEOUT_SEC", "6.0")),
        )
    )

    # RAGFlow may miss valid image cases. Continue with cautious image-only
    # analysis instead of returning a no-source refusal.
    ...
    if evidence_text:
        ...
        system_prompt += image_no_rag_caution_instruction(response_language)
...
def run_analyze_pest_core_from_vl_label(
    ...
    # RAGFlow may miss valid image cases. Continue with cautious image-label
    # analysis instead of returning a no-source refusal.
    ...
    prompt_evidence_text = build_image_prompt_evidence_text(evidence, image_profile)
    if prompt_evidence_text:
...
""".strip()
    ...
    if image_profile == "image_agri_medium":
        image_final_tokens = int(os.getenv("DURIAN_IMAGE_AGRI_MAX_NEW_TOKENS", "768"))
    elif image_profile == "image_consumer_natural":
    ...
    image_final_tokens = clamp_image_final_tokens(image_final_tokens)
...
def ragflow_evidence_matches_query_anchors(
    query: str,
    evidence: List[Dict[str, Any]],
) -> bool:
    """Reject semantically nearby chunks that omit the user's core subject."""
    query_text = unicodedata.normalize("NFKC", str(query or "")).lower()
    ...
    query_codes = {
        match.group(1)
        for match in _DURIAN_CODE_RE.finditer(query_text)
    }
    if query_codes:
        evidence_codes = {
            match.group(1)
            for match in _DURIAN_CODE_RE.finditer(evidence_text)
        }
        return bool(query_codes.intersection(evidence_codes))

    for aliases in _CULTIVAR_QUERY_ALIASES.values():
        if any(alias in query_text for alias in aliases):
            return any(alias in evidence_text for alias in aliases)

    identifier_subject = extract_identifier_lookup_subject(query_text)
    if identifier_subject:
        compact_subject = re.sub(
            r"[^a-z0-9\u3400-\u9fff]+",
            "",
            identifier_subject.lower(),
        )
        ...
        matched = bool(
            compact_subject
            and compact_subject in compact_evidence
        )
        logger.info(
            "RAGFlow exact identifier anchor: subject=%s matched=%s query=%s",
            identifier_subject,
            matched,
            query_text[:120],
        )
    ...
    named_durian_subjects = extract_named_durian_subjects(query_text)
    if named_durian_subjects:
        subject_matched = [subject for subject in named_durian_subjects if subject in evidence_text]
        if not subject_matched:
            logger.info(
                "RAGFlow anchor gate rejected missing named durian subject: subjects=%s query=%s",
                named_durian_subjects[:6],
                query_text[:120],
            )
            return False

    # Match symptom concepts, not only identical wording. For example, a user
    # may say "根系腐烂、流胶" while the source says "根部容易腐烂、溃疡伤口
    # 分泌脓液". Those are valid supporting passages and must not be discarded.
    query_topic_tags = _rag_topic_tags(query_text)
    strong_query_tags = {
        tag
        for tag in query_topic_tags
        if str((RAG_TOPIC_RULES.get(tag) or {}).get("group") or "")
        in {"disease", "pest"}
    }
    if strong_query_tags:
        evidence_topic_tags = _rag_topic_tags(evidence_text)
        matched_tags = strong_query_tags.intersection(evidence_topic_tags)
        logger.info(
            "RAGFlow concept anchor gate: query_tags=%s evidence_tags=%s matched=%s query=%s",
            sorted(strong_query_tags),
            sorted(evidence_topic_tags),
            sorted(matched_tags),
            query_text[:120],
        )
        return bool(matched_tags)
    ...
    cleaned = re.sub(r"[的是有和与或在对把给会能列了嘛吗呀啊]", " ", cleaned)

    anchors = set()
    for block in re.findall(r"[\u3400-\u9fff]{2,}", cleaned):
        if len(block) <= 3:
            anchors.add(block)
        else:
            anchors.update(
    ...
    matched = {anchor for anchor in anchors if anchor in evidence_text}
    coverage = len(matched) / max(1, len(anchors))
    logger.info(
        "RAGFlow anchor gate: anchors=%s matched=%s coverage=%.2f query=%s",
        sorted(anchors)[:16],
        sorted(matched)[:16],
        coverage,
        query_text[:120],
    )
    return bool(matched and coverage >= 0.50)


def retrieve_ragflow_grounded_context(
    query: str,
    response_language: str = "zh",
    *,
    ...
    """The only factual retrieval path used by live text and image chat."""
    del response_language  # Kept in the signature for one shared call shape.
    normalized_query = str(query or "").strip()
    if (
        not normalized_query
        or ragflow_retriever is None
        or not getattr(ragflow_retriever, "enabled", False)
    ):
        logger.warning(
            "RAGFlow-only retrieval unavailable: enabled=%s query=%s",
            bool(
                ragflow_retriever is not None
                and getattr(ragflow_retriever, "enabled", False)
            ),
            normalized_query[:120],
        )
        return [], "none", ""
    ...
    use_exact_mode = (
        is_exact_answer_query(normalized_query)
        if exact_mode is None
        else bool(exact_mode)
    )
    requested_top_k = int(top_k or RAGFLOW_CONFIG.get("top_k", 12))
    if use_exact_mode:
        requested_top_k = max(
            requested_top_k,
            int(os.getenv("DURIAN_RAG_EXACT_TOP_K", "16")),
        )

    candidate_top_k = max(
        requested_top_k,
        int(RAGFLOW_CONFIG.get("candidate_top_k", 16) or 16),
    )
    # Keep retrieval domain-scoped even when the user omitted the word
    # “榴莲” or only used a pronoun/short identifier.
    retrieval_query = scope_web_query_to_durian(normalized_query)
    validation_query = relevance_query or normalized_query
    cache_key = ""
    cache_ttl = max(0.0, float(RAGFLOW_CONFIG.get("cache_ttl_sec", 0) or 0))
    if cache_ttl > 0:
        raw_key = json.dumps(
            {
                "q": retrieval_query,
                "rel": str(relevance_query or ""),
                "exact": bool(use_exact_mode),
        ...
        cache_key = hashlib.sha1(raw_key.encode("utf-8", errors="ignore")).hexdigest()
        now = time.time()
        with RAGFLOW_CONTEXT_CACHE_LOCK:
            cached = _RAGFLOW_CONTEXT_CACHE.get(cache_key)
            if cached and now - cached[0] <= cache_ttl:
                logger.info(
                    "RAGFlow grounded context cache hit: exact=%s top_k=%d query=%s",
                    use_exact_mode,
                    requested_top_k,
                    normalized_query[:120],
                )
                return [dict(item) for item in cached[1]], cached[2], cached[3]
            if cached:
                _RAGFLOW_CONTEXT_CACHE.pop(cache_key, None)

    def remember_ragflow_context_cache(
        cached_evidence: List[Dict[str, Any]],
        cached_quality: str,
        cached_text: str,
    ) -> None:
        if not cache_key or cache_ttl <= 0:
            return
        now = time.time()
        max_entries = max(1, int(RAGFLOW_CONFIG.get("cache_max_entries", 512) or 512))
        with RAGFLOW_CONTEXT_CACHE_LOCK:
            _RAGFLOW_CONTEXT_CACHE[cache_key] = (
                now,
                [dict(item) for item in cached_evidence],
                cached_quality,
                cached_text,
            )
            if len(_RAGFLOW_CONTEXT_CACHE) > max_entries:
                oldest_key = min(
                    _RAGFLOW_CONTEXT_CACHE,
                    key=lambda key: _RAGFLOW_CONTEXT_CACHE[key][0],
                )
                _RAGFLOW_CONTEXT_CACHE.pop(oldest_key, None)

    evidence, quality = ragflow_retriever.retrieve(
        retrieval_query,
        top_k=requested_top_k,
        candidate_top_k=candidate_top_k,
        exact_mode=use_exact_mode,
        timeout_sec=timeout_sec,
    )
    raw_evidence_count = len(evidence or [])
    evidence = apply_query_intent_evidence_guard(
        normalized_query,
        evidence,
    )
    if use_exact_mode:
        evidence = [
            item
            for item in evidence
            if extract_exact_fill_answer(normalized_query, [item])
            or extract_durian_code_lookup_answer(
                normalized_query,
                [item],
                allow_fallback=False,
            )
            or extract_exact_fact_answer(normalized_query, [item])
        ]
        quality = "high" if evidence else "none"
    elif quality == "low":
        logger.info(
            "RAGFlow evidence is low quality; keeping it as optional context: query=%s",
            normalized_query[:160],
        )

    if not evidence:
        logger.info(
            "RAGFlow-only evidence gate returned empty: exact=%s query=%s",
            use_exact_mode,
            normalized_query[:160],
        )
        if raw_evidence_count > 0:
            remember_ragflow_context_cache([], "none", "")
        return [], "none", ""
    if not use_exact_mode and not ragflow_evidence_matches_query_anchors(
        relevance_query or normalized_query,
        evidence,
    ):
        logger.info(
            "RAGFlow-only evidence rejected by query anchor gate: query=%s",
            str(relevance_query or normalized_query)[:160],
        )
        remember_ragflow_context_cache([], "none", "")
        return [], "none", ""

    if not use_exact_mode:
        evidence, quality = validate_rag_evidence(
            validation_query,
            evidence,
            quality or "low",
            source="ragflow_pre_rerank",
        )
        if not evidence:
            remember_ragflow_context_cache([], "none", "")
            return [], "none", ""
        evidence = focus_evidence_on_query_topics(
            validation_query,
            evidence,
        )
    pre_rerank_evidence = list(evidence)
    pre_rerank_quality = quality or "low"
    ...
    if not evidence and not use_exact_mode and pre_rerank_evidence:
        # Reranker ordering must not erase candidates that already passed the
        # lexical/topic/anchor gates. Keep the validated pre-rerank window and
        # let the answer model use those sources instead of forcing web search.
        evidence = pre_rerank_evidence[:requested_top_k]
        quality = pre_rerank_quality
        logger.info(
            "RAG rerank validation fallback: restored=%d quality=%s query=%s",
            len(evidence),
            quality,
            normalized_query[:120],
        )
    ...
    result_quality = quality or "low"
    evidence_text = build_evidence_text(
        evidence,
        max_chars=int(RAG_CONFIG.get("max_evidence_chars", 900)),
    )
    remember_ragflow_context_cache(evidence, result_quality, evidence_text)

    return evidence, result_quality, evidence_text


class DurianLangGraphAdapter:
    """Connect the generic LangGraph runtime to the existing Durian GPT stack."""

    @staticmethod
    def _domain_messages(payloads: List[Dict[str, Any]]) -> List[Message]:
        return [
            Message(
    ...
    def resolve_language(self, query: str, fallback: str) -> str:
        return resolve_response_language(fallback, query)

    def has_image_context(self, payloads: List[Dict[str, Any]]) -> bool:
        return has_recent_image_context(self._domain_messages(payloads))

    def is_casual_query(self, query: str) -> bool:
        return classify_casual_message(query) is not None

    def casual_reply(self, query: str, response_language: str) -> str:
        return globals()["casual_reply"](query, response_language)
    ...
    def route_context(
        ...
        return route_context_with_llm(
            query,
            self._domain_messages(payloads),
            response_language,
        )
    ...
    def enhance_query(self, query: str, response_language: str) -> str:
        return enhance_retrieval_query(query, response_language)

    def is_exact_query(self, query: str) -> bool:
        return is_exact_answer_query(query)

    def allow_exact_generation_fallback(self, query: str) -> bool:
        # Cultivar-code lookups should still be written by the model: retrieval
        # and deterministic extraction provide high-confidence facts, while the
        # model judges wording and context. Fill-in/strict copy tasks remain
        # deterministic because generation can change required wording.
        query_text = str(query or "")
        return (
            bool(_DURIAN_CODE_RE.search(query_text))
            and not bool(_EXACT_FILL_BLANK_RE.search(query_text))
        )

    def is_contextual_exact_query(
        self,
        query: str,
        payloads: List[Dict[str, Any]],
    ) -> bool:
        """Allow an exact question to inherit only an explicit image subject.

        A self-contained fact query such as "黑刺的编码是什么" must remain
        isolated from stale history. A pronoun-based query immediately after
        image analysis, such as "它是什么编号", needs the image card before
        exact RAG can retrieve the correct source row.
        """
        if not is_exact_answer_query(query):
            return False
        messages = self._domain_messages(payloads)
        if classify_user_intent(
            query,
            has_recent_image=has_recent_image_context(messages),
        ) != "explicit_follow_up":
            return False
        for message in reversed(messages[:-1] if messages else []):
            if message.role != "assistant":
                continue
            card = message.active_context_card
            if not isinstance(card, dict) or not card:
                return False
            source = str(card.get("source") or "").lower()
            return bool(card.get("image_url") or source.startswith("image"))
        return False
    ...
    def extract_exact_answer(
        ...
        exact_answer = (
            extract_exact_fill_answer(query, evidence)
            or extract_registered_durian_answer(
                query,
                evidence,
                response_language,
            )
            ...
            or extract_exact_fact_answer(query, evidence)
        )
        return exact_answer or extract_partial_exact_answer(
            query,
            evidence,
            response_language,
        )

    def exact_not_found(self, response_language: str) -> str:
        return exact_answer_not_found(response_language)

    def extract_grounded_followup_answer(
        self,
        evidence: List[Dict[str, Any]],
        response_language: str,
    ) -> Optional[str]:
        return format_registered_durian_details(evidence, response_language)

    def grounded_not_found(self, response_language: str) -> str:
        return grounded_answer_not_found(response_language)

    def soft_no_source_answer(
        self,
        query: str,
        response_language: str,
    ) -> Optional[str]:
        """Answer common judgment questions safely without invented details."""
        value = unicodedata.normalize("NFKC", str(query or "")).strip()
        lower = value.lower()
        language = str(response_language or "zh").lower()

        image_sufficiency = bool(
            any(term in lower for term in ("照片", "图片", "影像", "photo", "image"))
            and any(term in lower for term in ("诊断", "判断", "识别", "diagnos", "identify"))
            and any(term in lower for term in ("足以", "准确", "所有", "仅凭", "只凭", "enough", "accurate", "all"))
        )
        if image_sufficiency:
            if language == "en":
                return "No. A single image can narrow down visible possibilities, but it cannot reliably diagnose every disease. Also check symptom distribution across the whole plant, both sides of leaves, trunk/collar, roots and fruit, how symptoms changed over time, recent weather and management; some cases require field inspection or laboratory confirmation."
            if language == "ms":
                return "Tidak. Satu gambar hanya boleh mengecilkan kemungkinan berdasarkan tanda yang kelihatan, tetapi tidak boleh mendiagnosis semua penyakit dengan tepat. Perlu juga menyemak taburan simptom pada seluruh pokok, kedua-dua permukaan daun, batang/pangkal, akar dan buah, perkembangan simptom, cuaca serta rekod pengurusan; sesetengah kes memerlukan pemeriksaan lapangan atau makmal."
            if language == "th":
                return "ไม่เพียงพอ ภาพเดียวช่วยจำกัดความเป็นไปได้จากอาการที่มองเห็นได้ แต่ไม่สามารถวินิจฉัยโรคทั้งหมดได้อย่างแม่นยำ ควรตรวจการกระจายอาการทั้งต้น ใบทั้งสองด้าน ลำต้น โคน รากและผล ลำดับการเกิดอาการ สภาพอากาศและการจัดการ และบางกรณีต้องยืนยันภาคสนามหรือห้องปฏิบัติการ"
            return "不足。一张照片只能根据可见特征缩小排查范围，不能准确诊断所有病害。还应检查整株症状分布、叶片正反面、根颈与树干、根系和果实，记录症状发展时间、近期天气及管理变化；必要时还需要现场检查或实验室检测。"

        single_feature_inference = bool(
            any(term in lower for term in ("是否代表", "是否意味着", "能否说明", "是不是说明", "does it mean", "indicate"))
        )
        if single_feature_inference:
            if "生命线" in lower and "树龄" in lower:
                return (
                    "不一定。如果这里的“生命线”指果壳上的分瓣线或缝合线，它的深浅属于果实外观特征，"
                    "不能单独用来推断树龄；还可能受到品种、成熟度、生长条件和果实个体差异影响。"
                    "判断树龄应优先依据种植记录，并结合树干粗度、树冠结构和历年结果情况。"
                )
            return "不一定。单一外观或症状特征通常不足以可靠推断另一项属性，还需要结合对象定义、时间变化、环境条件和其他独立指标交叉判断。"
        return None

    def extract_grounded_diagnosis_answer(
        self,
        query: str,
        evidence: List[Dict[str, Any]],
        response_language: str,
    ) -> Optional[str]:
        return format_grounded_symptom_diagnosis(
            query,
            evidence,
            response_language,
        )
    ...
    def rag_available(self) -> bool:
        ...
        web_available = bool(WEB_SEARCH_CONFIG["enabled"] and WEB_SEARCH_CONFIG["api_key"])
        return local_available or web_available
    ...
    def retrieve(
        ...
        """Run local hybrid retrieval, then a targeted entity web pass.

        Ordinary questions keep the existing local-first behavior. Compact
        identifiers get a second, exact-lexical web pass even when local RAG
        returned something, because a broad local hit must not hide a better
        registration row or identifier definition.
        """
        normalized_query = str(query or "").strip()

        def _local_retrieve() -> Tuple[List[Dict[str, Any]], str, str]:
            local_evidence, local_quality, local_text = retrieve_comparison_grounded_context(
                normalized_query,
                response_language,
            )
            if not local_evidence:
                local_evidence, local_quality, local_text = retrieve_ragflow_grounded_context(
                    normalized_query,
                    response_language,
                )
            return local_evidence, local_quality, local_text

        is_code_entity_query = bool(
            is_exact_fact_query(normalized_query)
            and (
                _DURIAN_CODE_RE.search(normalized_query)
                or extract_identifier_lookup_subject(normalized_query)
            )
        )
        is_short_entity = is_short_entity_query(normalized_query)
        entity_web_attempted = False
        entity_web_evidence: List[Dict[str, Any]] = []
        entity_web_quality = "none"
        should_parallel_entity_web = bool(
            WEB_SEARCH_CONFIG["enabled"]
            and WEB_SEARCH_CONFIG["api_key"]
            and (
                is_code_entity_query
                or (is_short_entity and env_bool("DURIAN_SHORT_ENTITY_WEB_ENABLED", True))
            )
        )

        if should_parallel_entity_web:
            entity_web_attempted = True
            started_parallel = time.perf_counter()
            executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="rag-entity-parallel")
            try:
                local_future = executor.submit(_local_retrieve)
                if is_code_entity_query:
                    web_future = executor.submit(
                        retrieve_durian_code_web_evidence,
                        normalized_query,
                        response_language,
                    )
                else:
                    web_future = executor.submit(
                        retrieve_short_entity_web_evidence,
                        normalized_query,
                        response_language,
                    )
                try:
                    entity_web_evidence, entity_web_quality, _ = web_future.result()
                except Exception as exc:
                    logger.warning("Parallel entity web retrieval failed: query=%s error=%s", normalized_query[:120], exc)
                    entity_web_evidence, entity_web_quality = [], "none"

                local_wait_sec = max(0.0, float(os.getenv("DURIAN_CODE_LOCAL_RAG_WAIT_SEC", "2.0") or 0.0))
                try:
                    if entity_web_evidence and local_wait_sec > 0:
                        evidence, quality, evidence_text = local_future.result(timeout=local_wait_sec)
                    elif entity_web_evidence:
                        evidence, quality, evidence_text = [], "none", ""
                    else:
                        evidence, quality, evidence_text = local_future.result()
                except FutureTimeoutError:
                    logger.info(
                        "Parallel local retrieval still cold after %.2fs; proceeding with entity web evidence: query=%s",
                        local_wait_sec,
                        normalized_query[:120],
                    )
                    evidence, quality, evidence_text = [], "none", ""
                except Exception as exc:
                    logger.warning("Parallel local retrieval failed: query=%s error=%s", normalized_query[:120], exc)
                    evidence, quality, evidence_text = [], "none", ""
            finally:
                executor.shutdown(wait=False, cancel_futures=False)
            logger.info(
                "Parallel entity retrieval timing: total=%.3fs local=%d web=%d query=%s",
                time.perf_counter() - started_parallel,
                len(evidence or []),
                len(entity_web_evidence or []),
                normalized_query[:120],
            )
        else:
            evidence, quality, evidence_text = _local_retrieve()

        if entity_web_evidence:
            local_count = len(evidence or [])
            evidence = merge_entity_evidence(
                normalized_query,
                evidence,
                entity_web_evidence,
                max_items=8,
            )
            quality = "high" if entity_web_quality == "high" else (quality or "medium")
            evidence_text = build_evidence_text(
                evidence,
                max_chars=RAG_CONFIG.get("max_evidence_chars", 700),
            )
            logger.info(
                "Entity web verification merged: local=%d web=%d final=%d quality=%s query=%s",
                local_count,
                len(entity_web_evidence),
                len(evidence),
                quality,
                normalized_query[:120],
            )

        if not entity_web_attempted and is_exact_fact_query(normalized_query) and not evidence:
            evidence, quality, evidence_text = retrieve_durian_code_web_evidence(
                normalized_query,
                response_language,
            )

        if should_use_tavily_web_search(normalized_query, evidence, quality) and not entity_web_attempted:
            web_evidence, _web_quality, _web_evidence_text = tavily_web_search(
                build_durian_code_web_query(normalized_query)
                if is_exact_fact_query(normalized_query)
                else normalized_query,
                response_language,
                max_results=WEB_SEARCH_CONFIG["top_k"],
            )
            if is_exact_fact_query(normalized_query):
                web_evidence = [
                    item
                    ...
                    if (
                        extract_durian_code_lookup_answer(
                            normalized_query,
                            [item],
                            response_language,
                            allow_fallback=False,
                        )
                        or extract_exact_fact_answer(
                            normalized_query,
                            [item],
                        )
                    )
            ...
            if web_evidence:
                ...
                evidence = merge_web_evidence(evidence, web_evidence, max_items=8)
                evidence, quality = validate_rag_evidence(
                    normalized_query,
                    evidence,
                    "high" if is_exact_fact_query(normalized_query) else "medium",
                    top_k=8,
                    source="langgraph_web_merge",
                )
                evidence_text = build_evidence_text(
                    evidence,
                    max_chars=RAG_CONFIG.get("max_evidence_chars", 700),
                ...
                logger.info(
                    "LangGraph web fallback merged: local=%d web=%d final=%d quality=%s",
                    local_count,
                    len(web_evidence),
                    len(evidence),
                    quality,
                )
        return evidence, quality, evidence_text
    ...
    def prepare_evidence(
        ...
        prepared = prepare_evidence_for_client(evidence, response_language)
        return filter_metadata_evidence_for_client(prepared)

    def greeting_reply(self, response_language: str) -> str:
        return globals()["greeting_reply"](response_language)
    ...
    def stream_generate(
        ...
        if not is_model_ready():
            raise RuntimeError("模型未加载")
        return collect_model_stream(
            model_instance.generate_stream,
            messages,
            max_new_tokens=int(generation.get("max_tokens") or INFERENCE_CONFIG["max_new_tokens"]),
            temperature=float(generation.get("temperature", 0.3)),
            top_p=float(generation.get("top_p", 0.85)),
            top_k=int(generation.get("top_k", 40)),
            repetition_penalty=float(generation.get("repetition_penalty", 1.08)),
            response_language=response_language,
        )
    ...
    def build_context_card(
        ...
        *,
        ...
        return build_text_context_card(
            query,
            response,
            response_language,
            base_card=base_card,
    ...
    def log_route(self, **values):
        route = values.get("route") or {}
        logger.info(
            "LangGraph route: intent=%s history=%s source=%s history_count=%s query=%s resolved_query=%s rag_query=%s",
            route.get("intent"),
            route.get("use_history"),
            route.get("source"),
            values.get("history_count"),
            str(values.get("query") or "")[:100],
            str(values.get("resolved_query") or "")[:180],
            str(values.get("rag_query") or "")[:180],
        )


model_instance: Optional[DurianGPTModel] = None
rag_retriever: Optional[RAGRetriever] = None
ragflow_retriever: Optional[RAGFlowRetriever] = RAGFlowRetriever(RAGFLOW_CONFIG)
pest_classifier: Optional[PestClassifier] = None
qwen_vl_max_client: Optional[QwenVLMaxClient] = None
langgraph_runtime: Optional[Any] = None
...
def rebuild_rag_retriever(force_auto_sync: bool = False) -> Dict[str, Any]:
    ...
    with RAG_REBUILD_LOCK:
        # Avoid replacing the global retriever while a request is using it.
        with RAG_QUERY_LOCK:
            ...
            retriever_config = dict(RAG_CONFIG)
            retriever_config["force_rebuild_index"] = bool(force_auto_sync)

            new_retriever = RAGRetriever(retriever_config)
            rag_retriever = new_retriever
...
async def startup_event():
    global model_instance, rag_retriever, pest_classifier, qwen_vl_max_client, langgraph_runtime

    try:
        model_instance = DurianGPTModel(MODEL_CONFIG)
        model_instance.load_model()

        # Warm up LlamaIndex RAG in the backend process.
        # This avoids the first real stream2 request paying the 15-20s cold-start cost.
        try:
            async def _warmup_llamaindex_rag():
                try:
                    warmup_started = time.perf_counter()
                    await asyncio.to_thread(
                        llamaindex_rag_retrieve,
                        "榴莲品种",
                        4,
                    )
                    logger.info(
                        "LlamaIndex RAG warmup completed in %.3fs",
                        time.perf_counter() - warmup_started,
                    )
                except Exception as warmup_exc:
                    logger.warning("LlamaIndex RAG warmup failed: %s", warmup_exc)

            asyncio.create_task(_warmup_llamaindex_rag())
            logger.info("LlamaIndex RAG warmup task scheduled")
        except Exception as warmup_schedule_exc:
            logger.warning("LlamaIndex RAG warmup schedule failed: %s", warmup_schedule_exc)
        ...
        if LANGGRAPH_AVAILABLE and DurianLangGraphRuntime is not None:
            try:
                langgraph_runtime = DurianLangGraphRuntime(
                    adapter=DurianLangGraphAdapter(),
                    checkpoint_path=str(BASE_DIR / "durian_langgraph_checkpoints.sqlite"),
                    user_memory_path=str(BASE_DIR / "durian_user_memories.sqlite"),
                    max_memory_messages=int(os.getenv("DURIAN_LANGGRAPH_MEMORY_MESSAGES", "12")),
                    max_prompt_messages=int(os.getenv("DURIAN_LANGGRAPH_PROMPT_MESSAGES", "8")),
                    max_topic_memories=int(os.getenv("DURIAN_LANGGRAPH_TOPIC_MEMORIES", "10")),
                )
                logger.info("✓ LangGraph 会话编排与持久化记忆已初始化")
            except Exception:
                langgraph_runtime = None
                logger.exception("LangGraph 初始化失败，stream3 将不可用")
        else:
            logger.warning("LangGraph 依赖不可用，stream3 将不可用")
...
async def shutdown_event():
    global model_instance, langgraph_runtime

    if langgraph_runtime is not None:
        try:
            langgraph_runtime.close()
        except Exception:
            logger.exception("关闭 LangGraph runtime 失败")
        finally:
            langgraph_runtime = None
...
@app.post(
    ...
    tags=["RAG"],
    dependencies=[Depends(verify_internal_embedding_api_key)],
)
async def openai_compatible_embeddings(request: OpenAIEmbeddingRequest):
    """Internal OpenAI-compatible embedding endpoint for RAGFlow and Dify."""
    ...
    if not texts or len(texts) > 64:
        raise HTTPException(
            status_code=400,
            detail="input must contain between 1 and 64 texts",
        )
    normalized = [str(text or "").strip() for text in texts]
    if any(not text for text in normalized):
        raise HTTPException(
            status_code=400,
            detail="embedding input cannot be empty",
        )
    if sum(len(text) for text in normalized) > 120000:
        raise HTTPException(
            status_code=413,
            detail="embedding input is too large",
        )
    if (
        rag_retriever is None
        or not getattr(rag_retriever, "enabled", False)
        or getattr(rag_retriever, "model", None) is None
    ):
        raise HTTPException(
            status_code=503,
    ...
    vectors = await asyncio.to_thread(rag_retriever._encode, normalized, 32)
    if vectors.size == 0 or len(vectors) != len(normalized):
        raise HTTPException(
            status_code=503,
            detail="embedding generation failed",
        )
    prompt_tokens = sum(approx_token_count(text) for text in normalized)
    return OpenAIEmbeddingResponse(
        data=[
            OpenAIEmbeddingData(index=index, embedding=vector.tolist())
            for index, vector in enumerate(vectors)
        ],
        model=request.model or "durian-e5",
        usage={
...
async def rag_upload_pdf(file: UploadFile = File(...)):
    ...
    try:
        ...
        # If indexing fails, remove the just-uploaded PDF so it does not poison
        # future startup/rebuild attempts.
        try:
            target_path.unlink(missing_ok=True)
        except Exception:
            logger.warning("Failed to remove uploaded PDF after rebuild exception: %s", target_path)
        raise HTTPException(status_code=500, detail=f"RAG rebuild failed after upload: {exc}")

    if status.get("error"):
        try:
            target_path.unlink(missing_ok=True)
        except Exception:
            logger.warning("Failed to remove uploaded PDF after rebuild error: %s", target_path)
        raise HTTPException(status_code=500, detail=status["error"])
...
def run_with_model_lock(fn, *args, **kwargs):
    """Serialize only in local-vLLM mode; proxy mode can use vLLM server batching."""
    if is_openai_proxy_mode():
...
def collect_model_stream(fn, *args, **kwargs):
    """Consume the model stream while preserving proxy-mode concurrency.

    OpenAI proxy mode forwards to an independent vLLM server, so holding the
    FastAPI process lock would prevent vLLM from batching concurrent requests.
    """
    if is_openai_proxy_mode():
...
async def infer(request: InferenceRequest):
    ...
    try:
        ...
        response_language = infer_response_language_from_user_query(user_query, request.response_language)
...
async def chat(request: InferenceRequest):
    ...
    try:
        ...
        is_code_entity_query = bool(
            _DURIAN_CODE_RE.search(user_query)
            and is_exact_fact_query(user_query)
        )
        code_web_allowed = bool(
            is_code_entity_query
            and request.use_rag
            and WEB_SEARCH_CONFIG["enabled"]
            and WEB_SEARCH_CONFIG["api_key"]
        )

        if rag_allowed and code_web_allowed:
            rag_task = asyncio.create_task(asyncio.to_thread(llamaindex_rag_retrieve, rag_query))
            web_task = asyncio.create_task(asyncio.to_thread(retrieve_durian_code_web_evidence, user_query, response_language))
            try:
                web_evidence, web_quality, _ = await web_task
            except Exception as exc:
                logger.warning("chat code web evidence failed during parallel lookup: query=%s error=%s", user_query[:120], exc)
                web_evidence, web_quality = [], "none"

            local_wait_sec = max(0.0, float(os.getenv("DURIAN_CODE_LOCAL_RAG_WAIT_SEC", "2.0") or 0.0))
            try:
                if web_evidence and local_wait_sec > 0:
                    evidence, evidence_quality = await asyncio.wait_for(
                        asyncio.shield(rag_task),
                        timeout=local_wait_sec,
                    )
                elif web_evidence:
                    evidence, evidence_quality = [], "none"
                else:
                    evidence, evidence_quality = await rag_task
            except asyncio.TimeoutError:
                logger.info(
                    "chat local RAG still cold after %.2fs; proceeding with code web evidence: query=%s",
                    local_wait_sec,
                    user_query[:120],
                )
                evidence, evidence_quality = [], "none"
            except Exception as exc:
                logger.warning("chat local RAG failed during parallel code lookup: query=%s error=%s", user_query[:120], exc)
                evidence, evidence_quality = [], "none"
            evidence_text = build_evidence_text(
                evidence,
                max_chars=RAG_CONFIG.get("max_evidence_chars", 700),
            ...
            if web_evidence:
                ...
                evidence_quality = "high" if web_quality == "high" else (evidence_quality or "medium")
                evidence_text = build_evidence_text(
                    evidence,
                    max_chars=max(int(RAG_CONFIG.get("max_evidence_chars", 700)), 1400),
                ) if evidence else ""
                logger.info(
                    "chat code entity parallel evidence merged: local=%d web=%d final=%d quality=%s",
                    local_count,
                    len(web_evidence),
                    len(evidence),
                    evidence_quality,
            ...
            if rag_allowed:
                evidence, evidence_quality = await asyncio.to_thread(llamaindex_rag_retrieve, rag_query)
                evidence_text = build_evidence_text(
                    evidence,
                    max_chars=RAG_CONFIG.get("max_evidence_chars", 700),
                )
            if code_web_allowed:
                web_evidence, web_quality, _ = await asyncio.to_thread(
                    retrieve_durian_code_web_evidence,
                    user_query,
                    response_language,
                )
                if web_evidence:
                    ...
                    logger.info(
                        "chat code entity web evidence merged: local=%d web=%d final=%d quality=%s",
                        local_count,
                        len(web_evidence),
                        len(evidence),
                        evidence_quality,
        ...
        if (not code_web_allowed) and should_use_tavily_web_search(rag_query or user_query, evidence, evidence_quality):
            web_evidence, _web_quality, _web_evidence_text = await asyncio.to_thread(
                tavily_web_search,
                rag_query or user_query,
                response_language,
                max_results=WEB_SEARCH_CONFIG["top_k"],
            )
            if web_evidence:
                ...
                logger.info(
                    "chat web fallback merged: local=%d web=%d final=%d quality=%s",
                    local_count,
                    len(web_evidence),
                    len(evidence),
                    evidence_quality,
...
async def chat_stream(request: InferenceRequest):
    ...
    try:
        ...
        if should_use_tavily_web_search(rag_query or user_query, evidence, evidence_quality):
            web_evidence, _web_quality, _web_evidence_text = await asyncio.to_thread(
                tavily_web_search,
                rag_query or user_query,
                response_language,
            ...
            if web_evidence:
                ...
                logger.info(
                    "chat stream web fallback merged: local=%d web=%d final=%d quality=%s",
                    local_count,
                    len(web_evidence),
                    len(evidence),
                    evidence_quality,
...
async def delete_conversation(conv_id: str, current_user: str = Depends(get_current_user)):
    ...
    if langgraph_runtime is not None:
        try:
            langgraph_runtime.delete_thread(f"{current_user}:{conv_id}")
        except Exception:
            logger.exception("删除 LangGraph thread 失败: user=%s conv_id=%s", current_user, conv_id)
    return {"status": "success"}
...
# ============================================================
# Stable OpenAI-proxy streaming endpoint
# Bypasses legacy /chat/stream proxy path.
# Frontend should call /api/chat/stream2 through Caddy.
# ============================================================
try:
    import httpx as _durian_httpx
except Exception:
    _durian_httpx = None


def _rag_expansion_term_language(term: str) -> str:
    ...
    if re.search(r"[\u4e00-\u9fff]", value):
        return "zh"
    ms_terms = {
        "varieti", "daun kuning", "akar reput", "saliran", "tanah", "baja",
        "penyakit", "perosak", "berbunga", "buah gugur", "kematangan",
        "simpan", "pilih",
    }
    if lower in ms_terms:
        return "ms"
    if re.search(r"[a-zA-Z]", value):
        return "en"
    return "unknown"
...
def _rag_expansion_term_allowed(term: str, response_language: str) -> bool:
    mode = os.getenv("DURIAN_RAG_CROSS_LANGUAGE_MODE", "same_or_english").strip().lower()
    if mode in {"all", "legacy", "multilingual"}:
        return True

    target = str(response_language or "zh").strip().lower()
    if target not in {"zh", "en", "ms", "th"}:
        target = "zh"

    term_lang = _rag_expansion_term_language(term)
    return term_lang in {"unknown", target, "en"}


def expand_multilingual_rag_query(query: str, response_language: str = "zh") -> str:
    """Add conservative same-language/English terms for FAISS recall."""
    ...
    groups = [
        ["榴莲", "durian", "ทุเรียน"],
        ["品种", "品类", "variety", "cultivar", "พันธุ์", "varieti"],
        ["猫山王", "musang king", "d197"],
        ["黑刺", "black thorn", "d200"],
        ["金枕", "monthong", "หมอนทอง"],
        ["黄叶", "叶黄", "yellow leaves", "daun kuning", "ใบเหลือง"],
        ["根腐", "烂根", "root rot", "akar reput", "รากเน่า"],
        ["积水", "排水", "waterlogging", "drainage", "saliran", "น้ำขัง", "ระบายน้ำ"],
        ["土壤", "土质", "soil", "tanah", "ดิน"],
        ["施肥", "肥料", "fertilizer", "baja", "ปุ๋ย"],
        ["病虫害", "病害", "虫害", "pest", "disease", "penyakit", "perosak", "โรค", "แมลง"],
        ["炭疽", "anthracnose", "antraknos"],
        ["开花", "flowering", "berbunga", "ออกดอก"],
        ["落果", "fruit drop", "buah gugur", "ผลร่วง"],
        ["成熟", "熟度", "ripeness", "maturity", "kematangan", "ความสุก"],
        ["保存", "储存", "storage", "simpan", "เก็บรักษา"],
        ["购买", "挑选", "selection", "buying", "pilih", "เลือกซื้อ"],
    ]

    additions: List[str] = []
    for terms in groups:
        if any(term.lower() in lower for term in terms):
            additions.extend([term for term in terms if _rag_expansion_term_allowed(term, response_language)])

    # Every substantive local retrieval stays inside the durian domain.
    # The user's wording is preserved; these terms only guide recall.
    if not any(term in lower for term in ["榴莲", "durian", "durio", "ทุเรียน"]):
        additions.extend([
            term
            for term in ["榴莲", "durian"]
            if _rag_expansion_term_allowed(term, response_language)
        ])
    ...
    logger.info("RAG query expanded: lang=%s original=%s expanded_terms=%s", response_language, base[:80], unique_terms[:20])
    return expanded
...
def force_clean_boundary_evidence(query: str, evidence: List[Dict]) -> List[Dict]:
    """Force-remove low-value section-boundary chunks before client metadata/prompt.

    Example bad chunk:
    "40% 乙磷铝... 二、榴莲炭疽病"
    It contains the title of the anthracnose section, but the actual content belongs
    to the previous disease/treatment paragraph.
    """
    if not evidence:
        return evidence

    q = str(query or "").lower()
    if not any(x in q for x in ["炭疽", "anthracnose", "colletotrichum"]):
        return evidence
    ...
    def is_good_anthracnose_detail(item: Dict) -> bool:
        t = text_of(item)
        return any(x in t for x in [
            "叶片病斑", "病部产生", "浅褐色小病斑",
            "病原为", "胶孢炭疽菌", "colletotrichum",
            "分生孢子", "孢子盘", "高温、高湿", "阴雨多"
        ])

    has_good_raw = any(
        str(item.get("source_file") or "").lower() == "ocr_texts.json"
        and is_good_anthracnose_detail(item)
        for item in evidence
    )
    ...
    for item in evidence:
        t = text_of(item)
        item_index = str(item.get("item_index") or "")

        boundary_like = (
            item_index == "11485"
            or (
                "二、榴莲炭疽病" in t
                and not is_good_anthracnose_detail(item)
            )
            or (
                "乙磷铝" in t
                and "二、榴莲炭疽病" in t
    ...
    if dropped:
        logger.info(
            "Force boundary evidence cleanup: dropped=%d kept=%d query=%s",
            dropped,
            len(cleaned),
            str(query)[:100].replace("\\n", " "),
        )
...
def is_bad_client_evidence(item: Dict[str, Any]) -> bool:
    """Filter evidence that should not be shown in the right-side reference panel.

    This does not necessarily mean the chunk cannot exist in the index.
    It only prevents low-value / unrelated / parser-noise chunks from being
    displayed as trusted references.
    """
    if not isinstance(item, dict):
        return True
    ...
    # OCR parser / MinerU structure leakage.
    parser_markers = [
        "'bbox'", '"bbox"', "bbox:",
        "'spans'", '"spans"',
        "'lines'", '"lines"',
    ...
    marker_hits = sum(1 for m in parser_markers if m in lower)
    if marker_hits >= 2:
        return True

    # Book front matter / author responsibility / bibliography-like chunks.
    bad_phrases = [
        "本书由",
        "主要负责",
        "参与撰写",
    ...
    if any(x in compact for x in bad_phrases):
        # If it is only source description, not agronomic content, hide it.
        useful_terms = [
            "D24", "D197", "D200", "D175", "D160",
            "猫山王", "黑刺", "金枕", "红虾", "苏丹王",
            "症状", "病原", "防治", "施肥", "授粉", "坐果",
            "炭疽", "疫病", "根腐", "叶斑"
        ]
        if not any(t.lower() in lower for t in useful_terms):
            return True

    # Very generic source-introduction chunk, not evidence.
    ...
    if any(x in compact for x in source_intro_markers):
        return True
...
def filter_metadata_evidence_for_client(evidence: List[Dict]) -> List[Dict]:
    """Final hard filter before sending metadata evidence to frontend."""
    if not evidence:
        return []
    ...
    for item in evidence:
        ...
        if sum(1 for m in parser_markers if m in lower) >= 2:
            bad = True

        # Book preface / author responsibility / front matter
        front_matter = [
            "本书由",
            "主编",
            "副研究员",
        ...
        if any(x in compact for x in front_matter):
            bad = True
    ...
    if dropped:
        logger.info(
            "Metadata evidence hard filter dropped=%d kept=%d",
            dropped,
            len(cleaned),
        )
...
def _client_evidence_text_limit() -> int:
    try:
        return max(240, int(os.getenv("DURIAN_CLIENT_EVIDENCE_TEXT_MAX_CHARS", "900")))
    except Exception:
        return 900


def _clip_client_evidence_text(value: Any, limit: Optional[int] = None) -> str:
    text = re.sub(r"\s+", " ", str(value or "")).strip()
    max_chars = limit if isinstance(limit, int) and limit > 0 else _client_evidence_text_limit()
    if len(text) <= max_chars:
        return text
    return text[:max_chars].rstrip() + "..."


def _slim_client_evidence_item(item: Dict[str, Any]) -> Dict[str, Any]:
    text_limit = _client_evidence_text_limit()
    meta_limit = 260
    slim: Dict[str, Any] = {}
    for key in (
        "doc",
        ...
        value = item.get(key)
        if isinstance(value, str):
            if key in {"url", "source", "link"}:
                value = normalize_reference_url(value)
            elif key == "doc":
                value = normalize_reference_label(value)
            slim[key] = _clip_client_evidence_text(value, meta_limit)
    ...
    display = translated or item.get("display_text") or item.get("text") or item.get("original_text") or item.get("source_answer_text")
    if translated:
        slim["translated_text"] = _clip_client_evidence_text(translated, text_limit)
        slim["display_text"] = slim["translated_text"]
    elif display:
        slim["display_text"] = _clip_client_evidence_text(display, text_limit)
    else:
        fallback = rag_display_text(item)
        if fallback:
            slim["display_text"] = _clip_client_evidence_text(fallback, text_limit)
    return slim


def prepare_evidence_for_client(
    evidence: List[Dict],
    target_language: str = "zh",
    include_cross_language_raw: bool = False,
) -> List[Dict]:
    if not evidence:
        return []

    target = str(target_language or "zh").lower()
    if target not in {"zh", "en", "ms", "th"}:
        target = "zh"

    client_evidence = []
    for item in evidence:
        if is_weak_synthetic_qa_chunk(item):
            continue
        cloned = normalize_reference_item(dict(item))
        original_text = str(cloned.get("original_text") or _rag_chunk_text(cloned) or "").strip()
        text = rag_display_text(cloned)
        if original_text and text != original_text:
            cloned["original_text"] = original_text
            cloned["source_answer_text"] = original_text
        else:
            cloned.setdefault("original_text", original_text or text)
        cloned["text"] = text
        cloned["index_text"] = rag_index_text(cloned)
        cloned["canonical_language"] = _rag_canonical_language()
        lang = detect_rag_text_language(text)
        cloned["text_language"] = lang
        cloned["display_language"] = lang
        cloned["target_language"] = target

        if include_cross_language_raw or lang == target or lang == "unknown":
            cloned.setdefault("display_text", text)
        else:
            cloned["display_text"] = ""
            cloned["translation_pending"] = True

        client_evidence.append(_slim_client_evidence_item(cloned))

    return client_evidence


async def translate_evidence_for_client(evidence: List[Dict], target_language: str = "zh") -> List[Dict]:
    if not evidence:
        return []

    evidence = [item for item in evidence if not is_weak_synthetic_qa_chunk(item)]
    if not evidence:
        return []
    ...
    client_evidence = prepare_evidence_for_client(
        evidence,
        target_language=target,
        include_cross_language_raw=False,
    )
    to_translate = []
    for index, item in enumerate(evidence):
        text = rag_display_text(item)
        lang = client_evidence[index].get("text_language") or detect_rag_text_language(text)

        if (
            text
            and lang != target
            and env_bool("DURIAN_RAG_TRANSLATE_EVIDENCE", True)
        ):
            to_translate.append({
                "index": index,
                "text": text[:550],
                "source_language": lang if lang != "unknown" else "auto",
            })

    if not to_translate or _durian_httpx is None:
        return client_evidence

    base_url = os.getenv("DURIAN_OPENAI_BASE_URL", "http://127.0.0.1:8010/v1").rstrip("/")
    openai_model = os.getenv("DURIAN_OPENAI_MODEL", "durian-lora")
    url = f"{base_url}/chat/completions"
    source_payload = json.dumps(to_translate[:2], ensure_ascii=False)
    prompt = (
        f"Translate every RAG reference excerpt below into {target_names[target]} for UI display. "
        f"All returned translations must be in {target_names[target]}, regardless of whether the source is Chinese, English, Malay, Thai, or unknown. "
        "Preserve numbers, cultivar names, chemical names, units, and page/source wording. "
        "Do not add advice or new facts. Return only a JSON array like "
        '[{"index":0,"translation":"..."}].\n\n'
        f"{source_payload}"
    )

    try:
        translation_timeout = float(RAG_CONFIG.get("translate_timeout_sec", 6.0) or 6.0)
        timeout = _durian_httpx.Timeout(connect=2.0, read=translation_timeout, write=8.0, pool=2.0)
        async with _durian_httpx.AsyncClient(timeout=timeout, trust_env=False) as client:
            response = await client.post(
                url,
                json={
                    "model": openai_model,
                    "messages": [
                        {"role": "system", "content": "You are a precise translation engine for agricultural reference excerpts."},
            ...
            translations = _extract_json_array(content) or []
        ...
        for row in translations:
            ...
            try:
                ...
                continue
            if 0 <= idx < len(client_evidence):
                client_evidence[idx]["translated_text"] = translated
                client_evidence[idx]["display_text"] = translated
                client_evidence[idx]["display_language"] = target
                applied += 1
        logger.info("RAG evidence translated for client: target=%s requested=%d applied=%d", target, len(to_translate), applied)
    except Exception as exc:
        logger.warning("RAG evidence translation failed; cross-language snippets stay hidden: %s", exc)
...
# =========================
# Disease-aware RAG keyword fallback
# =========================
DISEASE_RAG_ALIASES = {
    "炭疽病": ["炭疽病", "炭疽", "anthracnose", "colletotrichum", "胶孢炭疽菌"],
    "疫病": ["疫病", "phytophthora", "棕榈疫霉", "palmivora"],
    "根腐病": ["根腐病", "根腐", "根茎腐烂病", "root rot", "phytophthora"],
    "茎腐病": ["茎腐病", "茎腐", "canker", "stem rot", "phytophthora"],
    "藻斑病": ["藻斑病", "algal leaf spot"],
    "煤烟病": ["煤烟病", "sooty mold", "sooty mould"],
    "叶斑病": ["叶斑病", "leaf spot"],
    "白根病": ["白根病", "white root disease"],
    "赤衣病": ["赤衣病", "pink disease"],
}

def detect_required_disease_terms(query: str) -> List[str]:
    q = str(query or "").lower()
    terms: List[str] = []
    for _disease, aliases in DISEASE_RAG_ALIASES.items():
        if any(str(a).lower() in q for a in aliases):
...
def rag_item_has_required_disease(query: str, item: Dict) -> bool:
    required = detect_required_disease_terms(query)
    if not required:
        return True
    meta = item.get("metadata") if isinstance(item.get("metadata"), dict) else {}
    text = " ".join([
        str(rag_index_text(item) or ""),
        str(item.get("text") or ""),
        str(item.get("original_text") or ""),
    ...
    return any(str(t).lower() in text for t in required)


def _lexical_norm_text(value: Any) -> str:
...
def _lexical_terms(query: str) -> List[str]:
    q = _lexical_norm_text(query)
    terms: List[str] = []

    # English/Malay/Thai-like tokens and scientific names.
    for t in re.findall(r"[A-Za-z][A-Za-z0-9\-]{2,}", q):
        terms.append(t.lower())

    # Chinese disease/agri meaningful fragments.
    cjk = "".join(re.findall(r"[\u4e00-\u9fff]+", q))
    if cjk:
        # 2-5 char ngrams; exact copied PDF sentence can still match after punctuation cleanup.
        for n in (5, 4, 3, 2):
            if len(cjk) >= n:
                for i in range(0, len(cjk) - n + 1):
                    gram = cjk[i:i+n]
                    # skip very generic grams
                    if gram in {"榴莲", "病害", "防治", "症状", "处理", "管理"}:
                        continue
                    terms.append(gram)
...
# Direct LlamaIndex RAG backend replacement.
try:
    from rag_llamaindex import (
        is_exact_answer_query as llamaindex_is_exact_answer_query,
        normalize_ocr_units as llamaindex_normalize_ocr_units,
        retrieve_for_backend as llamaindex_retrieve_for_backend,
    )
except Exception as _llamaindex_import_error:
    llamaindex_retrieve_for_backend = None
    llamaindex_is_exact_answer_query = None
    llamaindex_normalize_ocr_units = None
    try:
        logger.warning("LlamaIndex RAG backend import failed: %s", _llamaindex_import_error)
    except Exception:
        pass
...
RAG_TOPIC_RULES = {
    "anthracnose": {
        "group": "disease",
        "aliases": ("炭疽", "anthracnose", "colletotrichum", "胶孢"),
    },
    "leaf_spot": {
        "group": "disease",
        "aliases": (
    ...
    "stem_end_rot": {
        "group": "disease",
        "aliases": ("蒂腐", "茎端腐", "stem end rot"),
    },
    "pink_disease": {
        "group": "disease",
        "aliases": ("赤衣", "pink disease"),
    },
    "root_rot": {
        "group": "disease",
        "aliases": (
            "根腐", "根系腐烂", "根部腐烂", "根部容易腐烂", "烂根",
            "白根", "root rot", "root decay", "white root", "phytophthora",
        ),
    },
    "gummosis": {
        "group": "disease",
        "aliases": (
            "流胶", "溃疡病", "树干溃疡", "分泌脓液", "胶质渗出",
            "gummosis", "canker disease", "canker",
        ),
    },
    "fruit_spoilage": {
        "group": "quality",
        "aliases": ("变质", "果肉变质", "腐烂", "霉变", "果腐", "软腐", "烂果", "深色凹陷", "凹陷斑块", "水渍状", "酸馊", "酒味", "fruit rot", "soft rot", "spoilage", "mold", "mould"),
    },
    "spider_mite": {
        "group": "pest",
        "aliases": ("红蜘蛛", "spider mite"),
    },
    "thrips": {
        "group": "pest",
        "aliases": ("蓟马", "thrips"),
    ...
    "mealybug": {
        "group": "pest",
        "aliases": ("粉蚧", "介壳虫", "mealybug"),
    },
    "fruit_borer": {
        "group": "pest",
        "aliases": ("蛀果虫", "蛀果害虫", "钻心虫", "榴莲实象鼻虫", "实象鼻虫", "螟蛾", "咬果", "seed borer", "durian seed borer", "mudaria", "mudaria luteileprosa", "fruit borer"),
    },
    "fruit_fly": {
        "group": "pest",
        "aliases": ("果蝇", "fruit fly"),
    },
    "psyllid": {
        "group": "pest",
        "aliases": ("木虱", "psyllid"),
...
def _rag_evidence_full_text(item: Dict[str, Any]) -> str:
    if not isinstance(item, dict):
        return ""
    meta = item.get("metadata") if isinstance(item.get("metadata"), dict) else {}
    parts = [
...
def _rag_topic_tags(text: str) -> set[str]:
    value = str(text or "").lower()
    # OCR often inserts spaces inside Chinese words ("根 部容易腐 烂",
    # "分 泌脓液"). Normalize those before concept matching.
    value = re.sub(r"(?<=[\u3400-\u9fff])\s+(?=[\u3400-\u9fff])", "", value)
    tags: set[str] = set()
    for tag, rule in RAG_TOPIC_RULES.items():
        if any(alias.lower() in value for alias in rule["aliases"]):
            tags.add(tag)
    # "腐烂" alone must not turn a root/tree disease question into a harvested
    # fruit-quality question.
    if "fruit_spoilage" in tags:
        root_context = any(
            marker in value
            for marker in ("根系", "根部", "树根", "烂根", "root rot", "root decay")
        )
        fruit_context = any(
            marker in value
            for marker in ("果肉", "果实", "榴莲果", "采后", "食用", "fruit flesh", "postharvest")
        )
        if root_context and not fruit_context:
            tags.discard("fruit_spoilage")
    return tags


def format_grounded_symptom_diagnosis(
    query: str,
    evidence: List[Dict[str, Any]],
    response_language: str = "zh",
) -> Optional[str]:
    """Deterministic synthesis for a multi-symptom root/canker question.

    The answer intentionally distinguishes a symptom-compatible diagnosis from
    a pathogen identification. It never invents species, pesticide rates, soil
    types or treatment details that are absent from the retrieved passages.
    """
    query_tags = _rag_topic_tags(query)
    if not {"root_rot", "gummosis"}.issubset(query_tags):
        return None
    evidence_tags: set[str] = set()
    evidence_text = "\n".join(_rag_evidence_full_text(item) for item in evidence or [])
    evidence_tags.update(_rag_topic_tags(evidence_text))
    if not {"root_rot", "gummosis"}.issubset(evidence_tags):
        return None

    lang = str(response_language or "zh").lower()
    if lang == "en":
        return (
            "The evidence most strongly supports **waterlogging-related root decay/root rot after prolonged rain**, while the gummosis indicates that a **trunk or collar canker** may also be present. These symptoms alone do not identify one pathogen, so they should not be labelled as white-root disease or a particular *Phytophthora* species without field confirmation.\n\n"
            "Check: (1) standing water and drainage around the root zone; (2) how far the dark, soft or dead roots extend; (3) the trunk base/root collar for cankers and exudation; (4) whether the tree has weak growth or fails to produce new shoots; and (5) whether nearby trees show the same pattern. Photograph the root zone and trunk lesions for comparison."
        )
    if lang == "ms":
        return (
            "Bukti paling menyokong **pereputan akar akibat takungan air selepas hujan berpanjangan**, manakala aliran getah menunjukkan kemungkinan **kanker pada pangkal atau batang** berlaku serentak. Gejala ini sahaja tidak mencukupi untuk menentukan satu patogen tertentu.\n\n"
            "Periksa: air bertakung dan saliran zon akar; keluasan akar gelap/lembut/mati; luka kanker dan lelehan pada pangkal batang; kelemahan pertumbuhan atau kegagalan mengeluarkan pucuk baharu; serta sama ada pokok berdekatan menunjukkan pola yang sama."
        )
    if lang == "th":
        return (
            "หลักฐานสอดคล้องมากที่สุดกับ **รากเน่าจากน้ำขังหลังฝนตกต่อเนื่อง** และอาการยางไหลบ่งชี้ว่าอาจมี **แผลแคงเกอร์บริเวณโคนหรือลำต้น** ร่วมด้วย อาการเหล่านี้เพียงอย่างเดียวยังระบุเชื้อสาเหตุชนิดเดียวไม่ได้\n\n"
            "ควรตรวจน้ำขังและการระบายน้ำ ขอบเขตรากที่คล้ำ/นิ่ม/ตาย แผลและของเหลวที่โคนต้น การแตกยอดใหม่ และว่าต้นใกล้เคียงมีอาการแบบเดียวกันหรือไม่ พร้อมถ่ายภาพบริเวณรากและแผลลำต้น"
        )
    return (
        "从检索到的资料看，最应优先怀疑的是**连续降雨、根区积水造成的根部腐烂/根腐问题**；同时出现流胶，说明还要并行排查**根颈或树干溃疡**。这组症状只能确定排查方向，不能仅凭描述就定成“白根病”，也不能直接锁定某一种 *Phytophthora* 病原。\n\n"
        "还需要检查：\n"
        "- 根区是否仍有积水，排水沟是否堵塞，土层多久才能退水；\n"
        "- 细根和主根发黑、变软或死亡的范围，是否只在低洼侧更严重；\n"
        "- 根颈和树干是否有水渍状溃疡、裂口或渗出物，按压病部是否继续渗液；\n"
        "- 树势是否明显变弱、是否停止抽新梢；\n"
        "- 邻近树是否出现相同症状，并拍摄根区、根颈和流胶部位用于复核。"
    )


def focus_evidence_on_query_topics(
    query: str,
    evidence: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    """Replace long SOP/index previews with windows around matched concepts."""
    query_tags = _rag_topic_tags(query)
    if not query_tags:
    ...
    for item in evidence or []:
        ...
        for tag in sorted(query_tags):
            rule = RAG_TOPIC_RULES.get(tag) or {}
            positions = [
                lower.find(str(alias).lower())
                for alias in rule.get("aliases", ())
                if str(alias) and lower.find(str(alias).lower()) >= 0
            ]
            if not positions:
                continue
            position = min(positions)
            if any(abs(position - prior) < 260 for prior in used_positions):
                continue
            used_positions.append(position)
            start = max(0, position - 180)
            end = min(len(raw), position + 420)
        ...
        if windows:
            excerpt_text = "\n".join(windows[:3])
            updated["display_text"] = excerpt_text
            updated["text"] = excerpt_text
            updated["index_text"] = excerpt_text
            updated["full_original_text"] = raw
        focused.append(updated)
    return focused


def _rag_topic_groups(tags: set[str]) -> set[str]:
    return {
        str(RAG_TOPIC_RULES[tag].get("group") or "")
        for tag in tags
        if tag in RAG_TOPIC_RULES
    }


def _first_rag_topic_group(text: str) -> str:
    value = str(text or "").lower()
    best_pos = None
    best_group = ""
    for tag, rule in RAG_TOPIC_RULES.items():
...
def _rag_evidence_is_generic_multi_topic_index(text: str) -> bool:
    value = str(text or "").lower()
    if not value:
        return False
    tags = _rag_topic_tags(value)
    strong_markers = [
        "目录页面",
        "示意图7.2",
        "所有榴莲种植园区",
    ...
    if tags and any(marker.lower() in value for marker in strong_markers):
        return True
    ...
    marker_hits = sum(1 for marker in generic_markers if marker.lower() in value)
    return bool(len(tags) >= 2 and marker_hits >= 2) or bool(len(tags) >= 4 and marker_hits >= 1)


def _rag_evidence_has_substantive_detail(text: str) -> bool:
    value = str(text or "").lower()
    return any(
        marker in value
        for marker in (
            "会导致", "导致", "影响", "症状", "分泌", "腐烂", "感染",
            "无法长出", "容易发生", "建议", "防治", "处理", "检查方法",
            "causes", "symptom", "affects", "exud", "rot", "control",
        )
    )


def _rag_evidence_has_blocking_conflict(query: str, text: str) -> bool:
    q = str(query or "").lower()
    value = str(text or "").lower()
    if any(term in q for term in ["榴莲", "果肉", "外壳", "变质", "腐烂", "霉变", "果腐", "软腐", "烂果", "durian", "fruit rot", "spoilage"]):
        human_medical_terms = [
            "躯体症状", "焦虑", "广泛性焦虑", "精神科", "心理科", "phq-9", "gad-7",
            "somatic symptom", "generalized anxiety", "mental", "psychiat",
            "疾病管制", "衛生福利", "卫生福利", "cdc.gov", "医院", "醫院", "mountelizabeth",
        ]
        if any(term in value for term in human_medical_terms):
            return True
        unrelated_commercial_terms = [
            "乒乓球台", "篮球架", "組合滑梯", "组合滑梯", "体育器材", "體育器材",
            "健身路径", "公园椅", "教学设备", "爱采购 logo", "b2bwiki.baidu.com",
        ]
        if any(term in value for term in unrelated_commercial_terms):
            return True

    if any(term in q for term in ["炭疽", "anthracnose", "colletotrichum", "叶斑", "leaf spot"]):
        human_anthrax_terms = [
            "bacillus anthracis", "炭疽杆菌", "炭疽桿菌", "急性传染病", "急性傳染病",
            "疾病管制", "衛生福利", "卫生福利", "cdc.gov", "检体采检", "檢體採檢",
            "感染对象为牛", "感染對象為牛", "牛、羊", "皮毛", "焦痂", "呼吸困难",
            "anthrax disease", "human anthrax",
        ]
        if any(term in value for term in human_anthrax_terms):
            return True

    if any(term in q for term in ["蛀果虫", "蛀果害虫", "fruit borer", "seed borer", "mudaria"]):
        wrong_borer_terms = [
            "针孔蛀心虫", "針孔蛀心蟲", "pinhole borer", "ambrosia beetle",
            "钻出微小的针孔", "鑽出微小的針孔", "逐出木屑", "木屑聚集",
            "枝条蛀", "树干蛀", "枝干蛀", "stem borer",
        ]
        if any(term in value for term in wrong_borer_terms):
            return True
...
def validate_rag_evidence(
    ...
    *,
    ...
    """Final evidence gate before any retrieved text is inserted into the model prompt."""
    if not evidence:
        return [], "none"

    original_count = len(evidence)
    guarded = apply_query_intent_evidence_guard(query, list(evidence))
    if not guarded:
        logger.info(
            "RAG evidence validation rejected by intent guard: source=%s query=%s before=%d",
            source,
            str(query or "")[:120],
            original_count,
        )
    ...
    query_tags = _rag_topic_tags(query)
    query_groups = _rag_topic_groups(query_tags)
    query_has_pest = "pest" in query_groups or any(x in str(query or "").lower() for x in ["虫", "pest"])
    filtered: List[Dict[str, Any]] = []
    dropped_topic = 0
    dropped_conflict = 0

    for item in guarded:
        text = _rag_evidence_full_text(item)
        if not text:
            continue
        item_tags = _rag_topic_tags(text)
        if _rag_evidence_has_blocking_conflict(query, text):
            dropped_conflict += 1
            continue
        if (
            _rag_evidence_is_generic_multi_topic_index(text)
            and (
                not item_tags.intersection(query_tags)
                or not _rag_evidence_has_substantive_detail(text)
            )
        ):
            dropped_conflict += 1
            continue
        leading = text[:320]
        leading_group = _first_rag_topic_group(leading)

        if query_tags and item_tags and not item_tags.intersection(query_tags):
            dropped_topic += 1
            continue
        if query_tags and not item_tags:
            dropped_topic += 1
            continue
        if "disease" in query_groups and not query_has_pest and leading_group == "pest":
            dropped_conflict += 1
            continue

        filtered.append(item)

    if query_tags and not filtered:
        logger.info(
            "RAG evidence validation removed all off-topic evidence: source=%s query=%s before=%d topic_drop=%d conflict_drop=%d tags=%s",
            source,
            str(query or "")[:120],
            len(guarded),
            dropped_topic,
    ...
    has_explicit_topic_match = bool(
        query_tags
        and any(
            _rag_topic_tags(_rag_evidence_full_text(item)).intersection(query_tags)
            for item in filtered
        )
    )
    if not has_explicit_topic_match and not ragflow_evidence_matches_query_anchors(query, filtered):
        logger.info(
            "RAG evidence validation rejected by anchor gate: source=%s query=%s before=%d after=%d",
            source,
            str(query or "")[:120],
            original_count,
            len(filtered),
    ...
    if dropped_topic or dropped_conflict or len(filtered) != original_count:
        logger.info(
            "RAG evidence validation kept=%d before=%d topic_drop=%d conflict_drop=%d source=%s query=%s",
            len(filtered),
            original_count,
            dropped_topic,
            dropped_conflict,
    ...
    quality = evidence_quality or "high"
    if quality == "none":
        quality = "medium" if filtered else "none"
    return filtered, quality


COMPARISON_QUERY_MARKERS = ("区别", "差别", "不同", "对比", "比较", "怎么区分", "如何区分", "区分")
COMPARISON_CONNECTORS = ("和", "跟", "与", "同", "vs", "VS", "versus", "相比")


RAG_TOPIC_SEARCH_LABELS = {
    "anthracnose": "榴莲炭疽病 Anthracnose Colletotrichum 叶片病斑 症状 防治",
    "leaf_spot": "榴莲叶斑病 leaf spot 叶片褐斑 坏死斑 症状 防治",
    "stem_end_rot": "榴莲蒂腐病 Stem End Rot 果蒂 茎端腐 症状 防治",
    "pink_disease": "榴莲赤衣病 Pink Disease 树枝粉红病斑 防治",
    "root_rot": "榴莲根腐病 白根病 root rot white root 症状 防治",
    "gummosis": "榴莲流胶病 gummosis 树干流胶 症状 防治",
    "fruit_spoilage": "榴莲果腐病 采后腐烂 果实底部深色凹陷 软腐 霉变 食用安全 防治",
    "spider_mite": "榴莲红蜘蛛 spider mite 叶片危害 防治",
    "thrips": "榴莲蓟马 thrips 嫩梢叶片危害 防治",
    "mealybug": "榴莲粉蚧 mealybug 介壳虫 防治",
    "fruit_borer": "榴莲蛀果虫 fruit borer seed borer Mudaria 果实蛀孔 防治",
    "fruit_fly": "榴莲果蝇 fruit fly 果实危害 防治",
    "psyllid": "榴莲木虱 psyllid 嫩梢危害 防治",
}


def is_comparison_rag_query(query: str) -> bool:
    value = str(query or "").strip()
    if not value:
        return False
    lowered = value.lower()
    has_marker = any(marker in value for marker in COMPARISON_QUERY_MARKERS)
    has_connector = any(connector in value or connector.lower() in lowered for connector in COMPARISON_CONNECTORS)
    return has_marker and has_connector


def ordered_rag_topic_tags(text: str) -> List[str]:
    value = str(text or "").lower()
    positions: List[Tuple[int, str]] = []
    for tag, rule in RAG_TOPIC_RULES.items():
        best_pos: Optional[int] = None
        for alias in rule.get("aliases") or ():
            pos = value.find(str(alias).lower())
            if pos >= 0 and (best_pos is None or pos < best_pos):
                best_pos = pos
        if best_pos is not None:
            positions.append((best_pos, tag))
    positions.sort(key=lambda item: item[0])
    return [tag for _, tag in positions]


def build_comparison_retrieval_queries(query: str) -> List[Tuple[str, str]]:
    if not is_comparison_rag_query(query):
        return []
    tags = ordered_rag_topic_tags(query)
    if len(tags) < 2:
        return []
    ...
    for tag in tags:
        if tag in seen:
            continue
        seen.add(tag)
        label = RAG_TOPIC_SEARCH_LABELS.get(tag)
        if not label:
            continue
        queries.append((tag, f"{label}；{compact_query[:180]}"))
        if len(queries) >= 3:
            break
    return queries


def evidence_topic_coverage(evidence: List[Dict[str, Any]]) -> set[str]:
    coverage: set[str] = set()
    for item in evidence or []:
        if not isinstance(item, dict):
            continue
        text = _rag_evidence_full_text(item)
        if _rag_evidence_is_generic_multi_topic_index(text):
            continue
        coverage.update(_rag_topic_tags(text))
    return coverage


def retrieve_comparison_grounded_context(
    query: str,
    response_language: str = "zh",
    *,
    local_top_k: int = 3,
    ...
    """Retrieve both sides of a comparison instead of letting one broad query drift."""
    split_queries = build_comparison_retrieval_queries(query)
    if not split_queries:
        return [], "none", ""

    expected_tags = {tag for tag, _ in split_queries}
    merged: List[Dict[str, Any]] = []
    timeout_sec = float(os.getenv("DURIAN_COMPARISON_RAG_TIMEOUT_SEC", "5.0"))
    max_workers = max(1, min(len(split_queries), int(os.getenv("DURIAN_COMPARISON_RETRIEVAL_WORKERS", "3"))))

    def _local_side(task: Tuple[str, str]) -> Tuple[str, str, List[Dict[str, Any]]]:
        tag, side_query = task
        side_evidence, _side_quality, _side_text = retrieve_ragflow_grounded_context(
            side_query,
            response_language,
            exact_mode=False,
            top_k=local_top_k,
            relevance_query=side_query,
            timeout_sec=timeout_sec,
        )
        return tag, side_query, side_evidence

    if max_workers > 1:
        with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="comparison-rag") as executor:
            futures = [executor.submit(_local_side, task) for task in split_queries]
            for future in as_completed(futures):
                try:
                    tag, side_query, side_evidence = future.result()
                except Exception as exc:
                    logger.warning("Comparison local retrieval failed: query=%s error=%s", str(query or "")[:120], exc)
                    continue
                for item in side_evidence:
                    if isinstance(item, dict):
                        item = dict(item)
                        item["comparison_topic_tag"] = tag
                        item["comparison_side_query"] = side_query
                    merged = merge_web_evidence(merged, [item], max_items=12)
    else:
        for task in split_queries:
            try:
                tag, side_query, side_evidence = _local_side(task)
            except Exception as exc:
                logger.warning("Comparison local retrieval failed: query=%s error=%s", str(query or "")[:120], exc)
                continue
            for item in side_evidence:
    ...
    coverage = evidence_topic_coverage(merged)
    missing_tasks = [
        (tag, side_query)
        for tag, side_query in split_queries
        if tag in (expected_tags - coverage) or not merged
    ]
    if missing_tasks and WEB_SEARCH_CONFIG["enabled"] and WEB_SEARCH_CONFIG["api_key"]:
        web_workers = max(1, min(len(missing_tasks), int(os.getenv("DURIAN_COMPARISON_WEB_WORKERS", "3"))))

        def _web_side(task: Tuple[str, str]) -> Tuple[str, str, List[Dict[str, Any]]]:
            tag, side_query = task
            web_evidence, _web_quality, _web_text = tavily_web_search(
                side_query,
                response_language,
                max_results=web_top_k,
            )
            return tag, side_query, web_evidence

        if web_workers > 1:
            with ThreadPoolExecutor(max_workers=web_workers, thread_name_prefix="comparison-web") as executor:
                futures = [executor.submit(_web_side, task) for task in missing_tasks]
                for future in as_completed(futures):
                    try:
                        tag, side_query, web_evidence = future.result()
                    except Exception as exc:
                        logger.warning("Comparison web retrieval failed: query=%s error=%s", str(query or "")[:120], exc)
                        continue
            ...
            for task in missing_tasks:
                try:
                    tag, side_query, web_evidence = _web_side(task)
                except Exception as exc:
                    logger.warning("Comparison web retrieval failed: query=%s error=%s", str(query or "")[:120], exc)
                    continue
                for item in web_evidence:
    ...
    if not merged:
        logger.info("Comparison RAG returned empty: query=%s tags=%s", str(query or "")[:120], sorted(expected_tags))
        return [], "none", ""

    merged = rerank_evidence(query, merged, top_k=8)
    merged, quality = validate_rag_evidence(
        query,
        merged,
        "medium",
        top_k=8,
        source="comparison_split",
    )
    if not merged:
        return [], "none", ""

    coverage = evidence_topic_coverage(merged)
    if expected_tags.issubset(coverage):
        quality = "high"
    elif coverage.intersection(expected_tags):
        quality = "medium"
    else:
        quality = "low"

    logger.info(
        "Comparison RAG merged evidence=%d quality=%s expected=%s coverage=%s query=%s workers=%d/%d",
        len(merged),
        quality,
        ...
        str(query or "")[:120],
        max_workers,
        int(os.getenv("DURIAN_COMPARISON_WEB_WORKERS", "3")),
    )
    return (
        merged,
        quality,
...
def llamaindex_rag_retrieve(query: str, top_k: int = 3):
    """Replacement for llamaindex_rag_retrieve().
    ...
    if llamaindex_retrieve_for_backend is None:
        try:
            logger.warning("LlamaIndex RAG backend unavailable; returning empty evidence.")
        except Exception:
            pass
        return [], "none"
    ...
    try:
        ...
        evidence = llamaindex_retrieve_for_backend(str(query or ""), top_k=top_k)
        raw_elapsed = time.perf_counter() - raw_started
    except Exception as exc:
        try:
            logger.exception("LlamaIndex RAG retrieval failed after %.3fs: %s", time.perf_counter() - started, exc)
        except Exception:
            pass
        return [], "none"
    ...
    try:
        logger.info(
            "LlamaIndex RAG returned evidence: query=%r count=%d raw=%.3fs",
            query,
            len(evidence),
            raw_elapsed,
        )
    ...
    if not evidence:
        logger.info(
            "LlamaIndex RAG timing: total=%.3fs raw=%.3fs rerank=0.000s validate=0.000s evidence=0 quality=none query=%s",
            time.perf_counter() - started,
            raw_elapsed,
            str(query or "")[:120].replace("\n", " "),
        )
        return [], "none"

    rerank_started = time.perf_counter()
    evidence = rerank_evidence(query, evidence, top_k=top_k)
    ...
    if not evidence:
        logger.info(
            "LlamaIndex RAG timing: total=%.3fs raw=%.3fs rerank=%.3fs validate=%.3fs evidence=0 quality=none query=%s",
            time.perf_counter() - started,
            raw_elapsed,
            rerank_elapsed,
            validate_elapsed,
    ...
    logger.info(
        "LlamaIndex RAG timing: total=%.3fs raw=%.3fs rerank=%.3fs validate=%.3fs evidence=%d quality=%s query=%s",
        time.perf_counter() - started,
        raw_elapsed,
        rerank_elapsed,
        validate_elapsed,
...
def rag_lexical_fallback_evidence(query: str, limit: int = 12) -> List[Dict]:
    # Disabled because LlamaIndex directly replaces the old FAISS/lexical RAG path.
    return []
    if is_broad_short_keyword_query(query):
        logger.info(
            "RAG lexical fallback skipped for broad short keyword query: %s",
            str(query)[:100].replace("\n", " "),
        )
        return []
    ...
    Dense retrieval can miss exact PDF/OCR text. This scans loaded chunks by
    exact substring / ngram overlap and returns high lexical-overlap candidates.
    It is language-agnostic and works for copied PDF original text.
    """
    if not query or not rag_retriever or not getattr(rag_retriever, "chunks", None):
        return []
    ...
    q_norm = _lexical_norm_text(query)
    scored: List[Dict] = []

    for chunk in getattr(rag_retriever, "chunks", []) or []:
        if not isinstance(chunk, dict) or is_rag_noise_chunk(chunk):
            continue

        text = str(rag_index_text(chunk) or _rag_chunk_text(chunk) or "").strip()
        if not text:
            continue

        hay = _lexical_norm_text(" ".join([
            text,
            str(chunk.get("original_text") or ""),
        ...
        # Long exact overlap is more reliable than many tiny ngrams.
        longest_hit = max((len(t) for t in hits), default=0)
        hit_score = len(hits) * 0.35 + min(longest_hit, 18) * 0.35

        # If a normalized query fragment is directly contained, boost strongly.
        direct_bonus = 0.0
        if len(q_norm) >= 12 and (q_norm[:80] in hay or hay[:80] in q_norm):
            direct_bonus += 8.0

        source_file_l = str(chunk.get("source_file") or "").lower()
        type_l = str(chunk.get("type") or "").lower()
        doc_l = str(chunk.get("doc") or "").lower()

        source_bonus = 0.0
        source_penalty = 0.0
        if source_file_l == "ocr_texts.json":
            source_bonus += 3.0
        if type_l == "qa_pair" or doc_l.startswith("faq_"):
            source_penalty += 2.0

        score = hit_score + direct_bonus + source_bonus - source_penalty
        if score <= 1.0:
            continue

        item = chunk.copy()
        item["index_text"] = text
        item["text"] = text
        item["dense_score"] = float(item.get("dense_score", 0.0) or 0.0)
        item["lexical_score"] = round(score, 4)
        item["score"] = round(score, 6)
        item["_lexical_fallback"] = True
        item["_lexical_hits"] = hits[:12]
        scored.append(item)

    scored.sort(key=lambda x: float(x.get("score", 0.0)), reverse=True)
    if scored:
        logger.info(
            "RAG lexical fallback: query=%s terms=%d hits=%d top_score=%.4f top_doc=%s",
            str(query)[:100].replace("\\n", " "),
            len(terms),
            len(scored),
            float(scored[0].get("score", 0.0)),
            scored[0].get("doc") or scored[0].get("source_file"),
        )
    return scored[:limit]



def rag_keyword_fallback_evidence(query: str, limit: int = 8) -> List[Dict]:
    # Disabled because LlamaIndex directly replaces the old FAISS/keyword RAG path.
    return []
    if is_broad_short_keyword_query(query):
        logger.info(
            "RAG keyword fallback skipped for broad short keyword query: %s",
            str(query)[:100].replace("\n", " "),
        )
        return []
    ...
    if not required:
        return []
    if not rag_retriever or not getattr(rag_retriever, "chunks", None):
        return []

    required_l = [str(t).lower() for t in required]
    scored: List[Dict] = []
    ...
    for chunk in getattr(rag_retriever, "chunks", []) or []:
        ...
        index_text = rag_index_text(chunk)
        raw_text = str(index_text or _rag_chunk_text(chunk) or "").strip()
        if not raw_text:
            continue
        ...
        hits = [t for t in required_l if t in haystack]
        if not hits:
            continue
        ...
        # Prefer real disease-section chunks over shallow FAQ/SOP/list chunks.
        section_bonus = 0.0

        source_file_l = str(chunk.get("source_file") or "").lower()
        doc_l = str(chunk.get("doc") or "").lower()
        item_index_l = str(chunk.get("item_index") or "")
        page_l = str(chunk.get("page") or "")

        # 原始 OCR 正文章节强加权：这些通常比 QA_pair 更适合作为 RAG evidence。
        if source_file_l == "ocr_texts.json":
            section_bonus += 8.0
        if any(x in doc_text for x in ["中国榴莲栽培技术", "cultivation"]):
            section_bonus += 4.0
        if item_index_l.startswith("114") or page_l.startswith("114"):
            section_bonus += 3.0

        # 炭疽病正文特征词强加权。
        if any(x in haystack for x in ["二、榴莲炭疽病", "榴莲炭疽病的症状", "病原为", "分生孢子", "孢子盘", "高温、高湿", "pda培养基"]):
            section_bonus += 12.0
        if any(x in haystack for x in ["图6-3", "图6-4", "主要病虫害防控", "胶孢炭疽菌", "gloeosporioides"]):
            section_bonus += 8.0
        if any(x in haystack for x in ["叶片病斑多从叶尖或叶缘开始", "病斑边缘变为深褐色", "橘红色的孢子堆"]):
            section_bonus += 8.0

        shallow_penalty = 0.0

        # QA_pair 是二次生成资料，可以用，但不能排在原始章节前面。
        if str(chunk.get("type") or "").lower() == "qa_pair" or "faq_" in doc_text or "question:" in haystack:
            shallow_penalty += 12.0

        # SOP 只是列表/流程，不适合作为病害症状正文证据。
        if "sop_" in doc_text or "树况检测 sop" in haystack:
            shallow_penalty += 10.0

        # 只列出病名但没有症状/病原/防控细节的 chunk 降权。
        if "5.1.2 炭疽病" in haystack and "症状" not in haystack and "病原" not in haystack:
            shallow_penalty += 6.0

        # 图注类 chunk 信息量较低，不能压过正文症状/病原/发生条件。
        caption_like = (
            re.match(r"^图\s*\d", haystack.strip()) is not None
            or re.match(r"^图\d", haystack.strip()) is not None
            or ("谢昌平摄" in haystack and len(haystack) < 120)
            or ("a. 成熟叶片" in haystack and len(haystack) < 160)
        )
        if caption_like:
            shallow_penalty += 8.0

        # 问炭疽病时，混入疫病/根腐/Phytophthora 的 evidence 降权。
        if any(x in str(query or "").lower() for x in ["炭疽", "anthracnose", "colletotrichum"]):
            if any(x in haystack for x in ["phytophthora", "palmivora", "疫病", "根腐", "根茎腐烂", "流胶"]):
                shallow_penalty += 8.0

        item = chunk.copy()
        item["index_text"] = index_text or raw_text
        item["text"] = index_text or raw_text
        item["canonical_language"] = _rag_canonical_language()
        item["dense_score"] = float(item.get("dense_score", item.get("score", 0.0)) or 0.0)
        item["lexical_score"] = float(len(hits) * 3.0 + section_bonus - shallow_penalty)
        item["score"] = float(3.0 + len(hits) * 2.0 + section_bonus - shallow_penalty + item["dense_score"])
        item["_keyword_fallback"] = True
        item["_disease_hits"] = hits[:8]
        item["_section_bonus"] = section_bonus
        item["_shallow_penalty"] = shallow_penalty
        scored.append(item)
...
def build_stream2_rag_context_sync(
    user_query: str,
    response_language: str,
    ...
    try:
        # LlamaIndex direct-first path.
        # Do not let legacy FAISS rag_retriever.enabled or old query gate block LlamaIndex.
        direct_started = time.perf_counter()
        configured_top_k = int(RAG_CONFIG.get("top_k", 3))
        if is_exact_answer_query(user_query):
            configured_top_k = max(
                configured_top_k,
                int(os.getenv("DURIAN_RAG_EXACT_TOP_K", "10")),
            )
        configured_candidate_top_k = min(4, int(RAG_CONFIG.get("candidate_top_k", 4)))
        candidate_top_k = max(configured_top_k, min(4, configured_candidate_top_k))

        direct_query = (
            user_query
            if is_exact_answer_query(user_query)
            else expand_multilingual_rag_query(
                user_query,
                _rag_canonical_language(),
            )
        )

        evidence, evidence_quality = llamaindex_rag_retrieve(
            direct_query,
            top_k=candidate_top_k,
        )

        if evidence:
            evidence = evidence[:configured_top_k]
            evidence_quality = evidence_quality or "high"
            ...
            logger.info(
                "stream2 LlamaIndex direct-first return: total=%.3fs evidence=%d quality=%s query=%s",
        ...
        logger.info(
            "stream2 LlamaIndex direct-first empty: total=%.3fs query=%s",
            time.perf_counter() - direct_started,
            user_query[:100],
        )
        return [], "none", ""

        if not rag_retriever or not getattr(rag_retriever, "enabled", False):
            return [], "none", ""

        if require_domain_check and not should_use_rag_for_query(user_query):
            logger.info("stream2 RAG skipped by query gate: query=%s", user_query[:100])
            return [], "none", ""

        lock_wait_sec = max(0.0, float(RAG_CONFIG.get("lock_wait_sec", 2.0) or 0.0))
        if lock_wait_sec > 0:
            lock_acquired = RAG_QUERY_LOCK.acquire(timeout=lock_wait_sec)
        else:
            lock_acquired = RAG_QUERY_LOCK.acquire(blocking=False)

        if not lock_acquired:
            logger.warning(
                "stream2 RAG skipped because a previous RAG query is still running after %.2fs: query=%s",
                lock_wait_sec,
                user_query[:100],
            )
            return [], "none", ""
        ...
        retrieval_query = expand_multilingual_rag_query(user_query, _rag_canonical_language())
        ...
        # General lexical fallback:
        # Dense retrieval can miss exact PDF/OCR original text. Always add a few
        # lexical candidates, then let smart_filter rerank dense + lexical together.
        lexical_started = time.perf_counter()
        lexical_evidence = rag_lexical_fallback_evidence(
            user_query,
            limit=max(candidate_top_k, configured_top_k * 4),
        )
        if lexical_evidence:
            evidence = lexical_evidence + evidence
        filter_elapsed += time.perf_counter() - lexical_started

        if evidence:
            filter_started = time.perf_counter()
            evidence, evidence_quality = smart_filter_rag_evidence(
                user_query,
                evidence,
                top_k=configured_top_k,
            )
        ...
        # Disease keyword fallback:
        # If the user explicitly asks for a disease but vector retrieval/filtering returns
        # no matching evidence, scan loaded RAG chunks by disease aliases.
        if detect_required_disease_terms(user_query) and not any(
            rag_item_has_required_disease(user_query, ev) for ev in evidence
        ):
            fallback_started = time.perf_counter()
            fallback_evidence = rag_keyword_fallback_evidence(
                user_query,
                limit=max(candidate_top_k, configured_top_k * 4),
            )
            ...
            filter_elapsed += time.perf_counter() - fallback_started
        ...
        if evidence:
            ...
            evidence_text = build_evidence_text(
                evidence,
                max_chars=RAG_CONFIG.get("max_evidence_chars", 420),
            )
            prompt_elapsed = time.perf_counter() - prompt_started

        logger.info(
            "stream2 RAG timing: total=%.3fs retrieve=%.3fs filter=%.3fs prompt=%.3fs evidence=%d quality=%s query=%s",
            time.perf_counter() - started,
            retrieve_elapsed,
            filter_elapsed,
            prompt_elapsed,
        ...
        logger.warning("stream2 RAG failed after %.3fs: %s", time.perf_counter() - started, exc)
        return [], "none", ""
    finally:
        if lock_acquired:
            RAG_QUERY_LOCK.release()




DURIAN_CODE_PATTERN = re.compile(
    r"(?i)(?:\bD\s*[-]?\s*\d{1,4}\b|品种编号|品种代号|品种代码|编号为|代号为|猫山王|苏丹王|黑刺|金枕|青尼|干尧|托曼尼|D24|D197|D200)"
)


def is_durian_code_query(query: str) -> bool:
    ...
    return any(term in q for term in code_intent_terms)


def rag_item_matches_durian_code_intent(item: Dict[str, Any]) -> bool:
    text = " ".join([
        str(item.get("display_text") or ""),
        str(item.get("text") or ""),
        str(item.get("index_text") or ""),
    ...
    if not compact:
        return False
    return bool(DURIAN_CODE_PATTERN.search(compact))


def apply_query_intent_evidence_guard(query: str, evidence: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Query-aware final guard to prevent lexical false positives."""
    if not evidence:
        return []

    if is_durian_code_query(query):
        guarded = [
            item for item in evidence
            if isinstance(item, dict) and rag_item_matches_durian_code_intent(item)
        ]
        if len(guarded) != len(evidence):
            logger.info(
                "RAG query intent guard applied: durian_code query=%r before=%d after=%d",
                query,
                len(evidence),
                len(guarded),
            )
...
def final_clean_rag_evidence_for_query(query: str, evidence: List[Dict]) -> List[Dict]:
    # LlamaIndex has already done retrieval/filtering. Do not run old FAISS/OCR cleaners here,
    # because they can incorrectly remove valid LlamaIndex evidence before metadata is sent.
    if evidence and any(
        str(item.get("rag_backend") or item.get("retrieval_backend") or item.get("evidence_source") or "").lower() == "llamaindex"
        or str(item.get("id") or "").startswith("llamaindex_")
        for item in evidence
        if isinstance(item, dict)
    ):
        cleaned = []
        ...
        for item in evidence:
            ...
            doc = str(item.get("doc") or item.get("source_file") or "unknown")
            key = (doc, text[:180])
            if key in seen:
                continue
            seen.add(key)
            ...
            item = dict(item)
            item.setdefault("display_text", text)
            item.setdefault("text", text)
            item.setdefault("index_text", text)
            item.setdefault("original_text", text)
            item.setdefault("source_file", doc)
            item.setdefault("doc", doc)
            item.setdefault("score", item.get("dense_score", 0.0))
            item.setdefault("dense_score", item.get("score", 0.0))
            item.setdefault("lexical_score", 0.0)
            item.setdefault("canonical_language", "zh")
            item.setdefault("text_language", "zh")
            item.setdefault("display_language", "zh")
            item["rag_backend"] = "llamaindex"
            item["retrieval_backend"] = "llamaindex"
            item["evidence_source"] = "llamaindex"
        ...
        try:
            logger.info("final_clean bypassed for LlamaIndex evidence: query=%r count=%d", query, len(cleaned))
        except Exception:
            pass

        return cleaned

    """Final safety cleanup before prompt/client display.

    If the user pasted an original PDF/OCR sentence and raw OCR evidence exists,
    suppress synthetic FAQ duplicates that merely restate the same source evidence.
    This is intentionally placed after retrieval/rerank because there may be
    multiple smart_filter/chat_stream2 definitions in legacy code.
    """
    if not evidence:
        return evidence

    def _n(v: Any) -> str:
        if "_lexical_norm_text" in globals():
            return _lexical_norm_text(v)
        text = str(v or "").lower()
        text = re.sub(r"\\[a-zA-Z]+", " ", text)
        text = re.sub(r"[^\u4e00-\u9fffA-Za-z0-9%\.]+", " ", text)
    ...
    def _is_faq(item: Dict) -> bool:
        return (
            str(item.get("type") or "").lower() == "qa_pair"
            or str(item.get("doc") or "").lower().startswith("faq_")
            or bool(item.get("question") or item.get("answer"))
        )

    def _text(item: Dict) -> str:
        try:
            idx_text = rag_index_text(item)
        except Exception:
            idx_text = ""
        return _n(" ".join([
            str(idx_text or ""),
    ...
    raw_exact_exists = False
    for item in evidence:
        if _is_faq(item):
            continue
        if str(item.get("source_file") or "").lower() != "ocr_texts.json":
            continue
        txt = _text(item)
        if qn[:40] in txt or qn[:24] in txt or txt[:40] in qn:
            raw_exact_exists = True
            break

    if not raw_exact_exists:
        return evidence
    ...
    if dropped:
        logger.info(
            "Final RAG evidence cleanup: dropped_faq=%d kept=%d query=%s",
            dropped,
            len(cleaned),
            str(query)[:100].replace("\\n", " "),
        )
...
def is_broad_short_keyword_query(query: str) -> bool:
    """Detect short ambiguous keyword-only queries.

    These should not be forced through RAG, because a narrow evidence fragment
    can hijack the answer. Example: "榴莲品种" should not become a discussion
    of self-incompatibility unless the user asks about pollination/fruit set.
    """
    q = str(query or "").strip()
    if not q:
        return False
    ...
    # Explicit detailed intent: allow RAG.
    explicit_terms = [
        "对比", "比较", "区别", "推荐", "种植", "授粉", "坐果", "自交", "亲和",
        "防治", "症状", "原因", "怎么治", "怎么办", "表格", "列出", "全部",
        "compare", "recommend", "pollination", "fruit set", "self-incompatibility",
    ]
    if any(t.lower() in q_compact for t in explicit_terms):
        return False

    broad_terms = {
        "榴莲品种", "品种", "榴莲价格", "价格", "榴莲种植", "种植",
        "榴莲施肥", "施肥", "榴莲病害", "病害", "榴莲叶子", "叶子黄",
        "榴莲成熟", "成熟度", "榴莲保存", "保存",
    }

    if q_compact in broad_terms:
        return True

    # Very short CJK topic-like query, no question marker or action intent.
    cjk_chars = re.findall(r"[\u4e00-\u9fff]", q)
    has_question_or_action = any(x in q for x in ["吗", "么", "如何", "怎么", "为什么", "哪些", "什么", "能不能", "要不要"])
    if 2 <= len(cjk_chars) <= 6 and not has_question_or_action:
        return True

    return False


async def build_stream2_rag_context(
    user_query: str,
    response_language: str,
    require_domain_check: bool = True,
) -> Tuple[List[Dict], str, str]:
    """Build stream2 RAG context with timeout.

    Keep streaming responsive. If LlamaIndex is slow, continue without RAG.
    """
    started = time.perf_counter()
    timeout_sec = float(RAG_CONFIG.get("stream2_rag_timeout_sec", 1.0) or 3.0)

    try:
        task = asyncio.to_thread(
            build_stream2_rag_context_sync,
            user_query,
            response_language,
            require_domain_check,
        )

        evidence, evidence_quality, evidence_text = await asyncio.wait_for(
            task,
            timeout=timeout_sec,
        )

        logger.info(
            "stream2 RAG context completed: total=%.3fs evidence=%d quality=%s query=%s",
            time.perf_counter() - started,
            len(evidence or []),
            evidence_quality,
            user_query[:100],
        ...
        logger.warning(
            "stream2 RAG timeout after %.2fs; continuing without RAG: query=%s",
            timeout_sec,
            user_query[:100],
        )
        return [], "none", ""
        ...
        logger.warning(
            "stream2 RAG failed after %.3fs; continuing without RAG: query=%s error=%s",
            time.perf_counter() - started,
            user_query[:100],
            exc,
        )
...
@app.post("/chat/stream2", tags=["Chat"], dependencies=[Depends(verify_api_key)])
async def chat_stream2(request: InferenceRequest):
    if _durian_httpx is None:
    ...
    # 寒暄/感谢/极短闲聊直接规则回复。
    # 不调用模型，不走 RAG，避免“你好”被模型扩写成榴莲科普表格。
    if is_smalltalk_query(user_query):
        logger.info("stream2 smalltalk fast path: query=%s lang=%s", user_query[:80], response_language)
        return smalltalk_sse_response(response_language)
    ...
    if request.use_rag:
        evidence, evidence_quality, evidence_text = await build_stream2_rag_context(
            user_query,
            response_language,
            require_domain_check=True,
        )

        # Final evidence cleanup for exact PDF/OCR text queries.
        # Keep raw OCR evidence and suppress synthetic FAQ duplicates before both
        # prompt construction and frontend metadata display.
        try:
            logger.info("Before final_clean stream path: query=%r count=%d backend=%s",
                        user_query, len(evidence or []),
                        (evidence[0].get("rag_backend") if evidence and isinstance(evidence[0], dict) else None))
        except Exception:
            pass

        try:
            logger.info("Before final_clean stream path 2: query=%r count=%d backend=%s",
                        user_query, len(evidence or []),
                        (evidence[0].get("rag_backend") if evidence and isinstance(evidence[0], dict) else None))
        except Exception:
            pass

        evidence = final_clean_rag_evidence_for_query(user_query, evidence)

        try:
            logger.info("After final_clean stream path 2: query=%r count=%d backend=%s",
                        user_query, len(evidence or []),
                        (evidence[0].get("rag_backend") if evidence and isinstance(evidence[0], dict) else None))
        except Exception:
            pass
        ...
        try:
            logger.info("After final_clean stream path: query=%r count=%d backend=%s",
                        user_query, len(evidence or []),
                        (evidence[0].get("rag_backend") if evidence and isinstance(evidence[0], dict) else None))
        except Exception:
            pass
    ...
    evidence = force_clean_boundary_evidence(user_query, evidence)
    if evidence:
        evidence_text = build_evidence_text(
            evidence,
            max_chars=RAG_CONFIG.get("max_evidence_chars", 420),
    ...
    if should_use_tavily_web_search(user_query, evidence, evidence_quality):
        web_evidence, _web_quality, _web_evidence_text = await asyncio.to_thread(
            tavily_web_search,
            user_query,
            response_language,
        ...
        if web_evidence:
            ...
            logger.info(
                "stream2 web fallback merged: local=%d web=%d final=%d quality=%s",
                local_count,
                len(web_evidence),
                len(evidence),
                evidence_quality,
    ...
    initial_client_evidence = prepare_evidence_for_client(evidence, response_language)
    initial_client_evidence = filter_metadata_evidence_for_client(initial_client_evidence)
    if not initial_client_evidence:
        evidence_quality = "none"

    no_rag_named_subject_response = (
        build_no_rag_named_subject_response(user_query, response_language)
        if not evidence
        else ""
    )
    ...
    if evidence_text:
        system_prompt = (
            ...
            + evidence_text
            + "\n\n【RAG 使用规则】\n"
              "1. 如果参考资料与用户问题相关，必须优先基于参考资料组织答案。\n"
              "2. 回答中的关键判断、步骤、药剂/处理建议，应尽量来自参考资料。\n"
              "3. 如果参考资料没有直接说明，可以补充通用知识，但要避免编造具体数值、药剂浓度或固定周期。\n"
              "4. 如果参考资料不相关，明确按图像或常识判断，不要强行引用。"
        )
    ...
    logger.info(
        "stream2 request: model=%s url=%s query=%s evidence=%d quality=%s",
        openai_model,
        url,
        user_query[:100],
        len(evidence),
    ...
    async def event_generator():
        ...
        evidence_translation_task = None

        # 先发 metadata，前端可以先显示参考资料。
        yield "data: " + json.dumps(
            {
                "type": "metadata",
                "evidence_quality": evidence_quality if initial_client_evidence else "none",
                "evidence": initial_client_evidence,
            },
            ensure_ascii=False,
        ) + "\n\n"

        if no_rag_named_subject_response:
            full_response = no_rag_named_subject_response
            for index in range(0, len(full_response), 12):
                yield "data: " + json.dumps(
                    {
                        "type": "content",
                        "content": full_response[index : index + 12],
                    },
                    ensure_ascii=False,
                ) + "\n\n"
            yield "data: " + json.dumps(
                {
                    "type": "done",
                    "tokens_generated": approx_token_count(full_response),
                    "evidence_quality": evidence_quality,
                    "evidence": [],
                },
                ensure_ascii=False,
        ...
        if not is_openai_proxy_mode():
            try:
                ...
                for chunk in collect_model_stream(
                    ...
                    if chunk:
                        if not first_content_logged:
                            ...
                            logger.info(
                                "stream2 local first content latency: %.3fs evidence=%d quality=%s query=%s",
                ...
                if evidence:
                    try:
                        evidence_translation_task = asyncio.create_task(
                            translate_evidence_for_client(evidence, response_language)
                        )
                        final_client_evidence = await evidence_translation_task
                        yield "data: " + json.dumps(
                            {
                                "type": "evidence",
                                "evidence_quality": evidence_quality,
                                "evidence": final_client_evidence,
                            },
                            ensure_ascii=False,
                        ) + "\n\n"
                    except Exception as exc:
                        logger.warning("RAG evidence translation after local stream2 answer failed: %s", exc)

                return

            except Exception as e:
                logger.exception("stream2 local_vllm failed")
                yield "data: " + json.dumps(
                    {
                        "type": "error",
                        "error": str(e),
        ...
        try:
            ...
            async with _durian_httpx.AsyncClient(
                timeout=timeout,
                trust_env=False,
            ) as client:
                async with client.stream(
                    ...
                    if resp.status_code != 200:
                        ...
                        logger.error("stream2 upstream error: status=%s body=%s", resp.status_code, msg[:1000])
                        yield "data: " + json.dumps(
                            {
                                "type": "error",
                                "error": f"vLLM upstream HTTP {resp.status_code}: {msg[:500]}",
                            },
                            ensure_ascii=False,
                        ) + "\n\n"
                        return
                        ...
                        try:
                            ...
                            if content:
                                if not first_content_logged:
                                    first_content_logged = True
                                    logger.info(
                                        "stream2 first content latency: %.3fs evidence=%d quality=%s query=%s",
                                        time.perf_counter() - stream_started,
                                        len(evidence),
                                        evidence_quality,
                                        user_query[:100],
                            ...
                            if finish_reason:
                                logger.info("stream2 finish_reason=%s response_chars=%d", finish_reason, len(full_response))
                        except Exception as e:
                            logger.warning("stream2 parse chunk failed: %s line=%s", e, data[:300])
                            continue
            ...
            if evidence:
                try:
                    ...
                    logger.warning("RAG evidence translation after answer failed: %s", exc)

        except Exception as e:
            logger.exception("stream2 failed")
            yield "data: " + json.dumps(
                {
                    "type": "error",
                    "error": str(e),
            ...
            if evidence_translation_task is not None and not evidence_translation_task.done():
                evidence_translation_task.cancel()
            yield "data: [DONE]\n\n"
...
@app.post("/chat/stream3", tags=["Chat"], dependencies=[Depends(verify_api_key)])
async def chat_stream3(
    request: InferenceRequest,
    current_user: str = Depends(get_current_user),
):
    """LangGraph-backed streaming chat with persistent per-conversation memory."""
    if langgraph_runtime is None:
        raise HTTPException(status_code=503, detail="LangGraph runtime 未初始化")
    if not request.conversation_id:
        raise HTTPException(status_code=400, detail="conversation_id 不能为空")
    if not is_model_ready():
        raise HTTPException(status_code=503, detail="模型未加载")

    # Authorize the thread against the existing conversation store. This also
    # prevents a user from selecting another account's LangGraph thread ID.
    store.get_conversation(request.conversation_id, owner=current_user)
    ...
    message_payloads = [
        {
            ...
            "active_context_card": message.active_context_card,
            "evidence": message.evidence,
            "evidence_quality": message.evidence_quality or "none",
        }
    ...
    max_tokens = resolve_request_max_tokens(request)
    thread_id = f"{current_user}:{request.conversation_id}"
    generation_config = {
        "max_tokens": max_tokens,
        "temperature": request.temperature,
        "top_p": request.top_p,
    ...
    def event_generator():
        try:
            ...
            for event in langgraph_runtime.stream_turn(
                thread_id=thread_id,
                messages=message_payloads,
                requested_language=request.response_language,
            ...
            logger.info(
                "LangGraph stream client disconnected: user=%s conversation_id=%s",
                current_user,
            ...
            logger.exception(
                "LangGraph stream failed: user=%s conversation_id=%s",
                current_user,
                request.conversation_id,
            )
            yield "data: " + json.dumps(
    ...
    return StreamingResponse(
        ...
        headers={
            ...
            "Connection": "keep-alive",
            "X-Accel-Buffering": "no",
            "X-Durian-Orchestrator": "langgraph",
        },
    )
...
# Disabled duplicate second /chat/stream2 route. Active route is the first one above.
# @app.post("/chat/stream2", tags=["Chat"], dependencies=[Depends(verify_api_key)])
async def chat_stream2_duplicate_disabled(request: InferenceRequest):
    if _durian_httpx is None:
        raise HTTPException(status_code=500, detail="httpx is not installed. Run: pip install httpx")
...
# ============================================================
# Query-aware RAG post-retrieval filter
# Fix: dense-only FAISS often retrieves generic durian / market /
# processing documents for specific questions such as varieties.
# ============================================================
def smart_filter_rag_evidence(query: str, evidence: List[Dict], top_k: int = 4) -> Tuple[List[Dict], str]:
    if not evidence:
        return [], "none"
    ...
    variety_triggers = [
        "品种", "品類", "品类", "variety", "cultivar", "พันธุ์",
        "猫山王", "黑刺", "金枕", "d24", "musang", "monthong",
    ]

    variety_terms = [
        "猫山王", "musang king", "musang", "d197",
        "黑刺", "black thorn", "d200",
        "金枕", "monthong", "หมอนทอง",
        "d24", "苏丹王", "sultan", "xo",
        "红虾", "red prawn", "d175",
        ...
        "坤宝", "kanyao", "ก้านยาว",
        "chanee", "ชะนี",
        "พันธุ์", "variety", "cultivar",
    ]

    market_or_processing_noise = [
        "市场", "出口", "价格", "产量", "market", "export", "price",
        "ตลาด", "ส่งออก", "ราคา", "ผลผลิต",
        "加工", "副产品", "榴莲皮", "果皮", "เปลือก", "แปรรูป",
        "อินโดนีเซีย", "ฟิลิปปินส์", "philippines", "indonesia",
    ]

    disease_terms = [
        "黄叶", "发黄", "叶黄", "积水", "烂根", "根腐", "疫病",
        "炭疽", "施肥", "排水", "缺氮", "缺镁", "缺钾",
        "yellow", "root rot", "waterlogging", "fertilizer",
        "ใบเหลือง", "รากเน่า", "น้ำขัง", "ปุ๋ย", "โรค",
    ]

    is_variety_query = any(t.lower() in q for t in variety_triggers)
    is_disease_or_cultivation_query = any(t.lower() in q for t in disease_terms)

    scored = []

    for item in evidence:
        text = full_text(item)
        dense_score = float(item.get("dense_score", item.get("score", 0.0)) or 0.0)

        lexical = 0.0
        penalty = 0.0

        if is_variety_query:
            # 品种问题必须命中品种词/品种语义；不能只因为出现“榴莲”就算相关。
            for term in variety_terms:
                if term.lower() in text:
                    lexical += 1.0

            # 明确品种名权重更高。
            strong_terms = [
                "猫山王", "musang king", "黑刺", "black thorn", "金枕", "monthong",
                "d24", "d197", "d200", "red prawn", "红虾", "xo"
            ]
            for term in strong_terms:
                if term.lower() in text:
                    lexical += 2.0

            # 市场/出口/加工副产品对“品种介绍”是噪声。
            for term in market_or_processing_noise:
                if term.lower() in text:
                    penalty += 1.2
        ...
        elif is_disease_or_cultivation_query:
            ...
            # 泛问题：保守过滤市场/加工噪声，不强行显示文献。
            lexical = 0.5
            for term in market_or_processing_noise:
                if term.lower() in text:
                    penalty += 0.5

        final_score = dense_score + 0.18 * lexical - 0.15 * penalty

        if final_score <= 0:
            continue

        new_item = item.copy()
        new_item["dense_score"] = dense_score
        new_item["lexical_score"] = round(lexical, 4)
        new_item["score"] = round(final_score, 6)
        scored.append(new_item)

    scored.sort(key=lambda x: float(x.get("score", 0.0)), reverse=True)

    # 文本去重 + 来源限制
    # If copied PDF/OCR text is recovered from raw OCR chunks, suppress synthetic FAQ
    # duplicates that merely restate the same source evidence.
    raw_exact_available = False
    q_norm_for_exact = _lexical_norm_text(query) if "_lexical_norm_text" in globals() else norm(query)
    if len(q_norm_for_exact) >= 8:
        for _item in scored:
            _source_file = str(_item.get("source_file") or "").lower()
            _type = str(_item.get("type") or "").lower()
            _doc = str(_item.get("doc") or "").lower()
            _is_faq = _type == "qa_pair" or _doc.startswith("faq_")
            _txt = _lexical_norm_text(rag_index_text(_item) or _item.get("text") or "") if "_lexical_norm_text" in globals() else norm(rag_index_text(_item) or _item.get("text") or "")
            if _source_file == "ocr_texts.json" and not _is_faq and q_norm_for_exact[:60] in _txt:
                raw_exact_available = True
                break
    ...
    for item in scored:
        if raw_exact_available:
            _type = str(item.get("type") or "").lower()
            _doc = str(item.get("doc") or "").lower()
            _is_faq = _type == "qa_pair" or _doc.startswith("faq_")
            if _is_faq:
                continue
        text = norm(item.get("text", ""))[:260]
        ...
        source = (
            ...
            or item.get("metadata", {}).get("source") if isinstance(item.get("metadata"), dict) else None
        ) or "unknown"
    ...
    logger.info(
        "Smart RAG filter: query=%s before=%d after=%d quality=%s top_score=%.4f",
        query[:100],
        len(evidence),
        len(results),
        quality,
...
# ============================================================
# Override smart_filter_rag_evidence v2
# Stricter for variety queries:
# - Generic words like "พันธุ์ / variety / cultivar / 品种" are NOT enough.
# - Must match concrete cultivar names.
# ============================================================
def detect_rag_text_language(text: str) -> str:
    value = str(text or "")
    cjk = len(re.findall(r"[\u4e00-\u9fff]", value))
    thai = len(re.findall(r"[\u0e00-\u0e7f]", value))
    latin = len(re.findall(r"[A-Za-z]", value))
    lower = value.lower()
    ms_terms = [
        "yang", "dan", "untuk", "dengan", "atau", "tidak", "boleh", "perlu",
        "sebab", "cara", "pokok", "daun", "buah", "akar", "tanah", "saliran",
        "baja", "penyakit", "perosak", "matang", "simpan", "pilih", "hujan",
    ]
    ms_hits = sum(1 for term in ms_terms if re.search(rf"\b{re.escape(term)}\b", lower))

    if cjk >= max(4, thai, latin // 6):
        return "zh"
    if thai >= max(8, cjk * 2, latin // 4):
        return "th"
    if latin >= 12 and ms_hits >= 2:
        return "ms"
    if latin >= 12:
        return "en"
    return "unknown"
...
def rag_language_adjustment(query_lang: str, item_lang: str) -> Tuple[float, float]:
    if query_lang == "zh":
        if item_lang == "zh":
            return 0.25, 0.0
        if item_lang == "en":
...
def smart_filter_rag_evidence(query: str, evidence: List[Dict], top_k: int = 4) -> Tuple[List[Dict], str]:
    ...
    q = str(query or "").lower()
    query_lang = _rag_canonical_language()
    ...
    variety_query_triggers = [
        "品种", "品類", "品类", "variety", "cultivar", "พันธุ์",
        "猫山王", "黑刺", "金枕", "d24", "musang", "monthong",
    ]

    # 具体品种名。问“品种有哪些”时，必须命中这些，不能只命中 generic "พันธุ์"。
    concrete_variety_terms = [
        "猫山王", "musang king", "musang", "d197",
        "黑刺", "black thorn", "d200",
        "金枕", "monthong", "หมอนทอง",
        "d24", "苏丹王", "sultan",
    ...
    # 这些词太泛，不再作为品种问题的通过条件。
    generic_variety_terms = [
        "品种", "variety", "cultivar", "พันธุ์", "พันธุ์ทุเรียน"
    ]
    ...
    noise_terms = [
        ...
        "加工", "副产品", "榴莲皮", "果皮", "เปลือก", "แปรรูป",
        "อินโดนีเซีย", "ฟิลิปปินส์", "philippines", "indonesia",
        "雨季", "แตกใบ", "ใบอ่อน", "ปุ๋ย", "ไนโตรเจน",
        "ผลแก่", "เต่าเผา", "เนื้อตาย", "โรค", "แมลง",
    ]

    cultivation_query_triggers = [
        "黄叶", "发黄", "叶黄", "积水", "烂根", "根腐", "疫病",
        "炭疽", "施肥", "排水", "缺氮", "缺镁", "缺钾",
        "修剪", "开花", "坐果", "落果", "病虫害",
        "yellow", "root rot", "waterlogging", "fertilizer", "disease",
        "ใบเหลือง", "รากเน่า", "น้ำขัง", "ปุ๋ย", "โรค",
    ]

    is_variety_query = any(t.lower() in q for t in variety_query_triggers)
    is_cultivation_query = any(t.lower() in q for t in cultivation_query_triggers)
    required_disease_terms = detect_required_disease_terms(query)

    scored = []

    for item in evidence:
        if is_rag_noise_chunk(item):
            continue
        text = item_text(item)
        disease_hits = [t for t in required_disease_terms if str(t).lower() in text]
        dense_score = float(item.get("dense_score", item.get("score", 0.0)) or 0.0)
        item_lang = detect_rag_text_language(text)
        if query_lang == "zh" and item_lang == "th" and env_bool("DURIAN_RAG_BLOCK_THAI_FOR_ZH", True):
            continue
        language_bonus, language_penalty = rag_language_adjustment(query_lang, item_lang)

        concrete_hits = [t for t in concrete_variety_terms if t.lower() in text]
        generic_hits = [t for t in generic_variety_terms if t.lower() in text]
        noise_hits = [t for t in noise_terms if t.lower() in text]

        if is_variety_query:
            # 核心修复：品种问题必须命中具体品种名。
            # 只有 "พันธุ์ / variety / 品种" 这种泛词，一律不要。
            if not concrete_hits:
                continue

            lexical_score = len(concrete_hits) * 2.0 + min(len(generic_hits), 2) * 0.2
            penalty = min(len(noise_hits), 4) * 0.35

        elif is_cultivation_query:
            # 如果用户明确说了某个病名，例如“炭疽病 / Anthracnose / Colletotrichum”，
            # evidence 必须包含该病名或同义词；否则不允许根腐病/疫病资料混进来。
            if required_disease_terms and not disease_hits:
                continue

            cultivation_hits = [t for t in cultivation_query_triggers if t.lower() in text]
            if not cultivation_hits and not disease_hits:
                continue

            lexical_score = len(cultivation_hits) * 1.0 + len(disease_hits) * 3.0
            penalty = min(len(noise_hits), 4) * 0.2

        else:
            # 泛问题保守：不要让市场/加工/果皮资料乱入。
            lexical_score = 0.3
            penalty = min(len(noise_hits), 4) * 0.35

        # Carry keyword-fallback ranking signals into the final smart filter.
        # Without this, fallback section_bonus/shallow_penalty is computed but then lost here.
        section_bonus = float(item.get("_section_bonus", 0.0) or 0.0)
        shallow_penalty = float(item.get("_shallow_penalty", 0.0) or 0.0)

        # General lexical fallback score. This is critical for copied PDF/OCR original text:
        # dense embedding may miss it, but lexical overlap should still recover it.
        lexical_fallback_bonus = 0.0
        if item.get("_lexical_fallback"):
            lexical_fallback_bonus = min(float(item.get("lexical_score", 0.0) or 0.0), 12.0)

        rerank_bonus = 0.0
        if RERANK_CONFIG.get("enabled"):
            try:
                rerank_bonus = float(item.get("rerank_score_norm", 0.0) or 0.0)
            except Exception:
                rerank_bonus = 0.0

        disease_conflict_penalty = 0.0
        if required_disease_terms:
            q_lower = str(query or "").lower()

            # If user explicitly asks Anthracnose / Colletotrichum, suppress Phytophthora/root-rot evidence.
            if any(x in q_lower for x in ["炭疽", "anthracnose", "colletotrichum"]):
                conflict_terms = [
                    "phytophthora", "palmivora", "疫病", "根腐", "根茎腐烂",
                    "茎腐", "流胶", "root rot", "stem rot"
                ]
                conflict_hits = [x for x in conflict_terms if x in text]
                if conflict_hits:
                    disease_conflict_penalty += min(len(conflict_hits), 4) * 1.2

            # If a fallback item is only a shallow FAQ/SOP list, keep it usable but below real section chunks.
            if item.get("_keyword_fallback"):
                if section_bonus <= 0:
                    shallow_penalty += 1.5
        ...
        final_score = (
            ...
            + language_bonus
            + 0.35 * section_bonus
            + lexical_fallback_bonus
            + 0.70 * rerank_bonus
            - penalty
            - language_penalty
            - 0.35 * shallow_penalty
            - disease_conflict_penalty
        )
        ...
        # Keep the real lexical fallback score for debugging/display.
        # Otherwise exact PDF/OCR text hits may be shown as lexical_score=0.3
        # because the generic smart-filter branch overwrites lexical_score.
        display_lexical_score = float(lexical_score)
        if item.get("_lexical_fallback"):
            display_lexical_score = max(
                display_lexical_score,
                float(item.get("lexical_score", 0.0) or 0.0),
            )

        new_item["lexical_score"] = round(float(display_lexical_score), 4)
        new_item["score"] = round(float(final_score), 6)
        new_item["_debug_language"] = item_lang
        new_item["_debug_concrete_hits"] = concrete_hits[:8]
        new_item["_debug_noise_hits"] = noise_hits[:8]
        scored.append(new_item)

    scored.sort(key=lambda x: float(x.get("score", 0.0)), reverse=True)

    # Raw-source first rule for explicit disease queries.
    # If the user asks a concrete disease and raw textbook/OCR chunks are available,
    # prefer them over synthetic FAQ chunks.
    if detect_required_disease_terms(query):
        raw_scored = []
        faq_scored = []
        other_scored = []

        for item in scored:
            source_file_l = str(item.get("source_file") or "").lower()
            doc_l = str(item.get("doc") or "").lower()
            type_l = str(item.get("type") or "").lower()
            text_l = str(rag_index_text(item) or item.get("text") or "").lower()

            is_faq = type_l == "qa_pair" or doc_l.startswith("faq_") or "question:" in text_l
            is_raw_textbook = (
                source_file_l == "ocr_texts.json"
                or (
                    "中国榴莲栽培技术" in str(item.get("doc") or "")
                    and not is_faq
                )
            )

            if is_raw_textbook:
                raw_scored.append(item)
            elif is_faq:
                faq_scored.append(item)
            else:
                other_scored.append(item)

        if raw_scored:
            logger.info(
                "Smart RAG raw-first: query=%s raw=%d faq=%d other=%d",
                query[:100],
                len(raw_scored),
                len(faq_scored),
                len(other_scored),
            )
            scored = raw_scored + other_scored + faq_scored

    if query_lang == "zh":
        non_thai_scored = [item for item in scored if item.get("_debug_language") != "th"]
        if non_thai_scored:
            logger.info(
                "Smart RAG language filter: query_lang=zh before=%d after_non_thai=%d",
                len(scored),
                len(non_thai_scored),
            )
            scored = non_thai_scored
    ...
    for item in scored:
        dedup_text = norm(rag_index_text(item))[:260]
        if dedup_text in seen_texts:
            continue
        seen_texts.add(dedup_text)
        ...
        # 不把 debug 字段发给前端
        item.pop("_debug_language", None)
        item.pop("_debug_concrete_hits", None)
        item.pop("_debug_noise_hits", None)
        item.pop("_section_bonus", None)
        item.pop("_shallow_penalty", None)
        item.pop("_disease_hits", None)
        item.pop("_keyword_fallback", None)
        item.pop("_lexical_fallback", None)
    ...
    if not results:
        logger.info(
            "Smart RAG filter v2: query=%s before=%d after=0 quality=none reason=no concrete match",
            query[:100],
            len(evidence),
        )
        return [], "none"
    ...
    logger.info(
        "Smart RAG filter v2: query=%s before=%d after=%d quality=%s top_score=%.4f sources=%s",
        query[:100],
        len(evidence),
        len(results),
        quality,
        top_score,
        [
            r.get("doc") or r.get("source") or r.get("filename") or r.get("source_file")
            for r in results
        ],
    )
...
# ============================================================
# Stable image streaming endpoint
# Image pipeline:
#   1. Qwen-VL-Max image label/classification: non-streaming
#   2. Local advisor via vLLM OpenAI Server: true streaming
# Frontend should call /api/analyze_pest/stream2
# ============================================================

@app.post("/analyze_pest/stream2", tags=["Pest Classification"], dependencies=[Depends(verify_api_key)])
async def analyze_pest_stream2(
    file: UploadFile = File(...),
    query: str = Form(""),
    response_language: str = Form("zh"),
    ...
    query = query or ""
    response_language = infer_response_language_from_user_query(query, response_language or "zh")
    conversation_id = (conversation_id or "").strip()
    if conversation_id:
        store.get_conversation(conversation_id, owner=current_user)
    ...
    try:
        ...
        logger.exception("image stream2 upload setup failed")
    ...
    async def event_generator():
        ...
        active_context_card: Optional[Dict[str, Any]] = None
        prompt_evidence_text = ""
        ...
        def record_image_turn(card: Optional[Dict[str, Any]]) -> None:
            if not conversation_id or langgraph_runtime is None or not full_response:
                return
            try:
                langgraph_runtime.record_external_turn(
                    thread_id=f"{current_user}:{conversation_id}",
                    user_message={
                        "role": "user",
                        "content": query or "请分析这张榴莲图片",
                        "image_url": image_url,
                    },
                    assistant_message={
                        "role": "assistant",
                        "content": full_response,
                        "active_context_card": card,
                    },
                )
            except Exception:
                logger.exception(
                    "记录图片分析到 LangGraph 失败: user=%s conversation_id=%s",
                    current_user,
                    conversation_id,
                )
        ...
        try:
            ...
            visual_result = await asyncio.to_thread(
                qwen_vl_max_client.classify_durian_image_label,
                image,
                query,
                response_language,
            )
            if is_missing_image_vl_result(visual_result):
                logger.warning(
                    "image stream2 vision returned missing-image result; retrying once: label=%s evidence=%s",
                    visual_result.get("label_zh") or visual_result.get("label"),
                    str(visual_result.get("visual_evidence") or "")[:120],
                )
                yield sse({
                    "type": "status",
                    "phase": "vision_retry",
                    "message": "图片识别结果不稳定，正在重新识别...",
                })
            ...
            logger.info(
                "image stream2 vision timing: total=%.3fs model=%s label=%s category=%s query=%s",
                vision_elapsed,
                visual_result.get("model", "qwen-vl-max"),
                visual_result.get("label_zh") or visual_result.get("label") or "",
                visual_result.get("category_type") or "",
                query[:120].replace("\n", " "),
            )
        except Exception as exc:
            logger.exception("image stream2 vision stage failed")
            msg = str(exc)
            full_response = (
                "图片识别阶段没有成功返回结果，可能是视觉模型接口超时或不可用。"
                "我已经收到图片，但目前无法可靠读取图像内容；请稍后重试，或换一张更清晰、主体更近的图片。"
            ) if response_language == "zh" else (
                "The image recognition stage did not return a reliable result, likely because the vision model timed out or is unavailable. Please retry later or upload a clearer close-up image."
            )
            yield sse({"type": "content", "content": full_response})
            active_context_card = build_text_context_card(
                query or "图片诊断",
                full_response,
                response_language,
                source="image_diagnosis_error",
                image_url=image_url,
            )
            record_image_turn(active_context_card)
            yield sse({
                "type": "done",
                "tokens_generated": approx_token_count(full_response),
                "evidence_quality": "none",
                "evidence": [],
                "image_url": image_url,
                "visual_observation": json.dumps({"error": msg}, ensure_ascii=False),
                "active_context_card": active_context_card,
            })
            yield "data: [DONE]\n\n"
            return

        category_type = str(visual_result.get("category_type") or "unknown").strip().lower()
        label = str(visual_result.get("label") or "Unknown").strip()
        label_zh = str(visual_result.get("label_zh") or label).strip()
        confidence = float(visual_result.get("confidence") or 0.0)
        visual_evidence_text = str(visual_result.get("visual_evidence") or "").strip()
        alternatives = visual_result.get("alternatives") or []
        if not isinstance(alternatives, list):
            alternatives = [str(alternatives)]
        cannot_confirm = str(visual_result.get("cannot_confirm") or "").strip()
        vision_tokens = int(visual_result.get("tokens_generated", 0) or 0)
        vision_model = visual_result.get("model", "qwen-vl-max")
        ...
        try:
            ...
            search_query = (query + "\n" + label_zh + " " + label).strip()
            evidence, evidence_quality, evidence_text = await asyncio.to_thread(
                retrieve_ragflow_grounded_context,
                search_query,
                response_language,
                exact_mode=False,
                top_k=max(4, int(RAGFLOW_CONFIG.get("top_k", 12))),
                relevance_query=(query + "\n" + label_zh + " " + label).strip(),
                timeout_sec=float(os.getenv("DURIAN_RAGFLOW_LIVE_TIMEOUT_SEC", "6.0")),
            )
            rag_elapsed = time.perf_counter() - rag_started
            logger.info(
                "image stream2 RAG timing: total=%.3fs evidence=%d quality=%s query=%s",
                rag_elapsed,
                len(evidence or []),
                evidence_quality,
                search_query[:120].replace("\n", " "),
            )
        except Exception as exc:
            rag_elapsed = time.perf_counter() - rag_started if "rag_started" in locals() else 0.0
            logger.warning("image stream2 RAGFlow-only retrieval failed after %.3fs: %s", rag_elapsed, exc)
            evidence = []
            evidence_quality = "none"
            evidence_text = ""


        if should_use_tavily_web_search(search_query, evidence, evidence_quality):
            ...
            if web_evidence:
                ...
                logger.info(
                    "image stream2 web fallback merged: local=%d web=%d final=%d quality=%s",
                    local_count,
                    len(web_evidence),
                    len(evidence),
                    evidence_quality,
            ...
            logger.info(
                "image stream2 web timing: total=%.3fs web=%d final=%d quality=%s query=%s",
                web_elapsed,
                len(web_evidence or []),
                len(evidence or []),
                evidence_quality,
            ...
            logger.info(
                "image stream2 web skipped: local_evidence=%d quality=%s query=%s",
                len(evidence or []),
                evidence_quality,
                search_query[:120].replace("\n", " "),
            )
        ...
        if image_profile == "image_agri_medium":
            profile_rules = {
                "zh": "\n\n【农业图片回答档位】回答病虫害/种植问题时，给出可能问题、可见依据、现场复核点和优先处理建议。不要写成过长论文。",
                "en": "\n\n[Agri image profile] For pest/disease/cultivation images, give likely issue, visible basis, field checks and priority actions. Avoid an overly long report.",
                "ms": "\n\n[Profil imej pertanian] Beri isu kemungkinan, asas visual, semakan lapangan dan tindakan utama.",
                "th": "\n\n[รูปแบบภาพเกษตร] ให้ปัญหาที่เป็นไปได้ หลักฐานจากภาพ จุดตรวจภาคสนาม และการจัดการลำดับแรก.",
            }
            system_prompt += profile_rules.get(response_language, profile_rules["zh"])
            image_final_tokens = int(os.getenv("DURIAN_IMAGE_AGRI_MAX_NEW_TOKENS", "768"))
        elif image_profile == "image_consumer_natural":
            profile_rules = {
                "zh": "\n\n【消费/品种/品质图片回答档位】只谈品种可能性、口感、成熟度、能否食用、保存或挑选。不要输出病虫害、防治、喷药、施肥、复查时间、果园管理、商业分选或冷链 SOP。",
                "en": "\n\n[Consumer image profile] Discuss only likely variety, taste, ripeness, edibility, storage or selection. Do not add pest control, spraying, fertilization, orchard management or cold-chain SOP.",
                "ms": "\n\n[Profil imej pengguna] Jawab tentang varieti, rasa, kematangan, boleh dimakan, simpanan atau pemilihan sahaja.",
                "th": "\n\n[รูปแบบภาพผู้บริโภค] ตอบเฉพาะสายพันธุ์ รสชาติ ความสุก การกิน การเก็บ หรือการเลือกซื้อเท่านั้น.",
            }
            system_prompt += profile_rules.get(response_language, profile_rules["zh"])
            image_final_tokens = int(os.getenv("DURIAN_IMAGE_CONSUMER_MAX_NEW_TOKENS", "520"))
        else:
            system_prompt += (
                "\n\n[General image answer profile]\n"
                "Answer naturally from the image label and user question. If the label is uncertain, state what cannot be confirmed."
            )
            image_final_tokens = int(os.getenv("DURIAN_IMAGE_GENERAL_MAX_NEW_TOKENS", "700"))
        image_final_tokens = clamp_image_final_tokens(image_final_tokens)
        ...
        if image_profile == "image_agri_medium":
            task_hint = "Answer with medium detail. Include: likely problem name, visible basis, what to check on-site, and what to do first. Avoid a rigid table unless it is truly clearer."
        elif image_profile == "image_consumer_natural":
            task_hint = "Answer naturally about variety/quality/eating/storage only. No orchard treatment, commercial grading or pesticide advice."
        else:
            task_hint = "Answer naturally based on the label and the user's actual question."
...
""".strip()
        ...
        payload = {
            ...
            "temperature": 0.35,
            "top_p": 0.85,
            "max_tokens": int(image_final_tokens),
            "stream": True,
            "chat_template_kwargs": {"enable_thinking": False},
        }

        logger.info(
            "image stream2 request: model=%s query=%s label=%s category=%s evidence=%d quality=%s",
            openai_model,
            query[:100],
            label_zh,
            category_type,
        ...
        if evidence_text and len(prompt_evidence_text) < len(evidence_text):
            logger.info(
                "image stream2 prompt evidence compacted: original_chars=%d prompt_chars=%d evidence=%d",
                len(evidence_text),
                len(prompt_evidence_text),
                len(evidence),
            )

        final_client_evidence = filter_metadata_evidence_for_client(
            prepare_evidence_for_client(evidence, response_language)
        )
        yield sse({
            "type": "evidence",
            "evidence_quality": evidence_quality if final_client_evidence else "none",
            "evidence": final_client_evidence,
        })
        yield sse({
            "type": "status",
        ...
        try:
            ...
            if not is_openai_proxy_mode():
                ...
                for chunk in collect_model_stream(
                    model_instance.generate_stream,
                    payload["messages"],
                    max_new_tokens=int(payload.get("max_tokens", image_final_tokens)),
                    temperature=float(payload.get("temperature", 0.35)),
                    top_p=float(payload.get("top_p", 0.85)),
                ):
                    if not chunk:
                        continue
                    chunk_text = normalize_stream_text(str(chunk))
                    if not first_content_logged:
                        first_content_logged = True
                        logger.info(
                            "image stream2 first content latency: %.3fs mode=local evidence=%d quality=%s query=%s",
                    ...
                    yield sse({"type": "content", "content": chunk_text})
            else:
                timeout = _durian_httpx.Timeout(connect=10.0, read=None, write=30.0, pool=10.0)
                async with _durian_httpx.AsyncClient(timeout=timeout, trust_env=False) as client:
                    async with client.stream(
                        "POST",
                        url,
                        ...
                        if resp.status_code != 200:
                            body = await resp.aread()
                            msg = body.decode("utf-8", errors="ignore")
                            raise RuntimeError(f"vLLM upstream HTTP {resp.status_code}: {msg[:500]}")
                        async for line in resp.aiter_lines():
                            if not line:
                                continue
                            line = line.strip()
                            ...
                            try:
                                ...
                                content = delta.get("content") or ""
                                if content:
                                    content = normalize_stream_text(content)
                                    if not first_content_logged:
                                        first_content_logged = True
                                        logger.info(
                                            "image stream2 first content latency: %.3fs mode=openai_proxy evidence=%d quality=%s query=%s",
                                            time.perf_counter() - generation_started,
                                            len(evidence or []),
                                            evidence_quality,
                                            query[:120].replace("\n", " "),
                                ...
                                logger.warning("image stream2 parse chunk failed: %s line=%s", exc, data[:300])
                                continue

            if generation_truncated:
                continuation_note = "\n\n（回答已达到本次长度上限。需要后续清单时，可以直接发送“继续”。）"
                if response_language != "zh":
                    continuation_note = "\n\n(The answer reached the length limit. Send 'continue' for the remaining details.)"
                full_response += continuation_note
                yield sse({"type": "content", "content": continuation_note})
                logger.info(
                    "image stream2 generation truncated by max_tokens: max_tokens=%s chars=%d",
                    image_final_tokens,
                    len(full_response),
                )
            ...
            logger.info(
                "image stream2 generation timing: total=%.3fs chars=%d truncated=%s max_tokens=%s query=%s",
                generation_elapsed,
                len(full_response),
            ...
            logger.info(
                "image stream2 total timing: total=%.3fs vision=%.3fs rag=%.3fs web=%.3fs generation=%.3fs evidence=%d quality=%s query=%s",
            ...
            yield sse({
                "type": "done",
                "tokens_generated": len(full_response) + int(vision_tokens or 0),
                "evidence_quality": evidence_quality if final_client_evidence else "none",
                "evidence": final_client_evidence,
                "image_url": image_url,
                "visual_observation": vl_label_context,
            ...
            logger.exception("image stream2 failed after %.3fs", time.perf_counter() - turn_started)
...
# ============================================================
# RAG query gate
# Do not retrieve references for greetings / thanks / tiny chat.
# ============================================================
def should_use_rag_for_query(query: str) -> bool:
    ...
    greeting_exact = {
        ...
        "谢谢", "感谢", "thanks", "thankyou",
        "ok", "好的", "好", "嗯", "啊", "在吗", "你是谁",
        "早上好", "下午好", "晚上好",
    }

    if q_compact in greeting_exact:
        return False

    greeting_prefix = [
        "你好啊", "您好啊", "hello", "hi", "hey",
        "谢谢你", "感谢你",
    ]

    if any(q_compact.startswith(x.replace(" ", "")) for x in greeting_prefix):
        return False

    # 太短而且没有农业/榴莲关键词，不走 RAG
    domain_terms = [
        "榴莲", "durian", "ทุเรียน", "品种", "病", "虫", "叶", "果",
        "施肥", "修剪", "开花", "落果", "成熟", "保存", "购买",
        "猫山王", "黑刺", "金枕", "d24", "musang", "monthong",
        "variety", "cultivar", "disease", "pest", "leaf", "leaves", "fruit",
        "fertilizer", "fertiliser", "pruning", "flower", "flowering", "ripeness",
        "ripe", "storage", "buy", "soil", "water", "root", "rot", "yellow",
        "baja", "daun", "buah", "penyakit", "perosak", "akar", "tanah", "air",
        "matang", "simpan", "beli", "cantasan", "berbunga", "gugur",
        "โรค", "แมลง", "ใบ", "ผล", "ปุ๋ย", "ตัดแต่ง", "ดอก", "สุก",
        "เก็บ", "ซื้อ", "ดิน", "น้ำ", "ราก", "เหลือง",
    ]

    if not any(t.lower() in q for t in domain_terms):
        return False
...
# ============================================================
# Smalltalk / greeting fast path for stream2
# Prevent "你好" from being sent to the durian LLM and producing
# unrelated durian tables.
# ============================================================
def is_smalltalk_query(query: str) -> bool:
    ...
    # Compact identifiers are queries, not small talk. Keep this generic so
    # codes such as D229, HTTP 429, or CVE-... can reach retrieval/web search.
    if is_short_entity_query(q):
        return False

    # 非常短、没有明确榴莲/农业意图，也按寒暄处理。
    domain_terms = [
        "榴莲", "durian", "ทุเรียน",
        "叶", "果", "病", "虫", "肥", "水", "根", "花",
        "品种", "成熟", "保存", "购买", "猫山王", "黑刺", "金枕", "d24",
        "variety", "cultivar", "disease", "pest", "leaf", "leaves", "fruit",
        "fertilizer", "fertiliser", "pruning", "flower", "flowering", "ripeness",
        "ripe", "storage", "buy", "soil", "water", "root", "rot", "yellow",
        "baja", "daun", "buah", "penyakit", "perosak", "akar", "tanah", "air",
    ...
    if len(qc) <= 4 and not any(t.lower() in q for t in domain_terms):
        return True

    return False


def build_smalltalk_reply(response_language: str = "zh") -> str:
    lang = response_language or "zh"

    replies = {
        "zh": "你好，我在。你可以问我榴莲种植、病虫害、品种、成熟度、购买挑选、保存或食用安全相关问题。",
        "en": "Hello. I can help with durian cultivation, pests and diseases, varieties, ripeness, selection, storage, and food-safety questions.",
        "ms": "Hai, saya di sini. Anda boleh tanya tentang penanaman durian, penyakit, perosak, varieti, kematangan, pemilihan, penyimpanan atau keselamatan makan.",
        "th": "สวัสดีครับ ผมช่วยตอบเรื่องการปลูกทุเรียน โรคและแมลง สายพันธุ์ ความสุก การเลือกซื้อ การเก็บรักษา และความปลอดภัยในการกินได้ครับ",
    }

    return replies.get(lang, replies["zh"])


def smalltalk_sse_response(response_language: str = "zh"):
    reply = build_smalltalk_reply(response_language)
...
# User asks Chinese -> answer Chinese.
# User asks Thai -> answer Thai.
# RAG evidence language must not override user language.
# ============================================================
def infer_response_language_from_user_query(query: str, fallback: str = "zh") -> str:
    q = str(query or "").strip()
    fb = str(fallback or "zh").strip().lower()

    # 中文优先：只要用户问题里有中文，就强制中文回答。
    ...
    # 简单马来语识别
    ms_terms = [
        "apa", "kenapa", "bagaimana", "boleh", "tidak", "daun",
        "kuning", "penyakit", "baja", "akar", "buah", "durian",
        "macam mana", "perlu", "sebab",
    ]
    if any(t in lower for t in ms_terms):
        return "ms"
...
def language_lock_instruction(lang: str) -> str:
    ...
    if lang == "zh":
        return (
            "\n\n【输出语言硬性要求】"
            "用户使用中文提问，你必须全程使用简体中文回答。"
            "即使参考资料是泰文、英文或马来文，也必须翻译并整理成中文。"
            "不要输出泰文表头、泰文段落或外语正文。"
        )

    if lang == "th":
        return (
            "\n\n[Language lock] The user is asking in Thai. "
            "Answer in Thai. If references are Chinese, English, or Malay, translate the relevant points into Thai."
        )

    if lang == "ms":
        return (
            "\n\n[Language lock] The user is asking in Malay. "
            "Answer in Malay. If references are Chinese, English, or Thai, translate the relevant points into Malay."
        )

    return (
        "\n\n[Language lock] The user is asking in English. "
        "Answer in English. If references are in another language, translate the relevant points into English."
    )
