...
| 阶段四 | 记忆（Checkpoint/摘要/Token 预算/五级降级） | 5 | **✅ 全部完成**（2026-09-10） | §65 第四阶段 |
| 阶段五 | 权限/评估/运维（权限五项/错误处理/六指标/存储/服务拆分/E2E） | 17 | **✅ 全部完成**（2026-09-10） | §65 第五阶段 |
| 合计 | | **66** | **66 完成 / 0 待办（全部完成）** | §65 |
...
## 阶段四：记忆与长会话（5 任务，✅ 已完成，2026-09-10）

| # | 模块 | 任务 | 文档 | 提交 |
|---|---|---|---|---|
| 56 | 记忆 | Checkpoint 持久化（SqliteSaver 直连构造；跨实例续接验收） | §32 | e4f9a2f |
| 55 | 记忆 | 滑动窗口与摘要（16 条窗口/六保留三剔除/总闸先摘后剪/RemoveMessage 收缩） | §35 | 62b60ea |
| 57 | 记忆 | Token Budget（§34 冻结比例；分节截断；用量报告） | §34 | 488b354 |
| 51 | 记忆 | 五段式组装（§33；business_state 入 summary 预算；SimpleAgent 接入） | §33 | 76eeb9c |
| 48 | 记忆 | 五级降级（L3 关键消息=对话轮次保单调性；错误映射；degrade_until_fits） | §36 | 5c68894 |

实测修正：① 压缩总闸语义——总量未超阈值完全不裁剪（原文零丢失），
超阈值才先摘后剪；② L3 若含大 ToolMessage 会比 L2 还大，关键消息
限定为对话轮次；③ from_conn_string 返回 with 上下文管理器，进程级
持有须用 sqlite3 直连构造。
...
## 阶段五：权限/评估/运维（17 任务，✅ 已完成，2026-09-10）

| # | 模块 | 任务 | 文档 | 提交 |
|---|---|---|---|---|
| 47 | 权限 | Task Tool 幂等验收（Agent retry + 网络 retry 两类重放） | §42 | 98de7ed |
| 49 | 权限 | Tool Permission（Prompt 隐藏 + 执行拦截双层；确认链带角色防越权） | §39 | 63a1c28 |
| 53 | 权限 | Data Permission（tenant/role/orchard 三条件实测隔离） | §40 | cc34a3f |
| 52 | 权限 | 多用户隔离（Checkpoint key=tenantId:userId:threadId） | §37 | 50db93e |
| 54 | 权限 | 二次确认完整流程（Validation 前置，无效草稿不进确认流） | §41 | e1968bc |
| 46 | 错误 | 9 种错误分类与处理策略（§50 目录 + 异常归类） | §50 | 9e94221 |
| 50 | 错误 | Retry 原则（瞬态-only；LLM 2/Tool 2/Rewrite 2；引擎接入） | §51 | 7110693 |
| 59 | 评估 | 全链路 Trace（§52 15 字段；tool_calls 留痕启用） | §52 | ad2efad |
| 11 | 接入 | POST /api/rag/search（§60 契约，复用 #36） | §60 | 923bdae |
| 61 | 评估 | RAG 离线指标（五指标四语种分报，NDCG 手算对照） | §53 | e2c60cb |
| 58 | 评估 | 生成质量指标（五项；Faithfulness/Abstention 重点） | §54 | 61a1281 |
| 60 | 评估 | 多语言评估集（1247 条四语金标规范化 + 难度启发式） | §55 | 3d1bb65 |
| 63 | 评估 | 语料解析覆盖率（四类型分层；真实语料实测） | §56 | 9ee4d44 |
| 64 | 评估 | 长对话成功率（§57 请求级口径；20 轮实测 1.0） | §57 | e05af19 |
| 62 | 基建 | 存储层（八表 SQLite + RedisLike；审计链/幂等/限流） | §58 | 5796c05 |
| 66 | 基建 | 三服务拆分（Agent/RAG/BusinessTool 独立 FastAPI） | §59 | 126a7a5 |
| 65 | E2E | §43 十六步端到端回归（真实四路召回+确认流+记忆上下文） | §43 | 88de3c8 |

## 附录 C：全套验收状态（2026-09-10 收官）

- 66/66 任务完成，每任务独立 commit 并推送 origin main
- 全套测试 **561 项全绿**（unittest；milvus-lite 进程退出段错误吞汇总行，
  以 0 FAIL/ERROR 判定）
- 诚实记账的已知差距：术语库 206 概念/527 别名（文档 1330+ 不实，
  扩充需批量翻译）；alarm_query 按 §39 字面仅 admin（待业务确认）；
  图片/扫描件语料为 0（链路已备）；阈值/权重调优依赖评估集跑批
