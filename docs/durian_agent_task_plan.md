...
| 阶段二 | 检索增强（术语库/改写/重排/证据闭环/生成/引用/多模态建库） | 11 | 🔲 待办 | §65 第二阶段 |
| 阶段三 | ReAct 与工具层（九工具+PolicyGate+确认接口） | 12 | 🔲 待办 | §65 第三阶段 |
...
| # | 模块 | 任务 | 文档 | 状态 |
|---|---|---|---|---|
| 15 | 语义 | 多语言术语库扩充（1330+ 条，canonical_id 体系，CULTIVAR_D197 式 ID） | §10 | 🔲（现有 330 条 glossary 在用，是扩充底座） |
| 18 | 建库 | 图片处理（VLM 描述+图中文字+农业语义） | §21 | 🔲（VLM 需外部 API 或本地多模态模型，调用前须确认通道） |
| 22 | 建库 | 扫描件处理（OCR+版面恢复） | §21 | 🔲 |
| 24 | 检索 | Query Build 跨语言扩展 | §23 | ◐（build_queries 已随 #23 落地，阶段二补完整验收） |
| 28 | 检索 | Query Rewrite 两层机制（规则+LLM，§47 禁猜清单） | §24/§47 | ◐（图内占位=扩展查询重试，换 LLM 内核） |
| 29 | 检索 | Evidence Retry 循环（max 2-3 次+拒答） | §29/§63 | ◐（图内 retry 分支已通，换 LLM 改写内核） |
| 30 | 检索 | Evidence Check 五项判断（实体/条件/数值/冲突/单点） | §29 | ◐（占位=有证据即充分，换五项内核） |
| 31 | 检索 | Final Generation 证据约束生成 | §30/§48 | ◐（rag/generation.py 内核已随 #1 落地，补完整验收：诱导补剂量被拒） |
| 32 | 检索 | Cross Encoder Reranker（Top30-50→Top3-5） | §27 | ◐（图内 rerank 节点占位=透传 RRF 序；本地 bge-reranker-v2-m3 可用） |
| 26 | 检索 | Reranker 阈值机制（验证集调优） | §28 | 🔲 |
| 33 | 检索 | 引用返回（sources 与证据一一对应） | §31 | ◐（rag_sources 已通图+API，补一一对应校验） |

---

## 阶段三：ReAct 与工具层（12 任务，🔲 待办）
