...
class DurianLangGraphRuntimeTest(unittest.TestCase):
    ...
    def test_substantive_query_without_ragflow_evidence_still_generates(self):
        self.runtime.close()
        ...
        self.assertEqual(metadata["answer_mode"], "soft_no_source_generation")
        self.assertEqual(metadata["evidence"], [])
        self.assertIn("answer:火星榴莲应该怎么施肥", content)
        self.assertEqual(self.adapter.generation_calls, calls_before + 1)
