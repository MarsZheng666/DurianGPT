...
def payload_to_message(payload: Dict[str, Any]) -> AnyMessage:
    ...
    extra = {
        key: payload.get(key)
        for key in ("image_url", "active_context_card")
...
def build_followup_retrieval_query(messages: List[AnyMessage], query: str) -> str:
    """Create a standalone retrieval query without an extra LLM call."""
    previous = messages[:-1] if messages else []
    previous_user = _last_content(previous, "user")
    previous_assistant = _last_content(previous, "assistant")
    parts = []
    if previous_user:
        parts.append(previous_user[:220])
    if previous_assistant:
        cleaned = re.sub(r"<!--[\s\S]*?-->", " ", previous_assistant)
        cleaned = re.sub(r"\s+", " ", cleaned).strip()
        parts.append(cleaned[:360])
...
class DurianLangGraphRuntime:
    ...
    def _route_node(self, state: DurianGraphState) -> Dict[str, Any]:
            ...
            route.update(
                {
                    ...
                    "use_history": True,
                    "use_image_context": (
                        (context_resolution.get("card") or {}).get("source") == "image"
                    ),
                    "reason": (
        ...
        if route["intent"] == "follow_up":
            context_messages = (
                system_messages + payloads[-self.max_prompt_messages :]
            )
        elif route["intent"] in {"greeting", "casual"}:
    ...
    def _retrieve_node(self, state: DurianGraphState) -> Dict[str, Any]:
        ...
        if (
            ...
            or not bool(state.get("rag_allowed", state.get("use_rag", True)))
            or not self.adapter.rag_available()
        ):
            return {"evidence": [], "evidence_quality": "none", "evidence_text": ""}

        evidence, quality, evidence_text = self.adapter.retrieve(
            str(state.get("rag_query") or state.get("user_query") or ""),
    ...
    def _compose_node(self, state: DurianGraphState) -> Dict[str, Any]:
        ...
        if bool(state.get("exact_mode")) or bool(
    ...
    def _generate_node(self, state: DurianGraphState) -> Dict[str, Any]:
        ...
        try:
            if bool(state.get("exact_mode")):
                extractor = getattr(self.adapter, "extract_exact_answer", None)
                exact_answer = (
                    extractor(
                        str(
                            state.get("resolved_query")
                ...
                if not exact_answer:
                    not_found = getattr(self.adapter, "exact_not_found", None)
                    exact_answer = (
                        not_found(response_language)
                        if callable(not_found)
                        else "知识库原文中未找到能够完整对应这些空格的内容。"
                    )
                full_response = str(exact_answer)
                ...
                writer({"type": "content", "content": full_response})
            elif grounded_no_source:
                no_source_responder = getattr(
                    self.adapter,
                    "grounded_not_found",
                    None,
                )
                full_response = (
                    no_source_responder(response_language)
                    if callable(no_source_responder)
                    else "RAGFlow 知识库中没有找到足以支持回答的原文，因此我不进行推测。"
                )
                for index in range(0, len(full_response), 12):
                    writer(
                        {
            ...
            logger.exception("LangGraph generation failed")
