        ...
        try:
            ...
            response = httpx.post(
                ...
                json={
                    ...
                    "page": 1,
                    "page_size": max(requested_top_k * 2, 20),
                    "top_k": max(requested_top_k * 8, 64),
            ...
            logger.info(
                "RAGFlow hybrid retrieve: query=%s evidence=%d quality=%s top_score=%.4f vector_weight=%.2f",
...
class DurianGPTModel:
    ...
    def _fit_proxy_budget(
        ...
        while available < minimum_output:
            non_system_indices = [
                index
        ...
        shrink_rounds = 0
        while available < minimum_output and shrink_rounds < 8:
            system_index = next(
                (
                    index
                    for index, message in enumerate(fitted)
        ...
        if (
            ...
            logger.info(
                "OpenAI proxy token budget adjusted: input=%d output=%d->%d max=%d messages=%d->%d system_shrinks=%d",
                input_tokens,
                requested,
                final_output,
                max_model_len,
...
def build_evidence_text(evidence: List[Dict], max_chars: int) -> str:
    ...
    for i, ev in enumerate(evidence, 1):
        if is_weak_synthetic_qa_chunk(ev):
            continue
        text = rag_display_text(ev)[:max_chars]
    ...
    for item in evidence or []:
        ...
        if requested_name:
            ...
            if line_codes:
                after_alias = [
                    match
                    for match in line_codes
                    if match.start() >= alias_in_line + len(source_name_hit)
                ]
                selected = (
                    min(after_alias, key=lambda match: match.start())
                    if after_alias
                    else min(
                        line_codes,
                        key=lambda match: abs(match.start() - alias_in_line),
                    )
                )
            ...
            after_alias = [
                match
                for match in codes
                if match.start() >= alias_in_window + len(source_name_hit)
                and match.start() - alias_in_window <= 160
            ]
            nearest = (
                min(after_alias, key=lambda match: match.start())
                if after_alias
                    ...
                    codes,
                    key=lambda match: abs(match.start() - alias_in_window),
                )
            )
...
def build_messages(
    ...
    if evidence_text:
        grounding_rule = {
            "zh": (
                "\n\n【唯一事实来源】本轮事实性内容只能来自下方 RAGFlow 检索原文。"
        ...
        if response_language == "en":
            system_prompt += (
                "\n\nThe following are the retrieved source passages. Use only relevant statements from them. "
            ...
            system_prompt += (
                "\n\n以下是本轮检索到的 RAGFlow 原文。只能使用其中与问题直接相关的内容；"
                "原文没有的细节必须明确说没有，不得用模型记忆补充一般建议。\n\n"
                f"{evidence_text}"
            )
...
def run_analyze_pest_core_from_bytes(
    ...
    if not evidence:
        response = image_grounded_answer_not_found(
            class_name,
...
def run_analyze_pest_core_from_vl_label(
    ...
    if not evidence:
        response = image_grounded_answer_not_found(
            label_zh if response_language == "zh" else label,
            confidence,
            response_language,
        )
...
def ragflow_evidence_matches_query_anchors(
    ...
    present_domain_terms = [
        term for term in strong_domain_terms if term in query_text
    ]
    if present_domain_terms:
        return any(term in evidence_text for term in present_domain_terms)
    ...
    cleaned = re.sub(r"[的是有和与或在对把给会能了嘛吗呀啊]", " ", cleaned)
...
def retrieve_ragflow_grounded_context(
    ...
    *,
    ...
    if use_exact_mode:
        ...
        logger.info(
            "RAGFlow-only evidence rejected below grounded threshold: query=%s",
...
class DurianLangGraphAdapter:
    ...
    def retrieve(
        ...
        return retrieve_ragflow_grounded_context(
            query,
            response_language,
        )
...
def expand_multilingual_rag_query(query: str, response_language: str = "zh") -> str:
    ...
    if not additions and should_use_rag_for_query(base):
        additions.extend([
            term
            for term in ["榴莲", "durian", "ทุเรียน"]
...
def llamaindex_rag_retrieve(query: str, top_k: int = 3):
    ...
    try:
        ...
        try:
            logger.exception("LlamaIndex RAG retrieval failed: %s", exc)
        except Exception:
            pass
        return [], "none"

    try:
        logger.info("LlamaIndex RAG returned evidence: query=%r count=%d", query, len(evidence))
    except Exception:
        pass
...
async def analyze_pest_stream2(
    ...
    try:
        ...
        # ---------- Stage 1: image recognition ----------
        if not qwen_vl_max_client or not qwen_vl_max_client.api_key:
            raise HTTPException(status_code=503, detail="Qwen-VL-Max client is not available")
        ...
        try:
            ...
            logger.warning("image stream2 RAGFlow-only retrieval failed: %s", e)
            evidence = []
            evidence_quality = "none"
            evidence_text = ""
        ...
        if image_profile == "image_agri_medium":
            ...
            task_hint = (
                "Answer naturally about variety/quality/eating/storage only. "
                "No orchard treatment, commercial grading or pesticide advice."
            )
            image_final_tokens = int(os.getenv("DURIAN_IMAGE_CONSUMER_MAX_NEW_TOKENS", "520"))
        else:
            task_hint = "Answer naturally based on the label and the user's actual question."
...
""".strip()
        ...
        initial_client_evidence = filter_metadata_evidence_for_client(
        ...
        async def event_generator():
            ...
            def record_image_turn(active_context_card: Optional[Dict[str, Any]]) -> None:
            ...
            if not evidence:
                full_response = image_grounded_answer_not_found(
                    label_zh if response_language == "zh" else label,
                    confidence,
                    response_language,
                )
            ...
            # Local-vLLM path: image stream2 should reuse the in-process local model,
            # not call an external OpenAI-compatible server on 8010.
            if not is_openai_proxy_mode():
                try:
                    if not is_model_ready():
                        yield "data: " + json.dumps(
                    ...
                    logger.exception("image stream2 local vLLM generation failed")
                    yield "data: " + json.dumps(
                        {
                            "type": "error",
                            "error": f"本地模型生成失败: {str(exc)}",
                        },
                        ensure_ascii=False,
                    ) + "\n\n"
                    return
            ...
            try:
                ...
                async with _durian_httpx.AsyncClient(timeout=timeout, trust_env=False) as client:
                    async with client.stream(
                        ...
                        if resp.status_code != 200:
                            body = await resp.aread()
                            msg = body.decode("utf-8", errors="ignore")
                            logger.error("image stream2 upstream error: status=%s body=%s", resp.status_code, msg[:1000])
                            yield "data: " + json.dumps(
                                {
                                    "type": "error",
                                    "error": f"vLLM upstream HTTP {resp.status_code}: {msg[:500]}",
                        ...
                        async for line in resp.aiter_lines():
                            ...
                            try:
                                ...
                                logger.warning("image stream2 parse chunk failed: %s line=%s", e, data[:300])
                                continue
                ...
                final_client_evidence = await translate_evidence_for_client(evidence, response_language)
                yield "data: " + json.dumps(
                    {
                        "type": "done",
                ...
                logger.exception("image stream2 failed")
                yield "data: " + json.dumps(
                    {
                        "type": "error",
                        "error": str(e),
        ...
        logger.exception("image stream2 setup failed")
        raise HTTPException(status_code=400, detail=str(e))
